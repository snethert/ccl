    ...
    (ccl::wasm-smoke-f64-add
     (lambda (x y)
       ...
       (let* ((a (%double-float x))
         ...
         (if (= sum 3.0d0) 1 0))))
    (ccl::wasm-smoke-f32-add
     (lambda (x y)
       (declare (fixnum x y))
       (let* ((a (%short-float x))
              (b (%short-float y))
              (sum (+ a b)))
         (declare (single-float a b sum))
         (if (= sum 3.0f0) 1 0))))
    (ccl::wasm-smoke-f64-from-f32
     (lambda (x y)
       (declare (fixnum x y) (ignore y))
       (let* ((sf (%short-float x))
              (df (%double-float sf))
              (df2 (%double-float x)))
         (declare (single-float sf) (double-float df df2))
         (if (= df df2) 1 0))))
    (ccl::wasm-smoke-f32-from-f64
     (lambda (x y)
       (declare (fixnum x y) (ignore y))
       (let* ((df (%double-float x))
              (sf (%short-float df))
              (sf2 (%short-float x)))
         (declare (double-float df) (single-float sf sf2))
         (if (= sf sf2) 1 0))))))
