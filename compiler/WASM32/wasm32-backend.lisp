                     ...
                     ((consp x)
                      (let* ((xs (items x)) (head (car xs)) (n (length (cdr xs))))
                        (case head
                          ...
                          (quote (unless (and (= n 1) (or (null (second xs)) (eq (second xs) t) (and (integerp (second xs)) (<= -536870912 (second xs) 536870911)) (pool-literal-p (second xs)) (assoc (second xs) *b-call-links*) (member (second xs) *b-special-names*) (member (second xs) '(condition serious-condition error simple-condition simple-error type-error control-error warning simple-warning program-error undefined-function)))) (refuse :b-source)))
                          ((flet labels)
