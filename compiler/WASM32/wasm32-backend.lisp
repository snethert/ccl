    ...
    (case op
      ...
      (ccl::self-call
       (or (and *bootstrap-front-end*
                ...
                (if (eq (ccl::afunc-name *pool-current*) 'make-string)
...
(defun b-condition-runtime ()
 (when *b-cpl-conditions* (return-from b-condition-runtime (b-handler-runtime)))
...
(defun b-implicit-runtime ()
 (when *b-cpl-conditions* (return-from b-implicit-runtime (bootstrap-class-implicit-runtime)))
...
(defun bootstrap-condition (mask values)
  (when *b-cpl-conditions* (return-from bootstrap-condition (bootstrap-class-condition mask values)))
  (setq *b-condition-used* t)
  (pushnew "condition_registry" *b-symbols* :test #'equal)
  (b-wat "(block (result i32) ~a)"
      ...
      (let* ((object (first values)) (value (third values))
        ...
        (flet ((typed (kind)
          ...
          (b-wat "(local.set ~a ~a)
                 ...
                 tag (typed :bignum)
                 (b-wat "(if (result i32) (i32.eq (local.get ~a) (i32.const 1020)) (then ~a) (else ~a))"
...
(defun bootstrap-typed-access (op args)
  (let* ((kind (ccl::acode-immediate-operand (car args)))
    ...
    (case kind
      (:bit-vector (return-from bootstrap-typed-access (bootstrap-bit-access (cdr args))))
      (:simple-vector
...
(defun bootstrap-operator (ir)
  (let ((op (ccl::acode-operator-name (ccl::acode-operator ir)))
    ...
    (case op
      ...
      (ccl::%make-uvector (or (bootstrap-allocate-eq-vector args) (bootstrap-allocate-bignum args) (bootstrap-allocate-float args)
...
(defun bootstrap-type-literal (form)
...
(defun bootstrap-type-call (name forms)
  (when (and (member (length forms) '(2 3))
             (or (= (length forms) 2)
                 (and (eq name 'typep)
                      (multiple-value-bind (env constant) (bootstrap-type-literal (third forms))
                        (and constant (null env))))))
    (multiple-value-bind (type constant) (bootstrap-type-literal (second forms))
...
(defun bootstrap-error-call (forms)
  (when (and *b-cpl-conditions* (= (length forms) 2)
