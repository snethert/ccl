...
(defun b-multiple-call (callee forms)
  (let* ((base (temporary)) (context (temporary)) (root (temporary))
         ...
         (self (b-scalar callee))
         (codes (let ((*b-tail-position* nil)) (mapcar #'b-multiple forms))))
    (with-output-to-string (s)
      (format s "(local.set ~a (local.get $top)) (local.set ~a ~a) (local.set ~a ~a) (local.set ~a ~a) (local.set ~a ~a) (local.set ~a ~a)"
        base root (b-load wasm32::tcr.root_head) mv (b-load wasm32::tcr.mv_base)
        owner (b-load wasm32::tcr.mv_owner_top) vsp (b-load wasm32::tcr.vsp) old-count (b-load wasm32::tcr.mv_count))
      ...
      (format s "(local.set ~a ~a)" context (if *b-tail-position* (b-local base) (b-wat "(i32.add (local.get ~a) (local.get $result_bytes))" base)))
      (write-string (b-prepare-context (b-local context) (b-local base) (b-local root) "(i32.const 0)") s)
      (format s "(i32.store offset=40 (local.get ~a) ~a) (local.set ~a (i32.const 0)) (local.set ~a (i32.const 0))" context self n slots)
      (dolist (code codes)
        (write-string code s)
        ...
        (format s "(local.set ~a (local.get ~a)) (block $mv_fill_done (loop $mv_fill (br_if $mv_fill_done (i32.ge_u (local.get ~a) (local.get ~a))) (i32.store (i32.add (local.get ~a) (i32.add (i32.const 48) (i32.mul (local.get ~a) (i32.const 4)))) (i32.const 77825)) (local.set ~a (i32.add (local.get ~a) (i32.const 1))) (br $mv_fill)))" i slots i next-slots context i i i)
        (format s "(memory.copy (i32.add (local.get ~a) (i32.add (i32.const 48) (i32.mul (local.get ~a) (i32.const 4)))) (local.get $results) (i32.mul (local.get $count) (i32.const 4)))" context n)
      ...
      (format s "(call $resolve (i32.load offset=40 (local.get ~a))) (local.set $dispatch_slot) (local.set $dispatch_self) (i32.store offset=40 (local.get ~a) (local.get $dispatch_self))" context context)
      (if *b-tail-position*
        (write-string (b-tail-transfer (b-at (b-local context) 48) (b-local n)) s)
        (progn
          (write-string (b-store wasm32::tcr.vsp (b-at (b-local context) 48)) s)
          (write-string (b-store wasm32::tcr.mv_base (b-local base)) s)
          (write-string (b-store wasm32::tcr.mv_owner_top (b-local context)) s)
