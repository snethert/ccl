...
(defwasm2 wasm2-cons cons (seg vreg xfer y z)
  (declare (ignore vreg))
  (let* ((subprim (wasm2-conslist-star-subprim-fixnum)))
...
(defwasm2 wasm2-list list (seg vreg xfer forms)
  ...
  (if (null forms)
    (if (wasm2-returning-p xfer)
      (wasm2-emit-constant-return (target-nil-value))
      (wasm2-emit-const (target-nil-value)))
    (let* ((subprim (wasm2-conslist-subprim-fixnum))
           (argc (length forms)))
      (wasm2-with-spilled-locals
        (lambda ()
          (dolist (form forms)
...
(defun wasm2-constant-nil-form-p (form)
  (or (and (acode-p form)
           (eq (acode-operator form) (%nx1-operator nil)))
      (let* ((val (nx2-constant-form-value (acode-unwrapped-form-value form))))
        (and val (nx-null val)))))
...
(defwasm2 wasm2-progv progv (seg vreg xfer symbols values body)
  (declare (ignore vreg))
  (when (and (wasm2-constant-nil-form-p symbols)
             (wasm2-constant-nil-form-p values))
    (wasm2-form seg nil xfer body)
    (return-from wasm2-progv nil))
  (let* ((mvpass (wasm2-mv-p xfer))
         (progvsave (wasm2-progvsave-subprim-fixnum))
         (progvrestore (wasm2-progvrestore-subprim-fixnum))
    ...
    (labels ((emit-assign (var emitter)
      ...
      (when rest-var
        (let* ((rest-start (+ (length req) (length optvars)))
               ...
               (exit-label (wasm2-allocate-label))
               (loop-label (wasm2-allocate-label))
               (conslist (wasm2-conslist-subprim-fixnum)))
          (let* ((then-ir (wasm2-with-ir
                            (lambda ()
                              (wasm2-emit :local.get nargs-temp)
...
(defwasm2 wasm2-%consmacptr% %consmacptr% (seg vreg xfer form)
  ...
  (let* ((addr-temp (wasm2-allocate-temp))
         ...
         (count (wasm2-box-fixnum wasm::macptr.element-count))
         (subtag (wasm2-box-fixnum wasm::subtag-macptr)))
    (wasm2-form seg nil nil form)
...
(defwasm2 wasm2-svset svset (seg vreg xfer vector index value)
  (wasm2-%svset seg vreg xfer vector index value)
  nil)
...
(defwasm2 wasm2-%typed-uvref %typed-uvref (seg vreg xfer subtag uvector index)
  ...
  (let* ((subtag-value (or (acode-fixnum-form-p subtag)
         ...
         (proven-slot (and subtag-value (wasm2-proven-subtag-slot subtag-value index))))
    (unless boxed-subtag
      (wasm2-unimplemented))
    (let* ((vec-temp (wasm2-allocate-temp))
           (idx-temp (and (null proven-slot) (wasm2-allocate-temp))))
      (wasm2-form seg nil nil uvector)
      (wasm2-emit :local.set vec-temp)
      (if proven-slot
        (wasm2-emit-misc-slot-ref-with-subtag-guard vec-temp proven-slot subtag-value)
        (progn
          (wasm2-form seg nil nil index)
          (wasm2-emit :local.set idx-temp)
          (wasm2-emit-subtag-misc-ref-fallback-local vec-temp idx-temp boxed-subtag t)))))
  (when (wasm2-returning-p xfer)
    (wasm2-emit :set-arg-z)
    (wasm2-emit :set-nargs 1)
    (wasm2-emit :return))
    ...
    (let* ((vec-temp (wasm2-allocate-temp))
      ...
      (if proven-slot
        ...
        (progn
          (wasm2-form seg nil nil index)
          (wasm2-emit :local.set idx-temp)
          (wasm2-emit-subtag-misc-set-fallback-local vec-temp idx-temp val-temp boxed-subtag t t)))))
  (when (wasm2-returning-p xfer)
    (wasm2-emit :set-arg-z)
    (wasm2-emit :set-nargs 1)
    (wasm2-emit :return))
      ...
      (t
       (let* ((fn-temp (wasm2-allocate-temp)))
         ...
         (dolist (form (cdr args))
           (wasm2-multiple-value-body seg form)
           (wasm2-emit-add-values-boundary))
        (wasm2-emit-recover-values-boundary)
        (wasm2-emit :local.get fn-temp)
        (wasm2-emit :set-nfn)
        (wasm2-emit-funcall-nfn-boundary xfer)))))
...
(defwasm2 wasm2-%aref1 %aref1 (seg vreg xfer vector index)
  (wasm2-uvref seg vreg xfer vector index)
  nil)

(defwasm2 wasm2-%aset1 aset1 (seg vreg xfer vector index value)
  (wasm2-uvset seg vreg xfer vector index value)
  nil)
...
(defun wasm2-emit-call-by-arity (argc mvpass tmp)
  (case argc
    ...
    (t
     (error "WASM2: unsupported call arity: ~d" argc))))
...
(defun wasm2-emit-spread-call (seg xfer fn-emitter args spread-p)
  (let* ((fixed-args (butlast args))
    ...
    (when (wasm2-constant-nil-form-p list-arg)
      (funcall fn-emitter fn-temp)
      (if (> argc 10)
        (wasm2-emit-funcall-subprim seg xfer fn-temp fixed-args)
        (let* ((arg-temps nil))
          ...
          (wasm2-emit-call-by-arity argc mvpass tmp)
          (when (wasm2-returning-p xfer)
            (wasm2-emit :drop)
            (wasm2-emit :return))))
      (return-from wasm2-emit-spread-call nil))
    (funcall fn-emitter fn-temp)
    (dolist (arg fixed-args)
      (wasm2-form seg nil nil arg)
      (wasm2-emit :vpush))
...
(defun wasm2-funcall-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPfuncall))

(defun wasm2-save-values-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPsave-values))

(defun wasm2-add-values-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPadd-values))

(defun wasm2-recover-values-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPrecover-values))

(defun wasm2-progvsave-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPprogvsave))

(defun wasm2-progvrestore-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPprogvrestore))

(defun wasm2-conslist-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPconslist))

(defun wasm2-conslist-star-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPconslist-star))

(defun wasm2-spread-subprim-fixnum (spread-p)
  (wasm2-subprim-fixnum (if (eql spread-p 0) '.SPspread-lexprz '.SPspreadargz)))

(defun wasm2-subtag-misc-ref-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPsubtag-misc-ref))

(defun wasm2-subtag-misc-set-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPsubtag-misc-set))

(defun wasm2-misc-alloc-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPmisc-alloc))

(defun wasm2-misc-alloc-init-subprim-fixnum ()
  (wasm2-subprim-fixnum '.SPmisc-alloc-init))

(defun wasm2-emit-funcall-nfn-boundary (xfer)
  (let* ((mvpass (wasm2-mv-p xfer)))
    (wasm2-with-spilled-locals
      (lambda ()
        (wasm2-emit :call-subprim (wasm2-funcall-subprim-fixnum))
        (wasm2-emit :pending-throw-branch)))
    (unless mvpass
      (wasm2-emit :restore-vsp))
    (if (wasm2-returning-p xfer)
...
(defun wasm2-emit-save-values-boundary ()
  (wasm2-emit-call-subprim-no-spill (wasm2-save-values-subprim-fixnum)))

(defun wasm2-emit-add-values-boundary ()
  (wasm2-emit-call-subprim-no-spill (wasm2-add-values-subprim-fixnum)))

(defun wasm2-emit-recover-values-boundary ()
  (wasm2-emit-call-subprim-no-spill (wasm2-recover-values-subprim-fixnum)))

(defun wasm2-emit-misc-alloc-call-known-constants (boxed-subtag boxed-count)
  (let* ((obj-temp (wasm2-allocate-temp))
         (misc-alloc (wasm2-misc-alloc-subprim-fixnum)))
    (wasm2-emit :get-current-tcr)
    (wasm2-emit :const boxed-subtag)
    (wasm2-emit-unbox-fixnum)
    (wasm2-emit :const boxed-count)
    (wasm2-emit-unbox-fixnum)
    (wasm2-with-spilled-locals
      (lambda ()
        (wasm2-emit :call-external (wasm2-runtime-misc-alloc-import-index))))
    (wasm2-emit :local.set obj-temp)
    (wasm2-emit :local.get obj-temp)
    (wasm2-emit :const (target-nil-value))
    (wasm2-emit :i32-eq)
...
(defun wasm2-emit-misc-alloc-call ()
  (let* ((misc-alloc (wasm2-misc-alloc-subprim-fixnum)))
    (wasm2-emit-call-subprim misc-alloc)
    (wasm2-emit :arg0)))

(defun wasm2-emit-misc-alloc-init-call ()
  (let* ((misc-alloc-init (wasm2-misc-alloc-init-subprim-fixnum)))
...
(defun wasm2-emit-subtag-misc-ref-fallback-local
       (obj-local slot boxed-subtag &optional slot-is-local-p)
  (let* ((misc-ref (wasm2-subtag-misc-ref-subprim-fixnum)))
    (wasm2-with-spilled-locals
      (lambda ()
        (wasm2-emit :local.get obj-local)
        (if slot-is-local-p
...
(defun wasm2-emit-subtag-misc-set-fallback-local
       (obj-local slot value-local boxed-subtag &optional return-value-p slot-is-local-p)
  (let* ((misc-set (wasm2-subtag-misc-set-subprim-fixnum)))
    (wasm2-with-spilled-locals
      (lambda ()
        (wasm2-emit :local.get obj-local)
        (if slot-is-local-p
...
(defun wasm2-subtag-slot-keyword (subtag)
  (cond
    ((eql subtag (nx-lookup-target-uvector-subtag :simple-vector)) :simple-vector)
    ((eql subtag (nx-lookup-target-uvector-subtag :struct)) :struct)))

(defun wasm2-proven-subtag-slot (subtag slot-form)
  (let* ((slot (acode-fixnum-form-p slot-form))
         (keyword (and (typep slot 'fixnum)
                       (wasm2-subtag-slot-keyword subtag))))
    (when (and keyword
               (nx2-constant-index-ok-for-type-keyword slot keyword))
      slot)))
...
(defvar *wasm2-no-spill-subprim-fixnums* nil)

(defun wasm2-no-spill-subprim-fixnums ()
  (or *wasm2-no-spill-subprim-fixnums*
      (setf *wasm2-no-spill-subprim-fixnums*
            (list (wasm2-subprim-fixnum '.SPthrow)
                  (wasm2-subprim-fixnum '.SPmkcatch1v)
                  (wasm2-subprim-fixnum '.SPmkcatchmv)
                  (wasm2-subprim-fixnum '.SPnthrowvalues)
                  (wasm2-subprim-fixnum '.SPnthrow1value)
                  (wasm2-save-values-subprim-fixnum)
                  (wasm2-add-values-subprim-fixnum)
                  (wasm2-recover-values-subprim-fixnum)
                  (wasm2-progvsave-subprim-fixnum)
                  (wasm2-progvrestore-subprim-fixnum)
                  (wasm2-conslist-subprim-fixnum)))))

(defun wasm2-no-spill-subprim-fixnum-p (fixnum)
  (member fixnum (wasm2-no-spill-subprim-fixnums) :test #'eql))
