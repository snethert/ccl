...
export class InitializationOwner {
 ...
 constructor({memory,layout,layoutDigest,modules,table,tail_table}){
  ...
  need(table instanceof WebAssembly.Table&&tail_table instanceof WebAssembly.Table&&table.length>=l.tableCapacity&&tail_table.length>=l.tableCapacity,'ACTUAL_TABLE_CAPACITY');
 ...
 // Reserved control words are immutable zero throughout this protocol.
 #reserved(state){
 ...
 process(id,run){
  need(id===0,'BOOTSTRAP_WORKER');const state=this.#state();this.#reserved(state);
 ...
 worker(id,run){
  ...
  need(Atomics.load(state,0)===2,'PROCESS_NOT_READY');this.#identity();this.#reserved(state);
