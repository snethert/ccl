...
def compile_corpus(out):
    ...
    for name in ['whole-file.lisp','measure.lisp','execute.lisp','cases.lisp','controls.lisp','probes.lisp']:shutil.copy(here/name,driver/name)
...
def run(out):
    ...
    executed={r['definition'] for r in native if not r['definition'].startswith('CORE-')}
    ...
    assert all(r['status']=='ADMITTED' for r in whole)
    save(out/'summary.json',dict(status='PASS',original_definitions_executed=len(executed),
        unchanged_definitions_admitted=sum(r['proposal']=='admitted' for r in throughput['functions']),
        inventory_definitions=len(throughput['functions']),read_skips=len(throughput['skips']),
        callee_closed_definitions=sum(r['package']!='WASM32-COMPILER' and r['name']!='EQL' for r in closed),
        native_cases=len(native),native_comparisons=sum(r['comparisons'] for r in execution),
        ...
        legacy_files_identical=len(legacy),faults=json.loads((out/'faults.json').read_text()),
