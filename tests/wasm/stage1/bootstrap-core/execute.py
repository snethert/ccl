...
s=s.replace("if(row.package==='KEYWORD')", "if(row.package==='CCL'&&row.name==='*PRINT-STRING-LENGTH*')value=44;\n    if(row.package==='CCL'&&row.name==='$NHASH.VECTOR_OVERHEAD')value=56;\n    if(row.package==='KEYWORD')")
...
s=s.replace('const gen=await install({dir,memory,tcr,get,put,collect});','''let integer,floating,integerCalls=0,floatCalls=0,fastChecks=0;
  ...
  put(NIL-1,NIL);put(NIL+3,NIL);[1850,NIL,T,NIL,NIL,0,NIL,0].forEach((v,i)=>put(T-6+4*i,v));
  function services(){
   ...
   for(const [i,[start,end]] of [[77824,77864],[600000,gen.ownerEnd],[850000,gen.symbolEnd],[2097152,gen.imageEnd]].entries())regions.push({name:'image'+i,role:'image',start,end});
...
sys.path.insert(0,str(h));import probe;probe.build(o/'compiled')
