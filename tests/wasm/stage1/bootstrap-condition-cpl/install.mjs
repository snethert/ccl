...
export async function install({dir,memory,tcr,get,put,service,collector,config,result,collect,calculateI,calculateF,ensure}){
 ...
 const classNames=new Set(['CONDITION','SERIOUS-CONDITION','ERROR','WARNING','SIMPLE-CONDITION','SIMPLE-ERROR','TYPE-ERROR','CPL-LEFT','CPL-RIGHT','CPL-DIAMOND','CPL-OTHER']);
 symbols.condition_class_cells=vector(owners.filter(row=>classNames.has(row.name)&&row.package===(row.name.startsWith('CPL-')?'WASM32-COMPILER':'COMMON-LISP')).map(row=>{
   const p=cursor;cursor+=8;put(p,NIL);put(p+4,ownerWords.get(row.id));extraRoots.push(p);classCells.set(row.name,p);return p+1;
 }));
 ...
 let invocations=0;
 return {classCells,functions,mods,env,symbols,keywords,codes,call_error,imageEnd:cursor,symbolEnd:symbolNext,ownerEnd:600000+32*owners.length,keywords,ownerWords,wordOwners,extraRoots,
  reset(mat){
