"""Extend the generic-dispatch driver with the LAP entries of w32-lap.lisp."""
...
'''))
    ...
    if name == 'probes.lisp':
        ...
        needle = '(result (let (('
        assert text.count(needle) == 1
        path.write_text(text.replace(needle, "(result (let ((*b-cpl-conditions* (eql (search \"CORE-CPL-\" (symbol-name name)) 0)) ("))
    if name == 'numeric-files.lisp':
