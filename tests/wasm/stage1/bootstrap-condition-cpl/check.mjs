...
if(isMainThread){
  ...
  let currentGraph;
  const graphIO={get,put,bytes,tcr,encode,decode,conditionWord:name=>get(gen.classCells.get(name)??conditionClasses.get(name)),bindCondition:(name,word)=>{const slot=conditionClasses.get(name);if(slot!==undefined){put(slot,word);addRoot(slot);}const cell=gen.classCells.get(name);if(cell!==undefined)put(cell,word);},bindValue:(name,word)=>put(gen.ownerWords.get(ownerNames.find(x=>x.package+'::'+x.name===name).id)+2,word),bindFunction:(id,word)=>put(gen.ownerWords.get(id)+6,word),functionWord:id=>get(gen.ownerWords.get(id.startsWith('s') ? id : ownerNames.find(x=>x.package+'::'+x.name===id).id)+6)};
  function encode(x){
  ...
  for(const expected of native.filter(x=>process.env.CCL_DISPATCH_METADATA ? x===native[0] : !process.env.CCL_LIBRARY_CASE||(process.env.CCL_LIBRARY_CASE.endsWith('*') ? x.definition.startsWith(process.env.CCL_LIBRARY_CASE.slice(0,-1)) : x.definition===process.env.CCL_LIBRARY_CASE))){ fs.writeSync(2,expected.definition+' '+(expected.args[0]?.graph ? expected.args[0].graph.nodes.length+' graph nodes' : JSON.stringify(expected.args))+'\n');
    ...
    for(const cell of gen.classCells.values())put(cell,NIL);
