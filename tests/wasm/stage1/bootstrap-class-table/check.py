...
def check(out, native):
    ...
    assert len(callers) == 14 and len(new) == 29
    parent = backend.ROOT.parent / 'ccl-evidence/2026-09-23-stage1-condition-system-r1'
    ...
    assert after - before == Counter({name: 1 for name in ('CORE-CONDITION-TABLE-STORE', 'CORE-CONDITION-NEW-CELL', 'CORE-CONDITION-REBUILD-CELLS', 'CORE-CONDITION-TABLE-READ-ONLY')})
    execution = read(out / 'execution.json')
    assert execution['status'] == 'PASS'
    comparisons = sum(r['comparisons'] for r in execution['rows'])
    assert comparisons == 4 * len(rows) == 25984, comparisons
    ...
    summary = dict(status='PASS', original_definition_headline=550,
        ...
        scope='Default-off SETF GETHASH through accepted strong EQ leaf, target-allocated class cells, full cell reconstruction under collection, and SIGNAL source placement. Class objects remain native-projected; implicit registry and cross-dumped image/READY remain open.')
