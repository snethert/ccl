...
# All runtime sources are unchanged from the accepted condition-system packet.
# Reuse its digest-bound service binaries instead of rebuilding musl per run.
import hashlib, tarfile
packet=r.parent/'ccl-evidence/2026-09-23-stage1-condition-system-r1'
sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
assert sha(packet/'packet.json')=='eded372522040c2d9e18e26e11965e0cc147af244bc191a6deaaf08bf194652b'
listed={x['path']:x['sha256'] for x in json.loads((packet/'packet.json').read_text())['files']}
for name in ('source-pins.json','deterministic.json','execution.tar.gz'):
 assert sha(packet/name)==listed[name],name
pins=json.loads((packet/'source-pins.json').read_text())
for name,digest in pins.items():
 if name.startswith('runtime/wasm32/'):assert sha(r/name)==digest,name
expected=json.loads((packet/'deterministic.json').read_text())
with tarfile.open(packet/'execution.tar.gz') as archive:
 for name in ('collector.wasm','integer.wasm','float.wasm','hash.wasm','hash-adapter.wasm','detector.wasm','eql.wasm','stub.wasm'):
  data=archive.extractfile(name).read()
  assert hashlib.sha256(data).hexdigest()==expected[name],name
  (o/name).write_bytes(data)
(o/'runtime-reuse.json').write_text(json.dumps({'packet_sha256':sha(packet/'packet.json'),'source_pins_checked':sum(n.startswith('runtime/wasm32/') for n in pins),'binaries':{name:sha(o/name) for name in ('collector.wasm','integer.wasm','float.wasm','hash.wasm','hash-adapter.wasm','detector.wasm','eql.wasm','stub.wasm')}},indent=2)+'\n')
