...
def sources():
    ...
    parent = read(STORE / '2026-09-23-stage1-condition-system-r1/source-pins.json')
...
def main():
    ...
    if args.mode == 'retain':
        ...
        with tarfile.open(packet / 'development.tar.gz', 'w:gz') as archive:
            for name in ('ccl-class-table-r1/compiled/compile.log', 'ccl-class-table-r2/compiled/compile.log', 'ccl-class-table-r3/execution.log', 'ccl-class-table-r3/scratch-focused.log'):
                archive.add(Path('/tmp') / name, arcname=name, recursive=False)
        save(packet / 'packet.json', dict(id='STAGE1-CLASS-TABLE-R1',
