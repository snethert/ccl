...
export async function install({dir,memory,tcr,get,put,service,collector,config,result,collect,calculateI,calculateF,ensure}){
 ...
 const hashLeaf=new WebAssembly.Instance(hashModule,{env,hash:{run:eqTable.ht_run,collect:()=>{throw Error('unexpected hash collection');},config:0,operation:0,scratch:1800000,scratch_end:1860000,result:1169504}});
 register(1,hashLeaf);const hashFunction=object(1,NIL);
 const hashSetLeaf=new WebAssembly.Instance(hashModule,{env,hash:{run:eqTable.ht_run,collect:()=>{throw Error('unexpected hash collection');},config:0,operation:1,scratch:1800000,scratch_end:1860000,result:1169504}});
