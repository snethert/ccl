"""Keep the old DEFUN denominator and new file-compiler records distinct."""
...
def report(out):
    ...
    with tarfile.open(ROOT.parent/'ccl-evidence/2026-09-22-stage1-bootstrap-funcallable-r1/artifacts.tar.gz') as archive:
        previous=json.load(archive.extractfile('environment/results.json'))
    baseline={(source(row['file']),d['name']):d for row in previous for d in row['definitions']}
    before={(r['file'],r['name']):baseline[(r['file'],r['name'])] for r in old}
    ...
    assert absent<=excluded,sorted(absent-excluded)
    # Keep the historical cohort fixed. Withdrawn native entries gain no credit.
    for k in absent:
        by_key[k]=dict(name=k[1],outcome='TARGET-EXCLUDED')
        records[k]=dict(message='Native pointer/FFI entry excluded; callers remain explicit dependencies.')
