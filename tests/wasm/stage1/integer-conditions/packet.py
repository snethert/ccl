...
HERE=Path(__file__).resolve().parent;BASE='2026-09-19-stage1-integer-calls-r1'
...
def selected(d):
 ...
 return {str(f.relative_to(d)):sha(f) for f in files(d) if (f.suffix in ['.wat','.wasm','.dx64fsl'] or f.name in names or f.name in ['native-results.txt','source-refusals.txt','native-zero-traps.txt'] or f.name.startswith('materialized-')) and not any(n in f.relative_to(d).parts for n in ['proposal','executed-sources'])}
def manifest(p):save(p/'packet.json',dict(id='STAGE1-INTEGER-CONDITIONS-R1',kind='AUXILIARY_INTEGER_CONDITIONS',review_disposition='NOT_REVIEWED',files=[dict(path=str(f.relative_to(p)),sha256=sha(f),bytes=f.stat().st_size) for f in files(p) if f.name!='packet.json']))
...
def retain(e,x,native,p):
 ...
 archive(p/'development.tar.gz',[(f,base.name+'/'+str(f.relative_to(base))) for base in [Path('/tmp/ccl-numcond-r1'),Path('/tmp/ccl-numcond-r2'),Path('/tmp/ccl-numcond-r3'),Path('/tmp/ccl-numcond-r4')] for f in files(base)])
 save(p/'development.json',dict(attempts=[dict(name='ccl-numcond-r1',failure='The actual native bignum zero-divisor trap has no operands; a nested probe reading its first operand attempted TRUNCATE on NIL. Native trap observations retained separately; the documented native condition constructor supplies the field oracle.'),dict(name='ccl-numcond-r2',failure='Harness expected two arithmetic-error slots; native class export includes STATUS as the third slot. Corrected the owner schema expectation, preserving native defaults. Compiler unchanged.'),dict(name='ccl-numcond-r4',failure='The wrong-mask fault fails first at the earlier e_collect case, not e_zero; the first-case label was corrected. No escaped fault.'),dict(name='ccl-numcond-r3',failure='The skipped type-signal mutant removed a format directive, producing invalid WAT. It is not counted as a semantic rejection. Replaced with an executable false predicate retaining the format arguments.')]))
