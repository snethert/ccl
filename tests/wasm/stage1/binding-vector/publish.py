#!/usr/bin/env python3
"""Bind LL17 only after execution, R6 and a fresh complete retained replay."""
...
def publish(evidence,packet):
 ...
 t=next(t for t in inventory['tests'] if t['id']=='S1-LL17-a')
 need(t['status']=='EXECUTED' and t['runner']=='tests/wasm/stage1/binding-vector/qualify.py','registered runner')
 ...
 for role in sorted(set(inventory['required_record_roles']+t['required_record_roles'])):
  ...
  diagnostic='missing artifact roles: S1-LL17-a [generated]'
 ...
 print('S1-LL17-a published NOT_REVIEWED;',len(controls),'role omissions rejected')
