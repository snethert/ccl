...
for name in ('install.mjs','check.mjs','gf-check.mjs','installer-check.mjs'):shutil.copy(h/name,o/name)
...
spec=importlib.util.spec_from_file_location('math_build',h/'math-build.py');build=importlib.util.module_from_spec(spec);spec.loader.exec_module(build);build.build(o,runtime)
...
import snapshot
(o/'snapshot.mjs').write_text(snapshot.derive())
shutil.copy(r/'doc/WASM/contracts/wasm32-layout.v1.json',o/'layout.json')
shutil.copy(h/'snapshot-check.mjs',o/'snapshot-check.mjs')
subprocess.run(['/usr/local/bin/node',o/'snapshot-check.mjs',o/'snapshot-checks.json'],check=True)

shutil.copy(h/'owner-check.mjs',o/'owner-check.mjs')
subprocess.run(['/usr/local/bin/node',o/'owner-check.mjs',o/'collector.wasm',o/'owner-checks.json'],check=True)

(o/'owner-regression.mjs').write_text((h.parent/'collector-owner/check.mjs').read_text().replace("from './owner.mjs'", "from './runtime/collector-owner.mjs'"))
subprocess.run(['/usr/local/bin/node',o/'owner-regression.mjs',o/'collector.wasm',o/'owner-regression.json'],check=True)
