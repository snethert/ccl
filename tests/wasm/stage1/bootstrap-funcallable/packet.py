...
ID='STAGE1-BOOTSTRAP-FUNCALLABLE-R1'

def pins():
    result=parent.pins()
    result.update({str(p.relative_to(ROOT)):sha(p) for p in files(HERE.parent/'bootstrap-arch')})
    result.update({str(p.relative_to(ROOT)):sha(p) for p in files(HERE.parent/'bootstrap-hyperbolic')})
...
def dependencies():
    ...
    for name in ('packet.json','artifacts.tar.gz','recount.json','execution.json'):
        path='2026-09-22-stage1-bootstrap-arch-r1/'+name
        result[path]=sha(EVIDENCE/path)
    for name in ('packet.json','artifacts.tar.gz','execution.json'):
        path='2026-09-22-stage1-bootstrap-hyperbolic-r2/'+name
        result[path]=sha(EVIDENCE/path)
    return result

def source_check(numeric,native):
    for name in ('compile.lisp','cases.lisp','numeric-files.lisp','arch-probes.lisp','controls.lisp','probes.lisp','inputs.lisp','execute.lisp'):
    ...
    expected={backend.BACKEND:backend.generate(),backend.ARCH:backend.arch(),**backend.source_files(ROOT)}
    ...
    for name,text in backend.runtime_files().items():
        assert (numeric/'runtime'/name).read_text()==text,name
    for name in ('check.mjs','install.mjs','gf-check.mjs','installer-check.mjs','snapshot-check.mjs','owner-check.mjs'):
        assert (numeric/name).read_bytes()==(HERE/name).read_bytes(),name
    with tarfile.open(EVIDENCE/'2026-09-22-stage1-bootstrap-hyperbolic-r2/artifacts.tar.gz') as archive:
        for name,text in backend.source_files(ROOT).items():
            assert archive.extractfile('numeric/compiled/proposal/files/'+name).read().decode()==text,name
        for name in ('float.wasm','integer.wasm'):
            assert archive.extractfile('numeric/'+name).read()==(numeric/name).read_bytes(),name
        for f in (numeric/'compiled').glob('*.legacy'):
            assert archive.extractfile('numeric/compiled/'+f.name).read()==f.read_bytes(),f.name
...
def deterministic(environment,numeric):
    ...
    for p in files(environment/'files/l1-numbers'):
        if p.suffix in ('.json','.wat','.wasm'):result['environment/'+str(p.relative_to(environment))]=sha(p)
    for p in files(numeric/'runtime/libm'):
        result['numeric/'+str(p.relative_to(numeric))]=sha(p)
    for p in files(numeric/'readers'):
        if p.suffix=='.json':result['numeric/'+str(p.relative_to(numeric))]=sha(p)
    return {k:v for k,v in result.items() if not k.startswith('numeric/faults/')}
