...
export async function install({dir,memory,tcr,get,put,service,collector,config,result,collect,calculateI,calculateF,ensure}){
 ...
 for(let i=0;i<mods.length;i++){
  ...
  codes[row.name]=id*4;functions.set(row.name,object(id,material.roots[i]));if(row.name==='core_gf_identity'){const raw=functions.get(row.name)-6;put(raw+16,get(material.roots[i]-2));put(raw+20,get(material.roots[i]+2));}compiled.push({row,wasm,id});
 ...
 return {functions,mods,env,symbols,keywords,codes,call_error,imageEnd:cursor,symbolEnd:symbolNext,ownerEnd:600000+32*owners.length,keywords,ownerWords,wordOwners,extraRoots,
  reset(mat){
   ...
   mods.forEach((row,i)=>{const self=functions.get(row.name);put(self+18,mat.roots[i]);if(row.name==='core_gf_identity'){put(self+10,get(mat.roots[i]-2));put(self+14,get(mat.roots[i]+2));}});
