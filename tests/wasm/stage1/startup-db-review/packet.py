...
from run import HERE,ROOT,PARENT,pins,sha,read,save,old
ID='STAGE1-STARTUP-DB-REVIEW-R1'
...
def keep(p):
 for f in files(p):
  ...
  if 'faults' in n.parts and f.name in ['adapter.wasm','db.wasm','previous.mjs','fixed.mjs']:continue
  yield f
...
def main():
 ...
 if a.mode=='retain':
  ...
  save(p/'inputs.json',{name+'/'+n:sha(e/name/n) for name in [PARENT] for n in ['packet.json','source-pins.json','verification.json']})
  save(p/'tools.json',{str(t):sha(t) for t in [Path(old.CLANG),Path(old.NODE)]})
  with tarfile.open(p/'sources.tar.gz','w:gz') as t:
   for f in sorted(HERE.iterdir()):
    if f.is_file():t.add(f,arcname=str(f.relative_to(ROOT)),recursive=False)
  for f in keep(x):
