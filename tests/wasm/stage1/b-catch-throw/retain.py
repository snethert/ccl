...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-CATCH-THROW-R1','scope':'Generated CATCH/THROW and published control records, cleanup composition and loader exit-tag identity; auxiliary execution, no LL05/LL19 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 sources+=[HERE.parent/'b-unwind-protect/wasm32-backend.lisp',HERE.parent/'b-direct-context/mutants.py',ROOT/'compiler/nx1.lisp',ROOT/'lib/macros.lisp',ROOT/'level-1/l1-readloop.lisp']
