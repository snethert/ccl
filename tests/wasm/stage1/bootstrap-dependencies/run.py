...
def summarize(out):
    ...
    executed={r['definition'] for r in native if not r['definition'].startswith('CORE-') and r['definition'] not in ('EQL','FULLTAG','LISPTAG','TYPECODE')}
    ...
    assert len(files)==57 and next(n for path,n,c in files if path.endswith('l0-bignum64.lisp'))=='0'
    for name in ['operator-clauses.lisp','call-clauses.lisp','helpers.lisp','arch.lisp']:
    ...
    save(out/'summary.json',dict(status='PASS',original_definitions_executed=len(executed),
        ...
        callee_closed_definitions=sum(r['static'] and r['package']!='WASM32-COMPILER' and r['name']!='EQL' for r in closed),
