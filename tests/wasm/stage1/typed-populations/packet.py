...
import run as r
ID='STAGE1-TYPED-POPULATIONS-R1'
SOURCES=['run.py', 'packet.py', 'derive.py', 'shape.mjs', 'image.mjs', 'README.md', 'development.json']
P=r.P
...
def pins():
 ...
 for n in ['runtime/wasm32/collector.c', 'runtime/wasm32/collector-owner.mjs', 'runtime/wasm32/bootstrap-populations.mjs', 'runtime/wasm32/sha256.mjs', 'runtime/wasm32/bytes.mjs', 'compiler/WASM32/wasm32-arch.lisp', 'tests/wasm/stage1/collector-owner/check.mjs']:
...
def retained(p):
 for f in files(p):
  ...
  if rel.parts[0]=='faults':
   name=rel.parts[1]
   prefix='collector' if name.startswith('collector') else 'owner' if name.startswith('owner') else 'builder' if name=='old-builder-header' else 'population'
   if len(rel.parts)!=3 or f.name not in [prefix+'.c',prefix+'.mjs',prefix+'.wasm','rejected.log',prefix+'-build.log']:continue
  yield f

def deterministic(p):return {str(f.relative_to(p)):r.sha(f) for f in retained(p) if f.suffix in ['.json','.wasm','.wat','.lisp','.mjs','.dx64fsl','.c']}
