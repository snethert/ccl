#!/usr/bin/env python3
"""Retain and replay selected native reset effects; no result envelope or gate mutation."""
import argparse,shutil,sys,tarfile
from pathlib import Path
from run import HERE,ROOT,read,save,sha,command
ID='STAGE1-STARTUP-RESETS-R1'
SOURCES=('compile.lisp','run.py','check.mjs','install.mjs','selection.json','census.py','controls.py','assess.py','inputs.json','packet.py','README.md','scope.json','development.json')
...
def pins(e):
 ...
 for n in ['query/capture.py','query/native.py','build-flow/flow.py','source-closure/analysis.py']:
  path='tests/wasm/native-census/'+n;source[path]=sha(ROOT/path)
 return source
def retained(out):
 for f in files(out):
  n=f.relative_to(out)
  if f.name.startswith('capture.sqlite'):continue
  if any(x in n.parts for x in ['driver','proposal','source','compile-input']):continue
  if n.parts[0]=='faults' and f.suffix in ['.mjs','.wasm','.json'] and f.name not in ['install.mjs','check.mjs','bootstrap-schedule.mjs','reset_5568.wasm','command.json']:continue
  yield f
...
def retain(e,x,p):
 ...
 for n in ['README.md','scope.json','development.json','install.mjs','selection.json']:shutil.copy(HERE/n,p/n)
 ...
 for n in ['2026-09-13-startup-seeds-r2/packet.json','2026-09-13-startup-seeds-r2/image.json.gz','2026-09-15-correlated-query-base-r1/packet.json']:
  refs[n]=sha(e/n)
