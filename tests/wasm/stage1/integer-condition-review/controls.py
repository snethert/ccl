...
def run(e,out,driver):
 ...
 faults=[
 ('nil-divisor-not-defaulted','(if (i32.eq ~a (i32.const 77825)) (then (i32.store offset=12 ~a (i32.const 4))))','(if (i32.eq ~a (i32.const 51)) (then (i32.store offset=12 ~a (i32.const 4))))','e_review_nil'),
 ('ash-left-first','((= op 3) (list b a))','((= op 3) (list a b))','e_review_ash'),
 ...
 # Admission faults are rejected by explicit source-mode assertions, not
 # classified as runtime failures or counted as faulty generated execution.
 old="(if *b-integer-service* '(list cons function simple-vector integer fixnum or number real truncate)"
 assert code.count(old)==1
 path=out/'quote-default-leak'
 try:driver.compile(e,path,code.replace(old,old.replace('*b-integer-service*','t')))
 except RuntimeError:
  assert 'Default quote admission escaped' in (path/'compiled/compile.log').read_text()
  rows.append(dict(name='quote-default-leak',oracle='Default quote admission escaped',status='REJECTED',kind='ADMISSION_CONTROL'))
 else:raise AssertionError('escaped default quote leak')
 # Run the original biased oracle against the exact same target compiler.
 oracle=(HERE/'compile.lisp').read_text();old="(and (typep a 'real) (eql b 0))";assert oracle.count(old)==1
 path=out/'biased-oracle';driver.compile(e,path,code,oracle.replace(old,'(eql b 0)'))
 try:execute(e,path,driver)
 except subprocess.CalledProcessError:
  assert 'e_review_priority' in (path/'execution.log').read_text()
  rows.append(dict(name='biased-oracle',oracle='e_review_priority',status='REJECTED',kind='ORACLE_CONTROL'))
 else:raise AssertionError('escaped biased oracle')
