...
def compile(e,out,code=None,oracle=None):
 out.mkdir(parents=True,exist_ok=False);h=out/'driver';shutil.copytree(C,h,ignore=shutil.ignore_patterns('__pycache__','review-followup'));shutil.copy(HERE/'compile.lisp',h/'compile.lisp');shutil.copy(HERE/'class-shapes.lisp',h/'class-shapes.lisp');shutil.copy(HERE/'review-native.lisp',h/'review-native.lisp')
 if oracle is not None:(h/'compile.lisp').write_text(oracle)
