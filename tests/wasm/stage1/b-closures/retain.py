...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-CLOSURES-R1','scope':'Generated lexical closures and shared mutable captures; auxiliary execution, no LL05 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 save(out/'development.json',{'failures':['The first IR probe accidentally nested its invocation inside its DEFUN and therefore collected no IR. The corrected probe and original scripts/logs are retained. A deliberate sentinel confirmed the fixture source was loaded.', 'The first compiler derivative had an unclosed source-validator form; the native reader refused it. The failed source and log are retained.', 'A development corpus directly FUNCALLed literal lambdas; U1 rewrote them to LAMBDA-BIND, which this unit refuses. The closure cases now bind function values before calling them, and a static refusal retains that boundary.'], 'superseded':'Successful development compiles and the original mutant trials precede the finalized producer. All failed trials, if any, are retained without reconstruction.'})
 sources=[p for p in HERE.iterdir() if p.is_file()]+list(REG.glob('*.py'))+list(REG.glob('*.lisp'))+[REG/'payload/wasm32-backend.lisp',REG/'payload/xwasm32-fasload.lisp',HERE.parent/'architecture/generate.py',HERE.parent/'b-callables/wasm32-backend.lisp',ROOT/'doc/WASM/tools/r6_registration.py',ROOT/'tests/wasm/native-census/observer.lisp',ROOT/'tests/wasm/native-baseline/tests.lisp',ROOT/'tests/wasm/stage0/abi-decision/decision.json',ROOT/'doc/WASM/contracts/wasm32-layout.v1.json',ROOT/'doc/WASM/contracts/tcr.v1.json']
 pins={}
 for p in sources:
 ...
 save(out/'toolchain.json',tools);save(out/'scope.json',{'accepted_slots_unchanged':True,'inventory_entries_executed':[],'maximum_arguments':'Runtime stack/heap extent; fixed source terms also subject to the existing bounded source-reader budget. Executed through 1024 arguments.','maximum_values':'Runtime caller-owned output reservation, propagated as a resource budget; positive tests return 1024 values. No automatic growth. Minimum four scratch words permit scalar evaluation with an empty final reservation.','entry_frame_bytes':'align16(8 + max(16, owner - output) + 4 * bound_variable_slots)', 'callable_objects':'Generated 24-byte logical function objects; D1 simple-vector environments of shared D1 cons cells. Static top-level objects and code registry remain fixture-materialized; registry metadata is owner-supplied, not loader-authenticated. Captured cells are conservatively allocated on function entry, once per captured local binding per activation; untaken LET branches may retain unused cells.','keyword_identities':'explicit fixture-supplied imports, no production namespace or symbol layout claim','root_record':'previous:u32, count:u32, tagged words','stores':{'arguments and retained values':'tagged root slots','result words':'TCR descriptor ownership','frame links and counts':'raw metadata','cons writes':'tagged heap fields, including mutable capture-cell CARs; no polling/collection path','closure objects and environments':'tagged heap fields; complete extent checked before writes and allocation pointer publication; captured cell identity is shared across siblings' },'remaining':['collection and allocator slow path','dynamic reservation growth and capacity policy at production adapters','FLET/LABELS and inlined LAMBDA-BIND IR','full Lisp condition path','lazy stubs and role-authenticated installation','tail chains and cleanup/binding extent','live-local spill/reload under collection']});manifest(out)
