...
(defun b-bind-value (var value)
  (if var (b-wat "(i32.store ~a ~a)" (b-variable-address var) value) ""))
...
(defun b-scalar (ir)
  (let ((op (ccl::acode-operator-name (ccl::acode-operator ir))) (args (ccl::acode-operands ir)))
    (case op
      ...
      ((ccl::call ccl::values ccl::progn ccl::multiple-value-prog1 ccl::prog1 ccl::if ccl::let ccl::let*)
...
(defun b-one-module (afunc)
  (let* ((ir (ccl::afunc-acode afunc)) (args (ccl::acode-operands ir)))
    ...
    (let* ((*required-vars* (first args)) (*temporary-count* 0) (*b-imports* nil) (*b-keywords* nil) (*b-symbols* nil) (*b-code-imports* nil)
           (*b-inherited* (mapcar #'ccl::nx-root-var (ccl::afunc-inherited-vars afunc)))
    ...
    (labels ((walk (x)
               (cond ((ccl::acode-p x)
                      (let ((op (ccl::acode-operator-name (ccl::acode-operator x))) (args (ccl::acode-operands x)))
                        (when (member op '(ccl::let ccl::let*)) (dolist (v (first args)) (pushnew v vars :test #'eq)))
...
(defun b-make-closure (afunc)
  (let* ((entry (assoc afunc *b-functions* :test #'eq))
         (vars (mapcar #'ccl::nx-root-var (ccl::afunc-inherited-vars afunc)))
...
(defun b-call-pass2 (root)
  (let ((*b-functions* nil) (*b-captured* nil) (counter 0) (name *module-name*))
    (labels ((collect (f)
                        ...
                        (case head
                          ...
                          (function
                           ...
                           (if (and (consp (second xs)) (eq (car (second xs)) 'lambda))
                             ...
                             (unless (assoc (second xs) *b-call-links*) (refuse :b-source))))
                 ...
                 (labels ((add-var (v) (variable v) (when (member v vars) (refuse :b-source)) (push v vars)))
                   (dolist (parameter (items (second parts)))
                     (cond ((eq parameter '&optional) (unless (eq mode :required) (refuse :b-source)) (setq mode :optional))
                           ...
                           (t (refuse :b-source)))))
                 (when (eq mode :rest) (refuse :b-source)) (walk (third parts) (append vars outer) (1+ depth)))))
