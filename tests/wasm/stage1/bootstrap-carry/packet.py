"""Retain and replay the carry-item proposal, with checkout-relative pins."""
import argparse
...
def dependencies():
    ...
    paths += [r.EVIDENCE / '2026-09-21-stage1-bootstrap-dependencies-r1/execution/execution-frontier.json']
    for path in paths:
...
def native_check(path):
    prior.assert_native(path)
    reference = r.EVIDENCE / ACCEPTED / 'native'
    report = read(path / 'run.json')
    assert report['registered_tests_reuse']['run_sha256'] == sha(reference / 'run.json')
    for name in ('registered-fasls.json', 'registered-snapshot.json'):
        assert read(path / name) == read(reference / name)


def source_check(out):
    for name in ('compile.lisp', 'cases.lisp', 'execute.lisp', 'probes.lisp',
                 'controls.lisp', 'whole-file.lisp', 'witnesses.lisp'):
...
def main():
    ...
    for name in ('packet', 'execution', 'native-run', 'output'):
    ...
    if args.mode == 'retain':
        ...
        r.run(out)
        source_check(out)
        actual, expected = deterministic(out), read(packet / 'deterministic.json')
