...
;;; Establish the compile-time constants needed by the reader in these files.
;;; Other compile-time effects (especially foreign-interface acquisition) are
;;; not evaluated by this inventory. A read failure still stops the file.
(defun library-reader-constants (form)
  (when (consp form)
    (case (car form)
      (eval-when
       (when (intersection (second form) '(:compile-toplevel compile))
         (labels ((visit (x)
                    (when (consp x)
                      (case (car x)
                        (defconstant (eval x))
                        (progn (mapc #'visit (cdr x)))))))
           (mapc #'visit (cddr form))))))))
               ...
               (let ((offset (file-position stream)) (end (gensym "EOF")))
                 (handler-case
                     (let ((form (read stream nil end)))
                       ...
                       (when (member file '("level-0/l0-bignum32.lisp" "level-0/l0-float.lisp" "level-1/l1-cl-package.lisp") :test #'equal)
                         (library-reader-constants form))
