...
def report(out):
    ...
    executed={r['definition'] for r in native if not r['definition'].startswith('CORE-') and r['name'] not in primitive and r['definition'] not in ('FULLTAG','LISPTAG','TYPECODE','ASSQ')}
    ...
    nil_only=sorted(n for n in executed if all(all(v is None for v in r['values']) for r in native if r['definition']==n))
    save('member-witnesses.json',dict(executed=len(executed),non_nil_witness=len(executed)-len(nil_only),nil_only=nil_only,
    ...
    baseline=Path(__file__).resolve().parents[5]/'ccl-evidence/2026-09-21-stage1-bootstrap-dependencies-r1/execution/execution-frontier.json'
    ...
    save('progress.json',dict(baseline_executed=252,executed=len(executed),new_executions=sorted(executed-set(before['names'])),
        ...
        reader_files_complete_before=35,reader_files_complete_after=38,
