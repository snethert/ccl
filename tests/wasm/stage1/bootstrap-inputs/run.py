...
def summarize(out):
    ...
    executed={r['definition'] for r in native if not r['definition'].startswith('CORE-') and r['definition'] not in ('EQL','FULLTAG','LISPTAG','TYPECODE','ASSQ')}
    ...
    assert len(files)==56 and not any(path.endswith('linux-files.lisp') for path,n,c in files) and next(n for path,n,c in files if path.endswith('l0-bignum64.lisp'))=='0'
    save(out/'summary.json',dict(status='PASS',original_definitions_executed=len(executed),
        ...
        native_cases=len(native),target_comparisons=sum(r['comparisons'] for r in execution),
        native_comparisons=sum(r['comparisons'] for r in execution)-4,
