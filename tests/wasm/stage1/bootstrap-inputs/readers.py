...
def run(source,proposal,out):
    ...
    substitutions=[]
    changed=derive(ROOT)
    for name in list(FILES)+['level-1/level-1.lisp']:
        ...
        if name in FILES:
            ...
            for key,value in CONSTANTS.items():
                ...
                count=len(re.findall(re.escape(old)+r'\b', original))
                if count:
            ...
            old='(l1-load "linux-files")';new='#-wasm32-target '+old
            assert inverse.count(new)==1
            inverse=inverse.replace(new,old)
            before.append(old);after.append(new)
            substitutions.append(dict(file=name,token=old,occurrences=1))
    ...
    paths=list(FILES)+['level-1/level-1.lisp']
