...
from sources import FILES, MODULE_ANCHOR, MODULE_ADDITION
...
def compare_source(before, after, source_before, source_after):
    comparator = locations.compare if '(l1-load "linux-files")' in source_before else native_source.compare
    return comparator(before, after, source_before, source_after)

def source_pairs():
    paths=['level-0/'+s+'.lisp' for s in ('l0-def','l0-pred','l0-utils','l0-symbol')]+list(FILES)+['level-1/level-1.lisp']
...
text=text.replace("{'CCL::TARGET-COMPILER-MODULES','CCL::TARGET-XLOAD-MODULES'}", "{'CCL::TARGET-COMPILER-MODULES','CCL::TARGET-XLOAD-MODULES','CCL::TARGET-LEVEL-1-MODULES'}").replace("len(old)-2", "len(old)-3")
...
# Extend the existing exact source-insertion proof by one target-only early
# return. Every other decoded component is still compared byte for byte.
import types
fasl=types.ModuleType('file_provider_fasl');fasl.__file__=str(REG/'fasl.py')
source=(REG/'fasl.py').read_text()
needle="    expected=source_before;positions=[]"
assert source.count(needle)==1
source=source.replace(needle,"    insertions.append((MODULE_ANCHOR,MODULE_ADDITION))\n"+needle)
source=source.replace("{'CCL::TARGET-XLOAD-MODULES','CCL::TARGET-COMPILER-MODULES'}", "{'CCL::TARGET-XLOAD-MODULES','CCL::TARGET-COMPILER-MODULES','CCL::TARGET-LEVEL-1-MODULES'}").replace('len(changed)==2','len(changed)==3').replace('len(a)-2','len(a)-3')
fasl.MODULE_ANCHOR=MODULE_ANCHOR;fasl.MODULE_ADDITION=MODULE_ADDITION
exec(compile(source,str(REG/'fasl.py'),'exec'),fasl.__dict__)
driver.compare_compiler=fasl.compare_compiler
