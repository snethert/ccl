...
def native_check(path):
    ...
    assert report['r6']['identical'] == 150 and report['restored_fasls'] == 164
...
def main():
    ...
    for name in ('packet', 'execution', 'native-run', 'readers', 'replay-repair', 'output'):
    ...
    if args.mode == 'retain':
        ...
        assert len(matrix['rows']) == 136 and all(row['equal'] for row in matrix['rows'])
        packet.mkdir()
        shutil.copytree(args.readers.resolve(), packet / 'readers')
        repair = args.replay_repair.resolve()
        assert read(repair / 'integration.json')['status'] == 'PASS'
        (packet / 'replay-repair').mkdir()
        for name in ('integration.json', 'owner-check/results.json'):
            dest = packet / 'replay-repair' / name
            dest.parent.mkdir(exist_ok=True)
            shutil.copy(repair / name, dest)
        r.save(packet / 'replay-repair/provenance.json', dict(commit='f63a4a3b', clean_checkout=True,
            command='python3 tests/wasm/stage1/bootstrap-witnesses-acceptance/run.py target --output OUTPUT',
            identical_files=885,collector_checks=40))
