...
export async function install({dir,memory,tcr,get,put,service,collector,config,result,collect,calculateI,calculateF}){
 const NIL=77825,registry=4096,table=new WebAssembly.Table({element:'anyfunc',initial:1024}),tail_table=new WebAssembly.Table({element:'anyfunc',initial:1024});
 ...
 put(registry,1024);put(registry+4,1);
 ...
 for(const {row,wasm,id} of compiled){const instance=new WebAssembly.Instance(wasm,{env,hash,probe:{collect},integer:{calculate:calculateI},floating:{calculate:calculateF},symbols:{...symbols,...Object.fromEntries(row.symbols.map(([wire,id])=>[wire,ownerWords.get(id)]))},keywords,codes});register(id,instance);entries.set(row.name,{fn:instance.exports.entry,self:functions.get(row.name)});}
 let invocations=0;
 return {call_error,imageEnd:cursor,symbolEnd:symbolNext,ownerEnd:600000+32*owners.length,keywords,ownerWords,wordOwners,extraRoots,
  reset(mat){
