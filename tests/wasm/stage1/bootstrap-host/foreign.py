...
def sites(text):
    ...
    while i<len(text):
        ...
        out[start:i]=' '*(i-start)
    return list(re.finditer(r'#([$_])\s*([A-Za-z0-9_]+)', ''.join(out)))
...
def qualify(out):
    ...
    for file in files():
        ...
        for m in sites(original):
            offset=m.start()+shift
            if m[1]=='$' and m[2] in CONSTANT_VALUES:
                new='#+wasm32-target target::'+target_name(m[2])+' #-wasm32-target '+m[0]
                shift+=len(new)-len(m[0])
            else:
    ...
    for row in data['sites']:
        ...
        row['implemented']=row['disposition']=='target-protocol-constant'
    ...
    assert sum(r['active_wasm_foreign_reference'] for r in data['sites'])==100
    assert not any(r['kind']=='$' for r in active['rows'])
    data['summary']=dict(sites=len(data['sites']),constant_branches=sum(r['implemented'] for r in data['sites']),constant_identifiers=len(CONSTANT_VALUES),active_functions=100,active_foreign_types=[r for r in active['rows'] if r['kind']=='>'],by_disposition=dict(Counter(r['disposition'] for r in data['sites'])))
