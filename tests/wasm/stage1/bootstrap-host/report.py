def pending_reason(name):
    ...
    return 'Recipe still owed: constructor/state or structured result transport must be added.'
...
def report(out):
    ...
    baseline=Path(__file__).resolve().parents[5]/'ccl-evidence/2026-09-21-stage1-bootstrap-inputs-r1/execution/execution-frontier.json'
    ...
    save('progress.json',dict(baseline_executed=332,executed=len(executed),new_executions=sorted(executed-set(before['names'])),
        ...
        reader_files_complete_before=39,reader_files_complete_after=sum(x=='T' for x in __import__('re').findall(r'\("[^"]+" \d+ (T|NIL)\)',(out/'compiled/worklist.sexp').read_text())),
