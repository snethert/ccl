...
def pins():
 ...
 paths.update((ROOT/'runtime/wasm32').glob('*.mjs'))
 for n in ['tests/wasm/stage1/symbols/generated.py','compiler/WASM32/wasm32-backend.lisp','compiler/WASM32/wasm32-arch.lisp','compiler/X86/X8632/x8632-arch.lisp','compiler/ARM/arm-arch.lisp','doc/WASM/contracts/wasm32-layout.v1.json','doc/WASM/contracts/tcr.v2.json','doc/WASM/stage1/integration-ll21a.json','runtime/wasm32/stub.wat','doc/WASM/tools/gate.py','doc/WASM/tools/evidence_binding.py']:paths.add(ROOT/n)
...
def retained(out):
 for p in files(out):
  ...
  if 'faults' in n.parts and any(part in ['compiled','installed','malformed'] for part in n.parts):continue
  yield p
...
def retain(e,out,p):
 ...
 for n in ['summary.json','assessment.json','controls.json','publication-controls.json','range-derivation.json']:shutil.copy(out/n,p/n)
 ...
 save(p/'options.json',dict(profile='shared-generated-initialization',placements=[4194304,2147483648],workers=3,seed='DETERMINISTIC'))
 refs={n:sha(e/n) for n in ['2026-09-12-native-census-r7/baseline/build/dx86cl64','2026-09-16-stage1-1a-r2/native/baseline.image']}
 ...
 save(p/'native-reuse.json',dict(status='PASS',compiler_sha256=sha(compiler),qualification=q,scope='Accepted integrated compiler unchanged; all generated initializers and native semantics freshly compiled. R6/R6a reused by exact compiler hash. No shared compiler or upstream kernel edit.'))
 shutil.copy(compiler,p/'wasm32-backend.lisp');shutil.copy(HERE/'owner.mjs',p/'owner.mjs')
 tools=[]
