...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-TAIL-CALLS-R1','scope':'Generated proper tail transfers and stack-temporary literal APPLY; auxiliary execution, no LL05 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
