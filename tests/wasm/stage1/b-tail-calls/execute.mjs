...
let checks=0,peakRoots=0,activeFrame=0,activeRoots=0,topContext=0;
...
for(const observed of [false,true]){
 ...
 for(const m of mods){
  ...
  const instance=new WebAssembly.Instance(compiled.get(m.name),{env:{memory,tcr,table,tail_table,code_registry:4096,call_error,type_error},symbols,keywords,codes});
 ...
 for(const start of [65536,2147483648]){
  ...
  for(const missing of ['null','range']){
   const slot=mods.findIndex(m=>m.name==='v1')+1,old=tail_table.get(slot);
   const entry=missing==='range'?new WebAssembly.Instance(compiled.get('dynamic_one'),{env:{memory,tcr,table,tail_table:new WebAssembly.Table({element:'anyfunc',initial:1,maximum:1}),code_registry:4096,call_error,type_error}}).exports.entry:functions.get('dynamic_one');
   ...
   try{entry(handles.get('dynamic_one'),2);}catch(e){assert(e instanceof WebAssembly.Exception&&e.is(call_error),'tail table checked refusal');reason=e.getArg(call_error,0);}finally{tail_table.set(slot,old);}
...
parentPort.postMessage({status:'PASS',tail_runs:tailRuns,non_tail_checks:nonTailChecks,tail_checks:tailChecks,modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:60,callable_checks:callableChecks,closure_checks:closureChecks,local_checks:localChecks,result_capacity_checks:resultChecks,allocation_boundary_checks:8});
}
