...
def run(evidence,native,qualification,out):
 ...
 summary={'status':'PASS','modules':target['modules'],'native_cases':len(read(out/'positive/cases.json')),'comparisons':target['comparisons'],'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'development_regressions':len(regression),'local_checks':target['local_checks'],'resource_refusals':target['resource_refusals'],'allocation_boundary_checks':target['allocation_boundary_checks'],'result_capacity_checks':target['result_capacity_checks'],'callable_checks':target['callable_checks'],'closure_checks':target['closure_checks'],'ownership_inspections':target['ownership_inspections'],'peak_root_frames':target['peak_root_frames'],'scope':'Generated FLET/LABELS, lexical self and mutual recursion, escaping local-function groups, inline LAMBDA-BIND and literal APPLY. Independent pre-emitter IR root contracts; owner-supplied registry, no GC, tail transfer, authenticated lazy installation or full LL05 credit.'};save(out/'summary.json',summary);print('S1-B-LOCAL-CALLS-PASS',summary)
