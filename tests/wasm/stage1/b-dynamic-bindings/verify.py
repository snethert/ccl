...
def verify(packet,evidence):
 for name in ('loader.mjs','binary.mjs','stub.wat'):
  require((HERE/name).read_bytes()==(HERE.parent/'b-catch-throw'/name).read_bytes(),'REVIEWED_LOADER_UNCHANGED '+name)
 ...
 temp=Path(tempfile.mkdtemp(prefix='ccl-dynamic-verify-')).resolve()
 try:
 ...
 print('S1-B-DYNAMIC-BINDINGS-VERIFIED: native R6/R6a, generated special parameters, PROGV and unwind restoration, native comparisons, mutants, lazy composition and refusals')
