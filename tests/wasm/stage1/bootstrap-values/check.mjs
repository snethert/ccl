...
if(isMainThread){
  ...
  const gen=await install({dir,memory,tcr,get,put,collect});
  ...
  for(const expected of native){
    ...
    gen.reset(movingPools[base]);
    put(config+76,gen.extraRoots.length);
    gen.extraRoots.forEach((slot,i)=>put(1180000+4*i,slot));
  ...
  parentPort.postMessage({base,comparisons:rows.length,collections,internalCollections,rows,...gen.summary()});
}
