...
def compile_proposal(out):
    ...
    for name in ('compile.lisp','measure.lisp','controls.lisp','legacy.lisp','extra.lisp'):
    ...
    spec=importlib.util.spec_from_file_location('values_compile',p)
    ...
    owners=json.loads((out/'compiled/symbols.json').read_text())
    mat=compile_pool(json.loads((out/'compiled/pools.json').read_text()),{x['id']:600006+32*i for i,x in enumerate(owners)}).at(2097152)
    save(out/'compiled/materialized.json',dict(image=mat.image.hex(),roots=mat.roots))
    pool=compile_pool(json.loads((out/'compiled/pools.json').read_text()),{x['id']:600006+32*i for i,x in enumerate(owners)})
    save(out/'compiled/moving-pools.json',{str(base):dict(image=pool.at(base).image.hex(),roots=pool.at(base).roots) for base in [262144,2147483648]})
...
def compare_legacy(out):
    ...
    spec=importlib.util.spec_from_file_location('values_baseline',driver/'compile.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
...
def run(out):
    ...
    build(out/'compiled')
    subprocess.run([sys.executable,HERE/'frontier.py',out/'compiled'],check=True)
    compare_legacy(out)
    ...
    spec=importlib.util.spec_from_file_location('values_qualification',HERE/'qualification.py')
    ...
    save(out/'summary.json',dict(status='PASS',inventory_functions=len(throughput['functions']),
        ...
        reader_and_name_skips=len(throughput['skips']),executed_original_definitions=len(json.loads((out/'compiled/originals.json').read_text())),
        ...
        internal_collections=sum(r['internalCollections'] for r in execution['rows']),
        static_dependency_closed=json.loads((out/'compiled/frontier.json').read_text())['static_dependency_closed'],
