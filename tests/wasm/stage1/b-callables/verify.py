...
def verify(packet,evidence):
 require((HERE/'wasm32-backend.lisp').read_text().split(';;; First B call unit:')[0]==(HERE.parent/'b-bindings/wasm32-backend.lisp').read_text().split(';;; First B call unit:')[0],'REVIEWED_NON_B_ENTRIES_UNCHANGED')
 ...
 with tempfile.TemporaryDirectory(prefix='ccl-b-call-verify-') as tmp:
  ...
  for name,backend in [('positive',None)]+list(proposed.items()):
   ...
   for p in original.iterdir():
    if p.suffix in ('.wat','.wasm') or p.name in ('native.json','cases.json','refusals.json','modules.json','compiler.dx64fsl','layout.json'):
 ...
 print('S1-B-CALLABLES-VERIFIED: native R6/R6a, object dispatch, live binding and recursion')
