...
def cases():
 out=[]
 def add(name,args,nodes=None,capacity=64,bindings=None):
  global BINDINGS
  ...
  out.append(dict(id=f'{name}-{len(out):03}',function=name,args=args,capacity=capacity,bindings=BINDINGS,nodes=before,expected=expected,allocated_cells=len(after)-len(before)))
 for n in (0,1,2,3,4,5,6,16,32,64):
...
REFUSALS=[('escaped-key-literal','(lambda () (values :A :|a|))'),('escaped-key-parameter','(lambda (&key ((:|a| x))) x)'),('host-macro','(lambda (p) (typep p \'fixnum))'),('missing-rest-var','(lambda (&rest) nil)'),('rest-extra-var','(lambda (&rest p q) p)'),('aux','(lambda (&aux p) p)'),('bad-key','(lambda (&key ((p x))) x)'),('bad-order','(lambda (&key a &optional b) a)'),('duplicate-var','(lambda (a &optional a) a)'),('forward-default','(lambda (&optional (a b) b) a)'),('macro-default','(lambda (&optional (a (typep nil \'fixnum))) a)'),('apply-missing-tail','(lambda (p) (apply p))'),('lambda-value','(lambda (p) (lambda () p))'),('unknown','(lambda (p) (unlinked p))'),('empty-prog1','(lambda () (prog1))'),('heap-constant','(lambda () \'(1 2))'),('cycle','(lambda (p) #1=(progn . #1#))'),('dotted','(lambda (p) (v1 . p))'),('extra-form','(lambda (p) p) 2')]
