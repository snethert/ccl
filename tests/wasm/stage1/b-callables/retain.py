...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-CALLABLES-R1','scope':'Generated object and symbol dispatch with live bindings and recursion; auxiliary execution, no LL05 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 save(out/'development.json',{'failures':['The first redefinition corpus serialized Python binding tuples as Python syntax rather than Lisp lists. Native CCL refused the input before execution. The failed capture and the consequent missing-module attempt are retained; the renderer now emits lists.'],'superseded':'Positive development compiles and mutant trials preceded the finalized producer and retained replay.'})
 sources=[p for p in HERE.iterdir() if p.is_file()]+list(REG.glob('*.py'))+list(REG.glob('*.lisp'))+[REG/'payload/wasm32-backend.lisp',REG/'payload/xwasm32-fasload.lisp',HERE.parent/'architecture/generate.py',HERE.parent/'b-results/wasm32-backend.lisp',ROOT/'doc/WASM/tools/r6_registration.py',ROOT/'tests/wasm/native-census/observer.lisp',ROOT/'tests/wasm/native-baseline/tests.lisp',ROOT/'tests/wasm/stage0/abi-decision/decision.json',ROOT/'doc/WASM/contracts/wasm32-layout.v1.json',ROOT/'doc/WASM/contracts/tcr.v1.json']
 pins={}
 for p in sources:
 ...
 save(out/'toolchain.json',tools);save(out/'scope.json',{'accepted_slots_unchanged':True,'inventory_entries_executed':[],'maximum_arguments':'Runtime stack/heap extent; fixed source terms also subject to the existing bounded source-reader budget. Executed through 1024 arguments.','maximum_values':'Runtime caller-owned output reservation, propagated as a resource budget; positive tests return 1024 values. No automatic growth. Minimum four scratch words permit scalar evaluation with an empty final reservation.','entry_frame_bytes':'align16(8 + max(16, owner - output) + 4 * bound_variable_slots)', 'callable_objects':'Proposed 24-byte logical function layout, D1 symbol layout, fixture-materialized; all Lisp calls use the validated table path. Registry signature/role metadata is owner-supplied, not loader-authenticated.','keyword_identities':'explicit fixture-supplied imports, no production namespace or symbol layout claim','root_record':'previous:u32, count:u32, tagged words','stores':{'arguments and retained values':'tagged root slots','result words':'TCR descriptor ownership','frame links and counts':'raw metadata','cons writes':'tagged heap fields; initialized before allocation pointer and bound rest root publication; no polling/collection path'},'remaining':['collection and allocator slow path','dynamic reservation growth and capacity policy at production adapters','lexical closure construction and mutation','full Lisp condition path','lazy stubs and role-authenticated installation','tail chains and cleanup/binding extent','live-local spill/reload under collection']});manifest(out)
