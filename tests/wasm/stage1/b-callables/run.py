...
def run(evidence,native,qualification,out):
 ...
 summary={'status':'PASS','modules':target['modules'],'native_cases':len(read(out/'positive/cases.json')),'comparisons':target['comparisons'],'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'resource_refusals':target['resource_refusals'],'allocation_boundary_checks':target['allocation_boundary_checks'],'result_capacity_checks':target['result_capacity_checks'],'callable_checks':target['callable_checks'],'ownership_inspections':target['ownership_inspections'],'peak_root_frames':target['peak_root_frames'],'scope':'Generated function-object and symbol designator dispatch, live function-cell lookup, retained old objects and recursion. Fixture-materialized object proposal and owner-supplied code registry; no closure allocation, collection, lazy installation or full LL05 slot credit.'};save(out/'summary.json',summary);print('S1-B-CALLABLES-PASS',summary)
