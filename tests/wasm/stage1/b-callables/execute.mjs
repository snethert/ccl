...
const observations=[];let resultChecks=0,callableChecks=0;
for(const observed of [false,true]){
 ...
 for(const m of mods){
  const instance=new WebAssembly.Instance(compiled.get(m.name),{env:{memory,tcr,table,code_registry:4096,call_error,type_error},symbols,keywords});
  const entry=instance.exports.entry;functions.set(m.name,entry);
  const wrapper=observed?new WebAssembly.Instance(observer,{env:{entry,observe:self=>{inspect('dispatch '+m.name);assert.equal(load(u32(self)-2),4*(mods.findIndex(x=>x.name===m.name)+1),'resolved self code');}}}).exports.entry:entry;
...
parentPort.postMessage({status:'PASS',modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:60,callable_checks:callableChecks,result_capacity_checks:resultChecks,allocation_boundary_checks:8});
}
