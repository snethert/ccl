...
def run(positive,expected,out):
 ...
 try:
  ...
  summary={'status':'PASS','compared_fields':fields,'comparisons':actual['comparisons'],'tail_chains':len(actual['tail_runs']),'cold_installations':[sum(e['event']=='INSTALLED'for e in r['events'])for r in actual['lazy_runs']], 'scope':'The reviewed loader with the temporary-result profile bump composes with generated MVC storage. The eager run additionally poisons the public table and checks internal-entry layout; those additional checks are not claimed for this inherited lazy harness.'}
