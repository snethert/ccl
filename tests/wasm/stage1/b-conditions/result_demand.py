...
def instrument(compiled, out):
    ...
    for p in (compiled/'installed').glob('*.wasm'):
        ...
        site='(local.set $dynamic_results (i32.load offset=20 (local.get $context)))'
        replace(site,site+' (call $demand_event (i32.const 0) (local.get $dynamic_results))')
        site='(local.set $result_descriptor (local.get $top))'
        replace(site,site+' (call $demand_event (i32.const 1) (i32.const 48))')
...
def run(evidence,out):
    ...
    for name in [c['id'] for c in CASES]:
        ...
        totals.append(total)
    require(totals[0]['arena_allocations']==0 and totals[0]['callee_descriptors']>=2,'INLINE_NOT_ALLOCATION')
    ...
    summary=dict(status='PASS',comparisons=len(CASES)*4,counts=totals,
                 ...
                 scope='Executed operation counts only. Production mode propagation is unchanged; this is not a timing measurement or a result-demand implementation.')
