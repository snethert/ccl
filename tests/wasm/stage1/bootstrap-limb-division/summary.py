...
def summarize(numeric, environment, native):
    packet = ROOT.parent / 'ccl-evidence/2026-09-22-stage1-bootstrap-limb-multiply-r1'
    ...
    assert sum(r['definition'] == 'CORE-WORD-DIVISION' for r in rows) == 36
    current = names(rows)
    assert previous <= current
    assert {'BIGNUM-TRUNCATE', 'BIGNUM-TRUNCATE-NO-REM', 'BIGNUM-REM'} <= current
    ...
    result = dict(status='PASS', original_definitions_executed=len(current),
                  ...
                  scope='Original CCL bignum long division, quotient-only division and remainder execute over signed inputs. Target Lisp supplies the digit division and quotient correction LAP entries. GCD still lacks dependencies; generic numeric restarts, full generic dispatch, further metadata and image/READY work remain unfinished. No LL15 credit.')
