...
restored = restored.replace('(return-from bignum-truncate #+wasm32-target (if no-rem 0 (values 0 x1)) #-wasm32-target (values 0 x1))', '(return-from bignum-truncate (values 0 x1))')
assert restored == old
