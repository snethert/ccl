...
let storageChecks=0;const observations=[];let resultChecks=0,callableChecks=0,closureChecks=0,localChecks=0;
for(const observed of [false,true]){
 ...
 for(const start of [65536,2147483648]){
  ...
  for(const name of ['rv_small','rv_cleanup'])for(const bytes of [0,32,528,544,576]){
   ...
   assert.equal(get(76),700000,'temporary scope released on both paths');
   assert(new Uint8Array(memory.buffer,700000+bytes,64).every(x=>x===0xa7),'temporary allocation fence');storageChecks++;
  }
  set(80,700000);set(76,700000);set(84,900000);
  ...
  for(const fault of ['header','nonfixnum','negative','reserved','index-limit','zero-pointer','alignment','empty','extent','near-end','partial-capacity']){
   ...
   if(fault==='header')store(symbol-6,1849);if(fault==='nonfixnum')store(symbol+22,5);if(fault==='negative')store(symbol+22,0xfffffffc);if(fault==='reserved')store(symbol+22,0);if(fault==='index-limit')store(symbol+22,4*get(108));
   ...
   assert.equal(get(112),0,'partial binding unwound');assert.deepEqual(Array.from({length:5},(_,i)=>load(610000+4*i)),[243,243,243,243,243],'refused binding preserves TLB');assert.deepEqual(new Uint8Array(memory.buffer,600000,96),before,'refused binding preserves symbols');bindingChecks++;
  }installObjects();
  ...
  for(const fault of ['constant','global','flags-tag','cycle-after-values','record-capacity','short-values']){
   ...
   assert.equal(get(112),0,'PROGV refusal unbinds '+fault);assert.deepEqual(Array.from({length:5},(_,i)=>load(610000+4*i)),[243,243,243,243,243],'PROGV refusal restores vector '+fault);
...
parentPort.postMessage({status:'PASS',storage_checks:storageChecks,multiple_value_resources:multipleValueResources,lexical_exit_checks:lexicalExitChecks,progv_checks:progvChecks,binding_inspections:bindingInspections,binding_checks:bindingChecks,peak_bindings:peakBindings,control_inspections:controlInspections,state_checks:stateChecks,chain_checks:chainChecks,cleanup_entries:cleanupEntries,cleanup_resources:cleanupResources,public_dispatches:publicDispatches,internal_entries:internalEntries,tail_runs:tailRuns,non_tail_checks:nonTailChecks,tail_checks:tailChecks,modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:60,callable_checks:callableChecks,closure_checks:closureChecks,local_checks:localChecks,result_capacity_checks:resultChecks,allocation_boundary_checks:8});
}
