...
def compile_cases(evidence,out,backend=None):
 ...
 require(next(r['value'] for r in rows(d1) if r.get('name')=='subtag-simple-vector')==layout['closure_environment']['subtag'],'D1_ENVIRONMENT_LAYOUT')
 ...
 tcr=read(ROOT/'doc/WASM/contracts/tcr.v1.json')
 for name,offset in [('tcr_index',0),('handler_checkpoint',140),('unwind_state',148),('tlb_pointer',104),('tlb_limit',108),('db_link',112)]:
  require(next(r['offset'] for r in rows(tcr) if r.get('name')==name)==offset,'TCR_CONTROL_LAYOUT '+name)
 dynamic=layout['dynamic_bindings']
 for name,key in [('subtag-no-thread-local-binding','no_thread_local_binding'),('unbound-marker','unbound')]:
  require(next(r['value'] for r in rows(d1) if r.get('name')==name)==dynamic[key],'D1_BINDING_MARKER '+name)
 for name,key in [('vcell','global_value_displacement'),('binding-index','binding_index_displacement'),('flags','flags_displacement')]:
  require(next(r['tagged_displacement'] for r in symbol['cells'] if r['name']==name)==dynamic['symbol'][key],'D1_BINDING_OFFSET '+name)
 import re
 flags=(ROOT/'library/lispequ.lisp').read_text()
 bits={name:int(re.search(r'\(defconstant \$sym_bit_'+name+r'\s+(\d+)\)',flags,re.I)[1]) for name in ('const','global')}
 require(bits=={'const':dynamic['progv']['constant_bit'],'global':dynamic['progv']['global_bit']} and ((1<<bits['const'])|(1<<bits['global']))<<2==dynamic['progv']['flags_mask'],'U1_SYMBOL_FLAGS')
 ...
 from root_contracts import derive
 save(out/'root-contracts.json',derive(read(out/'root-ir.json')))
 entries=read(out/'entries.json');require(len({m['name'] for m in entries})==len(entries),'ENTRY_NAMES')
 source={m['name']:m for m in modules()}
 for m in entries:
  if m['name'] in source:m['source']=source[m['name']]['source']
 save(out/'modules.json',entries)
 # Check the generated entry choice against the independent pre-emitter witness.
 contracts=read(out/'root-contracts.json')
 for m in entries:
  wat=(out/(m['name']+'.wat')).read_text()
  body=wat.split('(func $body ',1)[1].split('(local.set $result_bytes',1)[0]
  chosen='(local.set $dynamic_results (i32.const 0))' in body
  require(chosen==contracts[m['name']]['small_scratch'],'SMALL_SCRATCH_IR_JOIN '+m['name'])
 (out/'installed').mkdir()
 for m in entries+[{'name':'observe_entry'},{'name':'observe_tail'}]:
  name=m['name']
  with (out/(name+'-assembler.log')).open('w') as log:
   subprocess.run(['/usr/local/bin/wat2wasm','--enable-threads','--enable-exceptions','--enable-tail-call',str(out/(name+'.wat')),'-o',str(out/(name+'.wasm'))],check=True,stdout=log,stderr=subprocess.STDOUT)
  shutil.copy(out/(name+'.wasm'),out/'installed'/(name+'.wasm'))
 save(out/'compile-summary.json',{'status':'PASS','modules':len(entries),'source_restored':True})
