...
def adapt(s):
 ...
 s=s.replace("assert.deepEqual({status,values:result,nodes},c.expected,c.id+': native/logical result');", "const specials=specialNames.slice(0,3).map(name=>load(symbols[name]+2)===51?'unbound':decode(load(symbols[name]+2)));assert.deepEqual({status,values:result,nodes,specials},c.expected,c.id+': native/logical result');assert.equal(get(112),0,c.id+': binding chain restored');assert.deepEqual(Array.from({length:5},(_,i)=>load(610000+4*i)),[243,243,243,243,243],c.id+': binding vector restored');")
 ...
 s=s.replace("if(fault==='index-limit')store(symbol+22,16)","if(fault==='index-limit')store(symbol+22,4*get(108))")
 s=s.replace('call_error,type_error,nonlocal_exit}}).exports.entry',"call_error,type_error,nonlocal_exit},symbols,keywords,codes:Object.fromEntries(mods.map((m,i)=>[m.name,4*(i+1)]))}).exports.entry")
 return storage(s)
