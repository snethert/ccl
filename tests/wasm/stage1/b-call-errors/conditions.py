...
def compile_conditions(evidence,out,backend=None):
 ...
 def lisp_input():
  ...
  return '(in-package "CL-USER")\n(defparameter *condition-suite* t)\n'+('(defparameter *implicit-error-suite* t)\n' if IMPLICIT_ERRORS else '')+''.join('(defparameter '+n+" '"+lit(v)+')\n' for n,v in [('*call-sources*',[[x['name'],x['source']] for x in modules()]),('*call-cases*',[[c['id'],c['function'],c['nodes'],c['args'],[[n,t] for n,t in c['bindings'].items()]] for c in cases()]),('*call-refusals*',[list(x) for x in REFUSALS])])
 try:
