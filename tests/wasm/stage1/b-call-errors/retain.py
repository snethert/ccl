#!/usr/bin/env python3
"""Retain the combined LL05 qualification proposal and its R6 evidence."""
import argparse,json,shutil,tarfile,subprocess
from pathlib import Path
from support import HERE,ROOT,REG,read,save,sha,require

def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-LL05-R1','scope':'S1-LL05-a/b generated B calls, adapters, installation, tails and arity/designator conditions. NOT_REVIEWED, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 for p in run.iterdir():
  if p.name in ('mutants','condition-controls','error-controls','full-loader'):continue
  if p.is_dir():shutil.copytree(p,out/p.name)
  else:shutil.copy(p,out/p.name)
 for name in ('mutants','condition-controls','error-controls','full-loader'):
  with tarfile.open(out/(name+'.tar.gz'),'w:gz') as t:
   for p in sorted((run/name).rglob('*')):
    if p.is_file():t.add(p,arcname=str(p.relative_to(run/name)))
 ...
 sources+=list((HERE.parent/'b-lazy-calls').glob('*.py'))
 sources+=list((HERE.parent/'b-lazy-calls').glob('*.mjs'))+[HERE.parent/'b-lazy-calls/prepare.py',HERE.parent/'b-lazy-calls/stub.wat']
 sources += [ROOT/'doc/WASM/tools/gate.py',ROOT/'doc/WASM/tools/evidence_binding.py']
 pins={}
 for p in sources:
 ...
 known={}
 for predecessor in ('2026-09-17-stage1-b-mv-storage-r2','2026-09-17-stage1-b-conditions-r1','2026-09-17-stage1-b-conditions-r2','2026-09-17-stage1-b-result-scratch-r1'):
