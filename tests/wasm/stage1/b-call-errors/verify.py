#!/usr/bin/env python3
"""Replay LL05 native qualification, generated calls, installation and error controls."""
...
def hydrate(packet,evidence,dest):
 ...
 for name in ('error-controls','full-loader'):
  with tarfile.open(packet/(name+'.tar.gz')) as t:t.extractall(dest/name,filter='data')
...
def verify(packet,evidence):
 ...
 try:
  ...
  require(call_errors(evidence,temp/'call-errors')==read(retained/'call-errors/execution.json'),'IMPLICIT_ERROR_REPLAY')
  for p in (retained/'call-errors/compiled').glob('*.wasm'):
   require(p.read_bytes()==(temp/'call-errors/compiled'/p.name).read_bytes(),'IMPLICIT_ERROR_BINARY '+p.name)
  require(condition_lazy(temp/'call-errors/compiled',temp/'call-errors/execution.json',temp/'call-error-lazy',call_errors=True)==read(retained/'call-error-lazy/summary.json'),'IMPLICIT_ERROR_LAZY')
  from error_controls import run as error_controls
  require(error_controls(evidence,temp/'error-controls')==read(retained/'error-controls/controls.json'),'IMPLICIT_ERROR_CONTROLS')
  ...
  require(storage_probes(evidence,temp/'storage-probes')==read(retained/'storage-probes/summary.json'),'STORAGE_PROBES')
  from storage_controls import run as storage_controls
  require(storage_controls(evidence,temp/'storage-controls')==read(retained/'storage-controls/controls.json'),'STORAGE_CONTROLS')
  ...
  require(full_loader(temp/'positive',temp/'lazy-composition',temp/'full-loader')==read(retained/'full-loader/summary.json'),'FULL_LOADER_REPLAY')
  require(read(temp/'full-loader/full-controls.json')==read(retained/'full-loader/full-controls.json'),'FULL_LOADER_CASES')
  require(read(temp/'full-loader/mutants.json')==read(retained/'full-loader/mutants.json'),'FULL_LOADER_MUTANTS')
  ...
  write(retained)
  for name in ('coverage.json','abi-binding.json','options.json'):
   require((retained/name).read_bytes()==(packet/name).read_bytes(),'QUALIFICATION_CLAIMS '+name)
 ...
 print('S1-LL05-VERIFIED: native R6/R6a, generated B calls and conditions, compiler/loader mutants, cold installation and native comparisons')
