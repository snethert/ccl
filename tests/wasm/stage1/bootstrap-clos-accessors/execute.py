...
prior=h.parent/'bootstrap-lexpr-spread'
def fixture(name):return h/name if (h/name).exists() else prior/name
...
for name in ('install.mjs','check.mjs','gf-check.mjs','installer-check.mjs','metadata-check.mjs'):shutil.copy(fixture(name),o/name)
