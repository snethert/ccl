...
HERE=Path(__file__).resolve().parent;CLOS=HERE.parent/'bootstrap-clos-accessors';PRIOR=HERE.parent/'bootstrap-lexpr-spread'
...
parent.fixture=lambda name: HERE/name if (HERE/name).is_file() else CLOS/name if (CLOS/name).is_file() else PRIOR/name if (PRIOR/name).is_file() else fallback(name)
copy=shutil.copy
