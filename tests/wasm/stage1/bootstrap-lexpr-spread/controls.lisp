...
(defun core-controls (out)
  (let ((rows nil))
    ...
    (dolist (row '((:lfun-vector (defun refused (f) (ccl::lfun-vector f)) :function-immediate-layout)
                   (:function-vector (defun refused (f) (ccl::function-to-function-vector f)) :function-immediate-layout)
                   (:kernel-global (defun refused () (ccl::%get-kernel-global 'all-areas)) :kernel-global-capability)
                   (:kernel-global-pointer (defun refused (p) (ccl::%get-kernel-global-ptr 'all-areas p)) :kernel-global-capability)
                   (:native-ffi (defun refused () (ccl::ff-call 0 :void)) :native-ffi-excluded)
                   (:function-immediate-read (defun refused (f) (ccl::nth-immediate f 1)) :admitted)
                   (:function-immediate-write (defun refused (f v) (ccl::set-nth-immediate f 1 v)) :admitted)
                   (:escaping-lexpr (defun refused (ccl::&lexpr args) (lambda () (ccl::%lexpr-count args))) :b-local-owner)
                   (:unknown-handler-bind
                    (defun refused () (handler-bind ((reader-error #'identity)) 7)) :b-condition-type)
                   (:unknown-handler-case
                    (defun refused () (handler-case 7 (reader-error () 8))) :b-condition-type)
                   (:macro-introduced-handler
                    (defun refused () (macrolet ((check () '(handler-bind ((reader-error #'identity)) 7)))
                                        (check))) :b-condition-type)
                   (:compound-handler-type
                    (defun refused () (handler-bind (((or type-error reader-error) #'identity)) 7)) :b-condition-type)
    ...
    (dolist (form '((defun admitted ()
                     (macrolet ((handler-bind (&rest ignored) (declare (ignore ignored)) 7))
                       (handler-bind ((reader-error #'identity)) 9)))
                    (defun admitted ()
                      (macrolet ((handler-case (&rest ignored) (declare (ignore ignored)) 7))
                        (handler-case 9 (reader-error () 8))))
                    (defun admitted () '(handler-bind ((reader-error #'identity)) 9))))
            ...
            (call-with-target
             (lambda ()
               (compile-bootstrap-form
                '(defun admitted () (handler-bind ((reader-error #'identity)) 7))
