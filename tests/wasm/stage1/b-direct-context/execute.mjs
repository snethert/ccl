...
let publicDispatches=0,internalEntries=0;let checks=0,peakRoots=0,activeFrame=0,activeRoots=0,topContext=0;
...
for(const observed of [false,true]){
 ...
 for(const m of mods){
  ...
  tail_table.set(functions.size,observed?new WebAssembly.Instance(tailObserver,{env:{entry:tail,observe:(self,n,context)=>{internalEntries++;inspect('internal '+m.name);assert.equal(load(u32(self)-2),4*(mods.findIndex(x=>x.name===m.name)+1),'tail self');assert.equal(get(64),u32(context)+48,'tail argument area');assert.equal(get(128),u32(context)+32,'retired root frames');assert.equal(load(u32(context)+40),u32(self),'continuation SELF root');assert.equal(load(u32(context)+12),load(u32(context)+32),'saved and linked parent root');assert.equal(load(u32(context)+36),2+4*Math.ceil(n/4),'tail root count');if(stressState){stressState.transfers++;assert.equal(u32(context),stressState.context,'one continuation per tail chain');let depth=0;for(let h=get(128);h;h=load(h))depth++;assert.equal(depth,2,'constant tail root depth');stressState.maxRootDepth=Math.max(stressState.maxRootDepth,depth);stressState.maxArguments=Math.max(stressState.maxArguments,n);}if(u32(context)===topContext){activeFrame=u32(context)+48+16*Math.ceil(n/4)+load(u32(context)+24);activeRoots=Math.max(4,(get(124)-get(120))/4)+rootContracts[m.name].bound_words;}}}}).exports.entry:tail);
 }
...
parentPort.postMessage({status:'PASS',public_dispatches:publicDispatches,internal_entries:internalEntries,tail_runs:tailRuns,non_tail_checks:nonTailChecks,tail_checks:tailChecks,modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:60,callable_checks:callableChecks,closure_checks:closureChecks,local_checks:localChecks,result_capacity_checks:resultChecks,allocation_boundary_checks:8});
}
