...
def verify(packet,evidence):
 require((HERE/'wasm32-backend.lisp').read_text().split(';;; First B call unit:')[0]==(HERE.parent/'b-tail-calls/wasm32-backend.lisp').read_text().split(';;; First B call unit:')[0],'REVIEWED_NON_B_ENTRIES_UNCHANGED')
 ...
 temp=Path(tempfile.mkdtemp(prefix='ccl-direct-verify-')).resolve()
 try:
 ...
 print('S1-B-DIRECT-CONTEXT-VERIFIED: native R6/R6a, direct continuation arguments, internal ordinary calls, mutants and refusals')
