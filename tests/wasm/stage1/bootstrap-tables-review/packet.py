...
from derive import PARENT
def read(p):return json.loads(p.read_text())
ID='STAGE1-BOOTSTRAP-TABLES-REVIEW-R1'
SOURCES=['README.md','derive.py','population.mjs','population-check.mjs','admission.mjs','statistics-check.mjs','native.lisp','run.py','packet.py']
def pins():
 prior=read(ROOT.parent/'ccl-evidence/2026-09-20-stage1-bootstrap-tables-r1/source-pins.json')
 for n,h in prior.items():assert sha(ROOT/n)==h,n
 paths=[HERE/n for n in SOURCES]+[ROOT/n for n in ['level-0/nfasload.lisp','level-1/l1-lisp-threads.lisp','level-1/l1-dcode.lisp','lib/misc.lisp']]
...
def kept(p):
 for f in sorted(p.rglob('*')):
  ...
  if 'faults' in n.parts and ('compiled' in n.parts or f.name in ['check.mjs','collector.wasm','hash.wasm','selection.json','admission.mjs','population-check.mjs','statistics-check.mjs']):continue
...
def inputs(e):
 return {n:sha(e/n) for n in ['2026-09-20-stage1-bootstrap-tables-r1/packet.json','2026-09-20-stage1-startup-runtime-review-r1/packet.json','2026-09-12-native-census-r7/baseline/build/dx86cl64','2026-09-16-stage1-1a-r2/native/baseline.image']}
def main():
