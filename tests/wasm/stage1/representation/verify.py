#!/usr/bin/env python3
"""Replay native qualification and recompile every representation compiler mutant."""
...
spec=importlib.util.spec_from_file_location("representation_run",HERE/"run.py");runner=importlib.util.module_from_spec(spec);spec.loader.exec_module(runner);execute=runner.execute
...
def verify(packet,evidence):
 ...
 with tempfile.TemporaryDirectory(prefix='ccl-representation-verify-') as tmp:
  ...
  for name,backend in [('positive',None)]+list(proposed.items()):
   ...
   for p in original.iterdir():
    if p.suffix in ('.wat','.wasm') or p.name in ('native.json','cases.json','compiler.dx64fsl'):
 ...
 print('S1-LL04-VERIFIED: native R6/R6a, 19 modules, 740 target checks and 15 recompiled compiler mutants')
