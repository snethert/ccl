...
if(isMainThread){
  ...
  let integer,floating,integerCalls=0,floatCalls=0,fastChecks=0,retryCollections=0,activeCase='';
  ...
  function services(){
   ...
   const boundary=owner.atSafepoint.bind(owner);
  ...
  parentPort.postMessage({signedZero,libmRows,shiftRows,refusals,layout:{...layout,values:layoutValues},retryCollections,integerCalls,floatCalls,fastChecks,base,comparisons:rows.length,collections,internalCollections,rows,...gen.summary()});
}
