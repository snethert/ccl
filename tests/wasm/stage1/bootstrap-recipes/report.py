...
def report(out):
    ...
    destructive=[r for r in native if r['definition'].startswith('%DOUBLE-FLOAT-') and r['definition'].endswith('!')]
    assert len({r['definition'] for r in destructive})==13
    assert all(r['args'][-1]!=r['after'][-1] for r in destructive), 'destination snapshots must precede the native store'
    save('destructive-witnesses.json',dict(definitions=13,cases=len(destructive),
         source='Pinned native image functions, compiled with their original file-local macro environments.',
         scope='Exact identities only. Inputs are copied with native %COPY-FLOAT; destination before/after payloads must differ. Approximate libm rows remain separately counted.'))
    ...
    baseline=Path(__file__).resolve().parents[5]/'ccl-evidence/2026-09-21-stage1-bootstrap-closure-r1/execution/execution-frontier.json'
    ...
    save('progress.json',dict(baseline_executed=436,executed=len(executed),new_executions=sorted(executed-set(before['names'])),
