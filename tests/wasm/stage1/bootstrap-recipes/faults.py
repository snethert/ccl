...
def check(out):
 ...
 removed,n=re.subn(r'\(i64\.store offset=8 \(local\.get [^)]+\) \(i64\.load offset=8 \(local\.get [^)]+\)\)\)', '(nop)', sine)
 assert n==1,n
 controls.extend([
  ('missing-destination-store',sin,'%DOUBLE-FLOAT-SIN!',removed,'AssertionError'),
  ('immediate-proof-omission','core_word_and_mask','CORE-WORD-AND-MASK',(out/'compiled/omission-word-mask.wat').read_text(),'checked 32'),
  ('minus1-identity','core_minus1','CORE-MINUS1',(out/'compiled/omission-minus1.wat').read_text(),'AssertionError')])
 invalid=next(r['name'] for r in modules if r['function']==syms['INVALID-HASH-KEY-P'])
 controls.append(('missing-file-symbol-macros',invalid,'INVALID-HASH-KEY-P',
                  (out/'compiled/standalone-invalid-hash-key.wat').read_text(),'INVALID-HASH-KEY-P'))
 ...
 (out/'math-controls.json').write_text(json.dumps(dict(status='PASS',controls=reports,scope='Inherited seven controls plus the standalone symbol-macro environment regression; not an exhaustive predicate sweep.'),indent=2,sort_keys=True)+'\n')
