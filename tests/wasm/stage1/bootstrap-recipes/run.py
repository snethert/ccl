...
def compile_corpus(out):
    ...
    chunks=[]; sources=backend.source_files(root)
    for directory in ['level-0','level-1']:
     for path in sorted((root/directory).glob('*.lisp')):chunks.append((str(path.relative_to(root)),sources.get(str(path.relative_to(root)),path.read_text())))
    ...
    for name in ('closure-controls.lisp','observers.lisp'):shutil.copy(HERE/name,driver/name)
...
def summarize(out):
    ...
    from recipes import report as recipe_report
    cohort=recipe_report(out)
    ...
    from domains import check as domains_check
    domains=domains_check(out)
    ...
    assert all(r['status']=='ADMITTED' for r in whole if not r['file'].endswith(('l0-misc.lisp','l0-hash.lisp')))
    ...
    save(out/'summary.json',dict(status='PASS',original_definitions_executed=len(executed),
        ...
        native_comparisons=sum(r['comparisons'] for r in execution)-4-4*sum(r.get('targetOnly') or r['definition'].startswith('CORE-HOST-') for r in native)-domains['shift_comparisons']-domains['transcendental_comparisons'],
        ...
        allocation_retry_collections=sum(r['retryCollections'] for r in execution),
