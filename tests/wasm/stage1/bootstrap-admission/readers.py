...
def run(source,proposal,out):
    ...
    for name in list(FILES):
        ...
        from sources import plan
        edits,exclusions=plan(ROOT,name)
        inverse=proposed
        before=[];after=[]
        delta=0
        inverse_edits=[]
        for a,b,replacement in edits:
            inverse_edits.append((a+delta,a+delta+len(replacement),original[a:b]))
            delta+=len(replacement)-(b-a)
            if a==b:
                definition=next(row for row in exclusions if row['start']==a)
                witness='(existing-definition '+json.dumps(definition['name'])+' '+json.dumps(hashlib.sha256(original[a:definition['end']].encode()).hexdigest())+')'
                before.append(witness);after.append(replacement+witness)
                substitutions.append(dict(file=name,definition=definition['name'],kind='excluded-on-wasm-only',sha256=hashlib.sha256(original[a:definition['end']].encode()).hexdigest()))
            else:
                before.append(original[a:b]);after.append(replacement)
                substitutions.append(dict(file=name,token=original[a:b],occurrences=1))
        for a,b,replacement in reversed(inverse_edits):inverse=inverse[:a]+replacement+inverse[b:]
