...
def report(out):
    ...
    baseline=Path(__file__).resolve().parents[5]/'ccl-evidence/2026-09-21-stage1-bootstrap-host-r1/execution/execution-frontier.json'
    ...
    save('progress.json',dict(baseline_executed=349,executed=len(executed),new_executions=sorted(executed-set(before['names'])),
        ...
        reader_files_complete_before=42,reader_files_complete_after=sum(x=='T' for x in __import__('re').findall(r'\("[^"]+" \d+ (T|NIL)\)',(out/'compiled/worklist.sexp').read_text())),
