...
ID = 'STAGE1-BOOTSTRAP-ADMISSION-R1'
...
def pins():
    ...
    for name in ('tests/wasm/stage1/bootstrap-witnesses-acceptance/run.py',
                 ...
                 'compiler/X86/X8632/x8632-arch.lisp', 'lib/foreign-types.lisp'):
...
def dependencies():
    ...
    paths += [r.EVIDENCE / '2026-09-21-stage1-bootstrap-host-r1' / n for n in ('packet.json', 'execution/summary.json', 'execution/compiled/worklist-throughput.json', 'execution/compiled/closed.json', 'execution/compiled/native.json', 'execution/execution-frontier.json')]
    for path in paths:
...
def native_check(path):
    ...
    import tarfile
    from native_compare import compare, controls
    with tarfile.open(r.EVIDENCE / '2026-09-16-stage1-1a-r2/native/baseline-fasls.tar.gz') as baseline, tarfile.open(r.EVIDENCE / 'macos-u1-inputs/source.tar') as source:
        for name, fasl in native.source_pairs():
            before=baseline.extractfile(fasl).read()
            after=(path / ('registered-'+Path(fasl).name)).read_bytes()
            a=source.extractfile(name).read().decode()
            b=(path / 'proposal/files' / name).read_text()
            assert compare(before,after,a,b)==read(path / (Path(name).stem+'-comparison.json'))
            if name=='level-0/l0-cfm-support.lisp':
                assert controls(before,after,a,b)==['executable-byte','non-location-bit','callee-identity']
    ...
    assert (path / 'proposal/files' / arch).read_text() == backend.arch_source()
