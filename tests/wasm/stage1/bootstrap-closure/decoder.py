...
class CompilerFasl(Prior):
    def expr(self, depth=0):
        if self.pos < len(self.data) and self.data[self.pos] & 127 == 9:
            require(depth < 64, 'FASL_DEPTH')
            start, raw = self.pos, self.byte()
            self.ops.append({'offset': start, 'opcode': 9, 'epush': bool(raw & 128)})
            value = {'bignum32': self.take(4 * self.count()).hex()}
            if raw & 128:
