...
ID='STAGE1-BOOTSTRAP-CLOSURE-R1'
def pins():
    x=base.pins()
...
def dependencies():
    ...
    for directory,names in [
        ('2026-09-21-stage1-bootstrap-math-r2',['packet.json','native/run.json','execution/execution-frontier.json']),
...
def source_check(out):
    for name in ('compile.lisp','cases.lisp','execute.lisp','probes.lisp','controls.lisp','whole-file.lisp','witnesses.lisp','inputs.lisp','foreign-scan.lisp','w32-os.lisp','os-probes.lisp','closure-controls.lisp'):
...
def main():
    ...
    if args.mode=='retain':
        ...
        copy_native(n,p/'native');shutil.copytree(r.HERE,p/'source',ignore=shutil.ignore_patterns('__pycache__'))
        ...
        native_check(p/'native');out=args.output.resolve();r.run(out);source_check(out)
