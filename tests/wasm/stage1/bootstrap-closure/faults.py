...
def check(out):
 ...
 (out/'math-controls.json').write_text(json.dumps(dict(status='PASS',controls=reports,scope='Five emitted-code faults and two compiled compiler-omission controls; not an exhaustive predicate sweep.'),indent=2,sort_keys=True)+'\n')
