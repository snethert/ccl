...
def run(evidence,native,qualification,out):
 ...
 summary={'status':'PASS','modules':target['modules'],'native_cases':len(read(out/'positive/cases.json')),'comparisons':target['comparisons'],'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'resource_refusals':target['resource_refusals'],'allocation_boundary_checks':target['allocation_boundary_checks'],'result_capacity_checks':target['result_capacity_checks'],'ownership_inspections':target['ownership_inspections'],'peak_root_frames':target['peak_root_frames'],'scope':'Generated runtime-sized result scratch, call reservations and multiple-value retention; no fixed 64-value ceiling. Caller reservation supplies the result budget; stack exhaustion is checked. No automatic stack growth, collection or full LL05 slot credit.'};save(out/'summary.json',summary);print('S1-B-RESULTS-PASS',summary)
