...
def cases():
 out=[]
 def add(name,args,nodes=None,capacity=64):
  ...
  out.append(dict(id=f'{name}-{len(out):03}',function=name,args=args,capacity=capacity,nodes=before,expected=expected,allocated_cells=len(after)-len(before)))
 for n in (0,1,2,3,4,5,6,16,32,64):
