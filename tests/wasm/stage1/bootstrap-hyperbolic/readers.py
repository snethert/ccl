"""Existing-target reader proof for the three grouped native exclusions and six target definitions."""
...
def replay(evidence,proposal,out):
 ...
 paths=['level-1/l1-numbers.lisp'];proof=[]
 for name in paths:
  old=(ROOT/name).read_text();new=(proposal/name).read_text()
  start=new.index('\n#+wasm32-target\n(defun %double-float-asinh!')
  prefix=new[:start]
  assert prefix.replace('#-(or windows-target wasm32-target)', '#-windows-target')==old
  assert prefix.count('#-(or windows-target wasm32-target)')==3
  a='\n'.join('#-windows-target (retained-native-definition '+str(i)+')' for i in range(3))
  b='\n'.join('#-(or windows-target wasm32-target) (retained-native-definition '+str(i)+')' for i in range(3))+new[start:]
  for directory,text in ((before,a),(after,b)):
   p=directory/name;p.parent.mkdir(exist_ok=True);p.write_text('(in-package :ccl)\n'+text+'\n')
  proof.append(dict(file=name,original_sha256=hashlib.sha256(old.encode()).hexdigest(),proposal_sha256=hashlib.sha256(new.encode()).hexdigest(),sites=3,inverse_bytes_equal=True))
