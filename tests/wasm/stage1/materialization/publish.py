"""Bind LL21-a only after retained replay; exercise every artifact-role omission."""
...
def role_controls(e,inventory,report,inventory_hash):
 ...
 for role in sorted(set(inventory['required_record_roles']+t['required_record_roles'])):
  ...
  status,reasons=assess(inventory,bad,inventory_hash,e);diagnostic='missing artifact roles: S1-LL21-a [materialization]'
...
def publish(e,p):
 ...
 assert t['status']=='EXECUTED' and t['runner']=='tests/wasm/stage1/materialization/run.py'
