"""A numeric-only delta: no repeated worklist recount or native build at replay."""
...
PARENT = EVIDENCE / '2026-09-22-stage1-bootstrap-gcd-r1'
...
def main():
    ...
    if args.mode == 'retain':
        ...
        shutil.copyfile(out / 'method-summary.json', packet / 'summary.json')
        ...
        save(packet / 'packet.json', dict(id='STAGE1-METHOD-DISPATCH-R1', slot_credit=False,
        ...
        save(out / 'verification.json', dict(status='PASS', deterministic_files=len(actual),
             ...
             summary=read(out / 'method-summary.json')))
