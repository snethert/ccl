...
def run(kind, out):
    ...
    if kind == 'readers':
        ...
        driver.proposal = SimpleNamespace(sources=lambda: {
            ...
            if name in ('level-0/l0-def.lisp', 'level-0/l0-float.lisp')})
    ...
    def proposed(base):
        result = prepare(base)
        product.module('definition_metadata', HERE / 'metadata.py').overlay(base)
    ...
    # compiler.install() sets execute.prepare inside driver.run. Install the
    # collector overlay at the execution boundary, after that setup.
    original_execute = execute.execute
    def run_execution(*args, **kwargs):
        execute.prepare = proposed
        return original_execute(*args, **kwargs)
    execute.execute = run_execution
    try:
        result = driver.run(out)
    finally:
        execute.execute = original_execute
    assert c.read(out / 'base/execution-environment.json')['files']['collector.wasm'] == c.sha(out / 'runtime/collector.wasm')
