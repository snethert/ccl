// P2-0 diagnostic executor, not the CCL startup entry point. Boot0 must call
// the original %TOPLEVEL-FUNCTION%. This fixture runs the cold-load functions (the file's top-level effects) and call the
// named definitions; print the observations as JSON for the native oracle.
...
const layout={version:1,collector:'copying',workers:1,egc:false,tcr:TCR,maximumPages:32769,logCapacity:32768,regions:layoutRegions,
...
const env={memory,tcr:TCR,table:new WebAssembly.Table({element:'anyfunc',initial:64}),tail_table:new WebAssembly.Table({element:'anyfunc',initial:64}),code_registry:REGISTRY,
...
put(REGISTRY,64);put(REGISTRY+4,1);
...
function callObject(fn,args){
 ...
 for(let i=0;i<64;i++)if(![48,52,56,116].includes(i*4))assert.equal(t(i*4),before[i],'TCR '+i*4);
...
// The top-level effects, in file order.
const coldLoad=[];for(let list=roots['cold-load-functions'];list!==N;list=get(list-1)){const fn=get(list+3);coldLoad.push(callObject(fn,[]));}
const cases=JSON.parse(fs.readFileSync(casesPath)),observations=[];
for(const c of cases){
 if(c.call){const fn=get(symbolAddress(c.call[0],c.call[1])-6+12);observations.push({call:c.call,args:c.args,values:callObject(fn,c.args.map(a=>a*4))});}
 else if(c.value)observations.push({value:c.value,result:decode(get(symbolAddress(c.value[0],c.value[1])-6+8))});
 if(process.argv.includes('--collect'))collect();
}
console.log(JSON.stringify({status:'PASS',coldLoad:coldLoad.length,observations,collections,objects:admitted.objects,modules:codeSet.modules.length}));
