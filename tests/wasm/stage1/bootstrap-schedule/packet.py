#!/usr/bin/env python3
"""Retain and replay the bounded schedule; no result envelope or gate mutation."""
import argparse,shutil,sys,tarfile
from pathlib import Path
from run import HERE,ROOT,read,save,sha,command
ID='STAGE1-BOOTSTRAP-SCHEDULE-R1'
SOURCES=('schedule.mjs','compile.lisp','run.py','check.mjs','controls.py','assess.py','inputs.json','packet.py','README.md','scope.json','development.json')
...
def retained(out):
 for f in files(out):
  ...
  if n.parts[0]=='faults' and f.suffix in ['.mjs','.wasm'] and f.name!='schedule.mjs':continue
  yield f
...
def retain(e,x,p):
 ...
 for n in ['README.md','scope.json','development.json','schedule.mjs']:shutil.copy(HERE/n,p/n)
