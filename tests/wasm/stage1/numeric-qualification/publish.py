"""Publish LL16 only after a full retained replay, with production role refusals."""
...
def role_controls(e,inventory,report,inventory_hash):
 ...
 for role in sorted(set(inventory['required_record_roles']+t['required_record_roles'])):
  ...
  status,reasons=assess(inventory,damaged,inventory_hash,e);diagnostic='missing artifact roles: S1-LL16-a [generated]'
...
def publish(e,p):
 ...
 t=next(t for t in inventory['tests'] if t['id']==TEST);need(t['status']=='EXECUTED' and t['runner']=='tests/wasm/stage1/numeric-qualification/run.py','registered runner')
 ...
 for n,role in [('source-pins.json','implementation'),('source-additions.tar.gz','source'),('source-base.json','source'),('wasm32-backend.lisp','implementation'),('scope.json','schema'),('coverage.json','schema'),('abi-decision.json','abi'),('abi-binding.json','abi'),('toolchain.json','host-compiler'),('options.json','options'),('summary.json','log'),('verification.json','log'),('controls.json','test'),('assessment.json','log'),('dispatch.json','test'),('fallback-execution.json','test'),('execution.tar.gz','test'),('native-reuse.json','log')]:add(p/n,role)
