...
def measurements(out):
    ...
    expected={'ABS':'ccl:level-0;l0-numbers.lisp','REALPART':'ccl:level-0;l0-numbers.lisp','IMAGPART':'ccl:level-0;l0-numbers.lisp','COMPLEX':'ccl:level-0;l0-numbers.lisp','MAKE-CPL-BITS':'ccl:level-1;l1-clos-boot.lisp',
...
def run(out):
    ...
    times['stream_shapes']=c.command([c.NODE,HERE/'stream-shapes.mjs',out/'compiled',out/'stream-shapes.json'],out/'stream-shapes.log',timeout=60)
    local('stream_controls').check(out)
...
def summarize(out):
    ...
    assert c.read(out/'stream-controls.json')['status']=='PASS'
