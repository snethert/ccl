// The lock retains native six-field node layout; every field is traced.
// An owned lock may move during a call, but this packet does not admit locks
// into persistent images or claim cross-Worker exclusion.
...
for(const base of [8388608,2146500608]){
 setup(base,80);put(base,1602);put(base+28,0);
 for(let i=0;i<6;i++){put(base+4+i*4,base+32+i*8+1);put(base+32+i*8,NIL);put(base+36+i*8,i*4);}
 assert.equal(collector.collect(config),0);assert.equal(get(config+84),7,'all six fields traced');
 const moved=get(root+8)-6;
 assert.equal(get(moved),1602);assert.equal(get(moved+28),0);
 for(let i=0;i<6;i++){
  const pair=get(moved+4+i*4);assert(pair>=to&&pair<to+80);
  assert.equal(get(pair-1),NIL);assert.equal(get(pair+3),i*4);
 }
 rows.push({base,status:'SIX_FIELDS_MOVED'});
 for(const n of [5,7]){
  setup(base,32);put(base,n*256+66);
  for(let i=1;i<8;i++)put(base+i*4,NIL);
  const before=[bytes(base,32).slice(),bytes(root,32).slice(),bytes(tcr,256).slice()];
  assert.equal(collector.collect(config),2,'lock count');
 ...
 setup(base,24);put(base,1602);
 assert.equal(collector.collect(config),2,'lock extent');rows.push({base,status:'EXTENT_REFUSED'});
}
fs.writeFileSync(output,JSON.stringify({status:'PASS',rows},null,2)+'\n');
console.log('PASS',rows.length,'lock layout checks');
