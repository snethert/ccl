...
def measure(base,probe,output):
    ...
    for name in new_names:
        package='COMMON-LISP' if name in numeric_names+['WRITE-STRING','WRITE-CHAR','GET-OUTPUT-STREAM-STRING'] else 'CCL'
    ...
    result=dict(
      ...
      original_executions=dict(count=parent['original_definition_headline']+len(new_names),
        ...
        nil_only=nil_only,stream_native_witness_sha256=c.digest(streams),lock_native_witness_sha256=c.digest(locks),
        ...
        scope='550 inherited originals plus four numeric, eight lock and six stream definitions executed by READY support callers. Target lock primitive replacements are excluded from original credit. The earlier originals are not all exercised by this cold image.'),
    ...
    assert result['original_executions']['count']==568
    assert result['original_executions']['non_nil']==531
