...
def measure(base,probe,output):
    ...
    stream_sources={
        'WRITE-STRING':'ccl:level-1;l1-io.lisp',
        'WRITE-CHAR':'ccl:level-1;l1-io.lisp',
        'WRITE-SIMPLE-STRING':'ccl:level-1;l1-io.lisp',
        'STRING-OUTPUT-STREAM-IOBLOCK-WRITE-CHAR':'ccl:level-1;l1-streams.lisp',
        'STRING-OUTPUT-STREAM-IOBLOCK-WRITE-SIMPLE-STRING':'ccl:level-1;l1-streams.lisp',
        'GET-OUTPUT-STREAM-STRING':'ccl:level-1;l1-streams.lisp'}
    constructor_sources={name:'ccl:level-1;l1-streams.lisp' for name in (
        'MAKE-STRING-OUTPUT-STREAM','MAKE-SIMPLE-STRING-OUTPUT-STREAM',
        '%%MAKE-STRING-OUTPUT-STREAM','CREATE-STRING-OUTPUT-STREAM-IOBLOCK',
        '%CLOSE-STRING-OUTPUT-STREAM','%IOBLOCK-CLOSE','%%IOBLOCK-CLOSE')}
    stream_sources.update(constructor_sources)
    # The generated DEFSTRUCT constructor is executed, but is not a DEFUN
    # in the original-definition cohort. Keep it outside that headline.
    new_names=numeric_names+list(lock_sources)+list(stream_sources)
    nil_only=['%UNLOCK-RECURSIVE-LOCK-OBJECT','RELEASE-LOCK']
    return_unobserved=['%CLOSE-STRING-OUTPUT-STREAM','%IOBLOCK-CLOSE','%%IOBLOCK-CLOSE']
    ...
    for name in new_names:
        package='COMMON-LISP' if name in numeric_names+['WRITE-STRING','WRITE-CHAR','GET-OUTPUT-STREAM-STRING','MAKE-STRING-OUTPUT-STREAM'] else 'CCL'
        matches=[m for m in closure['modules'] if m['name']==package+'::'+name]
        assert len(matches)==1 and matches[0]['source']==stream_sources.get(name,lock_sources.get(name,'ccl:level-0;l0-numbers.lisp')), ('original source attribution', name, matches)
    ...
    streams=next(row for row in rows if row['definition']=='READY-STRING-OUTPUT')
    assert streams['values'] and streams['values']!=[None]

    result=dict(
      status='MEASURED',
      original_executions=dict(count=parent['original_definition_headline']+len(new_names),
        non_nil=parent['original_non_nil_headline']+len(new_names)-len(nil_only)-len(return_unobserved),fresh_credit=len(new_names),
        nil_only=nil_only,return_unobserved=return_unobserved,stream_native_witness_sha256=c.digest(streams),lock_native_witness_sha256=c.digest(locks),
        ...
        scope='550 inherited originals plus four numeric, eight lock and thirteen stream definitions executed by READY support callers. Target lock primitive replacements are excluded from original credit. The earlier originals are not all exercised by this cold image.'),
    ...
    assert result['original_executions']['non_nil']==535
    assert len(gfs)==startup['values'][1]==54
