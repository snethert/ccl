"""Execute native array access through the complete cross-loaded array file."""
...
def native(out, cases):
    ...
    with (support / 'packages.lisp').open('a') as stream:
        stream.write((HERE.parent / 'loader-locks/locks.lisp').read_text() + (HERE / 'arrays.lisp').read_text())
...
def run(out):
    ...
    assert c.read(out / 'proposal-inputs.json') == inputs, 'producer drivers changed before target execution'
    ...
    code = code.replace('  if(tag===191)', '''  if(tag===7){const n=get(v-6)>>>8;let u=0n;
...
console.log(JSON.stringify({refusals,status:''')
    shutil.copyfile(HERE.parent / 'loader-locks/controls.mjs', out / 'lock-controls.mjs')
    ...
    for name in ('matrix', 'cube', 'chain', 'vector-chain', 'ranks', 'bits', 'string', 'integers', 'evaluation', 'collection', 'hold', 'release'):
        cases.append(dict(id='array-' + name, call=['CCL', 'LOADER-ARRAY-' + name.upper()], args=[]))
    ...
    for mode in ('plain', 'collect', 'relocate', 'relocate-collect'):
        ...
        modes[mode] = result
    result = dict(status='PASS', source_identity=pins, runtime=c.read(out / 'runtime/array-runtime.json'),
                  artifacts=written, native=oracle, runs=modes,
                  ordered=c.read(out / 'ordered.json'), accepted_files=[0, 0, 0], slot_credit=False)
    c.save(out / 'summary.json', result)
    assert identity.drivers() == inputs, 'target drivers changed during execution'
