...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-SPECIAL-BINDINGS-R1','scope':'Generated dynamic special bindings and restoration through cleanup/nonlocal exits; auxiliary execution, no LL05/LL19 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
