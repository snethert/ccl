...
if(isMainThread){
  ...
  console.log('PASS:',rows.reduce((n,r)=>n+r.comparisons,0),'target comparisons (signed-zero differences recorded separately)');
}else{
  const {base,dir}=workerData,NIL=77825,T=77838,tcr=1024,root=131064;
  const config=5000000,size=65536,other=3145728;
  const memory=new WebAssembly.Memory({initial:Math.max(128,Math.ceil((base+size)/65536)),maximum:32769,shared:true});
  ...
  function services(){
   const regions=[['tcr',1024,1280],['vstack',131064,196608],['temp',700000,780000],['control',900000,1000000],['c-stack',1048576,1114112],['scratch',5000000,6600000],['root-list',4600000,4862144],['external',1170000,1174096],['bindings',680000,696384]].map(([role,start,end])=>({name:role,role,start,end}));
   ...
   const owner=CollectorOwner.create(memory,cb,sha256(cb),{version:1,collector:'copying',workers:1,egc:false,maximumPages:32769,tcr,logCapacity:65536,regions,spaces:[{name:'a',start:base,end:base+size},{name:'b',start:other,end:other+size}],groups:['module-constants','callbacks','registry','host'].map(kind=>({kind,slots:[]}))});
  ...
  let currentGraph;
  const graphIO={get,put,bytes,tcr,encode,decode,conditionWord:name=>get(conditionClasses.get(name)),bindCondition:(name,word)=>{const slot=conditionClasses.get(name);assert(slot,name);put(slot,word);addRoot(slot);},bindValue:(name,word)=>put(gen.ownerWords.get(ownerNames.find(x=>x.package+'::'+x.name===name).id)+2,word),bindFunction:(id,word)=>put(gen.ownerWords.get(id)+6,word),functionWord:id=>get(gen.ownerWords.get(id.startsWith('s') ? id : ownerNames.find(x=>x.package+'::'+x.name===id).id)+6)};
  function encode(x){
    if(x?.graph){currentGraph=x.graph;return encodeGraph(x.graph,graphIO);}
    if(x&&x.classFixture){
  ...
  let collections=0;const originalRootCount=gen.extraRoots.length;const initialSymbolFields=[...gen.ownerWords.values()].flatMap(p=>[6,10,14,18].map(d=>[p+d,get(p+d)]));
  for(const expected of native.filter(x=>process.env.CCL_DISPATCH_METADATA ? x===native[0] : !process.env.CCL_LIBRARY_CASE||(process.env.CCL_LIBRARY_CASE.endsWith('*') ? x.definition.startsWith(process.env.CCL_LIBRARY_CASE.slice(0,-1)) : x.definition===process.env.CCL_LIBRARY_CASE))){ fs.writeSync(2,expected.definition+' '+(expected.args[0]?.graph ? expected.args[0].graph.nodes.length+' graph nodes' : JSON.stringify(expected.args))+'\n');
    ...
    put(config+68,65536);put(config+72,4600000);put(config+80,6600000);
    initialConditionClasses.forEach(([p,v])=>put(p,v));initialSymbolFields.forEach(([p,v])=>put(p,v));gen.extraRoots.length=originalRootCount;gen.reset(movingPools[base]);nodeNext=4194304;nodeDescriptions.clear();rawDescriptions.clear();flaggedSymbols.clear();services();
    ...
    for(const move of [false,true]){
      ...
      if(expected.definition.startsWith('CORE-LIBM-')){
        ...
        assert.deepEqual({values,after},{values:expectedValues,after:expected.after},expected.definition+' '+JSON.stringify(expected.args)+' moved='+move);
      }
      const state=[];for(let xs=expected.globalsAfter;xs;xs=xs[1]){const [symbol,value]=xs[0],actual=decode(get(gen.ownerWords.get(symbol.symbol)+2));assert.deepEqual(actual,value,expected.definition+' global '+symbol.symbol);state.push([symbol,actual]);}rows.push({name:expected.name,moved:move,values,after,globals:state});
    }
  }
  if(process.env.CCL_DISPATCH_METADATA){
    const keywordMetadata=checkKeywordMetadata({gen,memory,tcr,root,get,put,encode});
    parentPort.postMessage({base,comparisons:rows.length,keywordMetadata});
  }else if(process.env.CCL_LIBRARY_CASE){
    parentPort.postMessage({focused:true,base,comparisons:rows.length,collections,internalCollections,rows,...gen.summary()});
  }else{
  ...
  parentPort.postMessage({bignums,keywordMetadata,installation,funcallable,signedZero,libmRows,shiftRows,refusals,layout:{...layout,values:layoutValues},retryCollections,integerCalls,floatCalls,fastChecks,base,comparisons:rows.length,collections,internalCollections,rows,...gen.summary()});
}
}
