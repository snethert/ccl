...
def fixture(name):
 for directory in (h,h.parent/'bootstrap-gcd',h.parent/'bootstrap-limb-division',h.parent/'bootstrap-limb-multiply',h.parent/'bootstrap-numeric-dispatch',h.parent/'bootstrap-clos-accessors',h.parent/'bootstrap-lexpr-spread'):
...
(o/'compiled/moving-pools.json').write_text(json.dumps({str(b):moving_vectors(b) for b in [262144,2147483648]}))
...
with (o/'execution.log').open('w') as log:subprocess.run(['/usr/local/bin/node',o/'check.mjs',o,o/'execution.json'],check=True,stdout=log,stderr=subprocess.STDOUT,timeout=300)
...
shutil.copy(h/'population-check.mjs',o/'population-check.mjs')
subprocess.run(['/usr/local/bin/node',o/'population-check.mjs',o/'collector.wasm',o/'population-checks.json'],check=True)
import os
kernel=r.parent/'ccl-evidence/2026-09-12-native-census-r7/baseline/build/dx86cl64'
image=r.parent/'ccl-evidence/2026-09-16-stage1-1a-r2/native/baseline.image'
shutil.copy2(kernel,o/'census-kernel')
(o/'census-kernel').chmod(0o755)
with (o/'census.log').open('w') as log:
 subprocess.run([o/'census-kernel','-I',image,'--no-init','--batch','--load',h/'census.lisp'],
                check=True,stdout=log,stderr=subprocess.STDOUT,
                env={**os.environ,'GENERIC_CENSUS_OUTPUT':str(o/'census.json')},timeout=30)

subprocess.run([sys.executable,h/'controls.py',o],check=True)
