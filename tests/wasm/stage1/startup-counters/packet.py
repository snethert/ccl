...
from run import HERE,ROOT,PARENT,CONFIG,old,SOURCES,pins,sha,read,save
ID='STAGE1-STARTUP-COUNTERS-R1'
...
def keep(p):
 for f in files(p):
  ...
  if 'faults' in n.parts and f.name in ['sha256.mjs','bytes.mjs']:continue
...
def main():
 ...
 if a.mode=='retain':
  ...
  save(p/'tools.json',{str(t):sha(Path(t)) for t in [old.CLANG,old.NODE,old.WABT]})
