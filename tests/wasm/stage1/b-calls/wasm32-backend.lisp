...
(defun b-scalar (ir)
  (let ((op (ccl::acode-operator-name (ccl::acode-operator ir))) (args (ccl::acode-operands ir)))
    (case op
      (ccl::lexical-reference
       (let ((n (position (first args) *required-vars* :test #'eq)))
         (unless n (refuse :b-lexical)) (b-wat "(i32.load offset=~d (local.get $incoming))" (* 4 n))))
...
(defun b-call-pass2 (afunc)
  (let* ((ir (ccl::afunc-acode afunc)) (args (ccl::acode-operands ir)))
    (unless (and (eq (ccl::acode-operator-name (ccl::acode-operator ir)) 'ccl::lambda-list)
                 (null (second args)) (null (third args)) (null (fourth args)) (equal (fifth args) '(nil nil))) (refuse :b-lambda))
    (let* ((*required-vars* (first args)) (*temporary-count* 0) (*b-imports* nil)
           (body (b-multiple (sixth args))) (arity (length *required-vars*))
           ...
           (wat (with-output-to-string (s)
             ...
             (write-string (b-condition (b-wat "(i32.ne (local.get $nargs) (i32.const ~d))" arity) 1) s)
             ...
             (write-string (b-condition (b-wat "(i64.gt_u (i64.add (i64.extend_i32_u (local.get $incoming)) (i64.const ~d)) (i64.extend_i32_u (local.get $output)))" (* 16 (ceiling arity 4))) 2) s)
             (write-string "(block $caught (result exnref) (try_table (catch_all_ref $caught) (local.set $frame (local.get $top))" s)
             (write-string (b-reserve 272) s)
             (write-string (b-initialize-roots "(local.get $frame)" 64) s)
...
(defun compile-call-module (source-text name links)
  ...
  (call-with-target
    (lambda ()
      (let* ((*b-call-mode* t) (*module-result-tag* (gensym "B-CALL")) (*module-name* name)
             (*package* (find-package "WASM32-COMPILER")) (*read-eval* nil)
          ...
          (let ((budget 8192))
            (labels ((items (x)
                     ...
                     (walk (x vars depth)
                       ...
                       (cond ((or (null x) (eq x t) (and (integerp x) (<= -536870912 x 536870911)) (member x vars)) t)
                             ((consp x)
                             ...
                             (t (refuse :b-source)))))
              (let* ((parts (items form)) (vars (items (second parts))))
                (unless (and (= (length parts) 3) (eq (first parts) 'lambda) (<= (length vars) 64)
                             (every (lambda (v) (and (symbolp v) v (not (constantp v)) (not (member v lambda-list-keywords)))) vars)
                             (= (length vars) (length (remove-duplicates vars)))) (refuse :b-source))
                (walk (third parts) vars 0))))
          (catch *module-result-tag*
            (ccl::compile-named-function form :name name :target :wasm32 :policy ccl::*default-compiler-policy*)
            (refuse :b-no-output)))))))
