...
def verify(packet,evidence):
 inherited=(HERE/'wasm32-backend.lisp').read_text().split(';;; First B call unit:')[0]
 inherited=inherited.replace('(defvar *b-call-links* nil)\n(defvar *b-call-mode* nil)\n','').replace('  (when *b-call-mode* (return-from wasm32-pass2 (b-call-pass2 afunc)))\n','').rstrip()
 require(inherited==(HERE.parent/'conversions/wasm32-backend.lisp').read_text().rstrip(),'REVIEWED_LL07_UNCHANGED')
 ...
 print('S1-B-CALL-CORE-VERIFIED: native R6/R6a, generated B calls and values, source refusals and compiler mutants')
