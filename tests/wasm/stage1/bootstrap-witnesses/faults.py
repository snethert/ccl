...
def run(out):
    ...
    def nonzero(s):
        assert '(i32.eq ' in s
        return s.replace('(i32.eq ', '(i32.ne ',1)
    wat('izerop-sense','core_izerop',nonzero,'CORE-IZEROP','CORE-IZEROP')
    ...
    # Exercise both image inventory and moving inventory on general ISTRUCTs.
    def istruct_fault(name,target,data,reason):
        original=target.read_bytes()
        try:
            target.write_bytes(data)
            with (dest/(name+'.log')).open('w') as log:
                result=subprocess.run([NODE,out/'istruct-check.mjs',out/'collector.wasm',dest/(name+'.json')],stdout=log,stderr=log,timeout=60)
            text=(dest/(name+'.log')).read_text()
            assert result.returncode!=0 and reason in text,(name,text[-2000:])
            rows.append(dict(name=name,rejected=True,case='istruct-check',diagnostic=reason))
        finally:target.write_bytes(original)
    owner=out/'runtime/collector-owner.mjs';original=owner.read_text()
    assert original.count('tag===130&&n>=1')==1
    istruct_fault('istruct-owner-still-six',owner,original.replace('tag===130&&n>=1','tag===130&&n===6').encode(),'image kind')
    istruct_fault('empty-pinned-istruct-admitted',owner,original.replace('tag===130&&n>=1','tag===130').encode(),'Missing expected exception')
    source=(out/'runtime/collector.c').read_text()
    for name,old,new,reason in [
        ('istruct-collector-still-six','tag==130&&n>=1','tag==130&&n==6','collection refused 2'),
        ('istruct-last-field-untraced','else if(node_subtag(tag)||(tag==130&&n>=1)){scan=n;',
         'else if(node_subtag(tag)||(tag==130&&n>=1)){scan=(tag==130?n-1:n);','AssertionError'),
        ('empty-istruct-admitted','tag==130&&n>=1','tag==130','Missing expected exception')]:
        assert source.count(old)==1
        c=dest/(name+'.c');c.write_text(source.replace(old,new));binary=c.with_suffix('.wasm')
        subprocess.run(['/usr/local/opt/llvm/bin/clang','--target=wasm32','-O2','-nostdlib','-fno-builtin','-matomics','-mbulk-memory','-Wl,--no-entry','-Wl,--import-memory','-Wl,--max-memory=2147549184','-Wl,-z,stack-size=65536','-Wl,--shared-memory','-Wl,--global-base=1048576','-Wl,--export=collect','-Wl,--export=__stack_pointer',c,'-o',binary],check=True)
        istruct_fault(name,out/'collector.wasm',binary.read_bytes(),reason)
