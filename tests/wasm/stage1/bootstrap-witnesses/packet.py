...
ID='STAGE1-BOOTSTRAP-WITNESSES-R1'
...
def pins():
    ...
    result.update({str(p.relative_to(r.ROOT)):sha(p) for p in (r.HERE.parent/'bootstrap-division').iterdir() if p.is_file()})
    for name in ['compiler/X86/X8632/x8632-arch.lisp','library/lispequ.lisp']:
        result[name]=sha(r.ROOT/name)
    return result

def dependencies():
    result=prior.dependencies()
    frontier='2026-09-21-stage1-bootstrap-dependencies-r1/execution/execution-frontier.json'
    result[frontier]=sha(r.EVIDENCE/frontier)
    previous='2026-09-21-stage1-bootstrap-division-r1/packet.json'
    ...
    for f in (r.EVIDENCE/'2026-09-21-stage1-bootstrap-dependencies-r1/execution/compiled/proposal').rglob('*'):
        if f.is_file():result[str(f.relative_to(r.EVIDENCE))]=sha(f)
    return result
...
def assert_source_copies(out):
    ...
    for name in ['compile.lisp','cases.lisp','execute.lisp','probes.lisp','controls.lisp','whole-file.lisp','witnesses.lisp']:
