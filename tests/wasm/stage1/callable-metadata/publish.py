"""Publish LL12 only after a full retained replay, with production role refusals."""
...
def role_controls(e,inventory,report,inventory_hash):
 ...
 for role in sorted(set(inventory['required_record_roles']+t['required_record_roles'])):
  ...
  status,reasons=assess(inventory,damaged,inventory_hash,e);diagnostic='missing artifact roles: S1-LL12-a [generated]'
...
def publish(e,p):
 ...
 t=next(t for t in inventory['tests'] if t['id']==TEST);need(t['status']=='EXECUTED' and t['runner']=='tests/wasm/stage1/callable-metadata/run.py','registered runner')
 ...
 for n,role in [('source-pins.json','implementation'),('source.tar.gz','source'),('wasm32-backend.lisp','implementation'),('snapshot.mjs','implementation'),('transport.mjs','implementation'),('scope.json','schema'),('coverage.json','schema'),('abi-decision.json','abi'),('abi-binding.json','abi'),('toolchain.json','host-compiler'),('options.json','options'),('summary.json','log'),('verification.json','log'),('assessment/controls.json','test'),('assessment/assessment.json','log'),('execution.tar.gz','test'),('native.tar.gz','log'),('native-references.json','log'),('qualification/summary.json','log'),('qualification/verification.json','log')]:add(p/n,role)
