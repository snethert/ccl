"""Compile the condition system with native class cells and initialization methods."""
...
GD = HERE.parent / 'bootstrap-generic-dispatch'
...
# GD's probes.lisp and inputs.lisp are fragments spliced in below, never
# whole driver files, so GD is not in the search list.
...
'''))
    ...
    if name == 'execute.lisp':
        ...
        copy(GD / 'graph.lisp', path.parent / 'graph.lisp')
    ...
    if name == 'new-inputs.lisp':
        ...
        needle = '(defun new-inputs (name)\n'
        assert text.count(needle) == 1
        path.write_text((HERE.parent / 'lap-primitives/lap-inputs.lisp').read_text() + '\n' + text.replace(needle, needle + '''  (let ((lap (lap-inputs (symbol-name name))))
    (when lap (return-from new-inputs (values lap t))))
'''))
    if name == 'arch-probes.lisp':
        # The generic-dispatch rewrite leaves "...))))\n)": call, form, quoted
        # list, APPEND and DEFUN closers. The LAP probes go inside the list.
        text = path.read_text().rstrip()
        assert text.endswith('))))\n)'), text[-40:]
        path.write_text(text[:-len('))\n)')] + '\n' + (HERE.parent / 'lap-primitives/lap-probes.lisp').read_text() + '))\n)\n')
    if name == 'numeric-files.lisp':
        path.write_text(path.read_text() + '\n' + (HERE.parent / 'lap-primitives/lap-files.lisp').read_text())
    if name == 'compile.lisp':
        compiler_driver = path.parent / 'compile.py'
        driver_text = compiler_driver.read_text().replace('timeout=120', 'timeout=300')
        old_loop = "    for p in out.glob('*.wat'):"
        assert driver_text.count(old_loop) == 1
        driver_text = driver_text.replace(old_loop, '    def assemble(p):')
        driver_text = driver_text.replace("    print('PASS: generated'",
            "    from concurrent.futures import ThreadPoolExecutor\n"
            "    with ThreadPoolExecutor(max_workers=4) as workers:\n"
            "        list(workers.map(assemble, sorted(out.glob('*.wat'))))\n"
            "    print('PASS: generated'")
        compiler_driver.write_text(driver_text)
        text = path.read_text()
        needle = "(dolist (name '(core-dispatch-make core-dispatch-set))"
        assert text.count(needle) == 1
        path.write_text(text.replace(needle,
            "(dolist (name '(core-dispatch-make core-dispatch-set core-selection-first core-selection-second core-selection-default core-selection-generic core-generic-class-of-instance core-generic-class-of-function core-generic-prepare core-generic-select core-condition-prepare))"))
    ...
    if name == 'arch-probes.lisp':
        text = path.read_text()
        text = '(load (merge-pathnames "cpl-inputs.lisp" *load-pathname*))\n' + text
        text = text.rstrip()
        assert text.endswith('))\n)')
        path.write_text(text[:-4] + '\n' + (HERE / 'probes.lisp').read_text() + '))\n)\n')
        copy(HERE / 'inputs.lisp', path.parent / 'cpl-inputs.lisp')
    if name == 'new-inputs.lisp':
        text = path.read_text()
        needle = '(defun new-inputs (name)\n'
        path.write_text(text.replace(needle, needle + "  (multiple-value-bind (inputs covered) (cpl-inputs name) (when covered (return-from new-inputs (values inputs t))))\n"))
    if name == 'probes.lisp':
        text = path.read_text()
        needle = '(result (let (('
        assert text.count(needle) == 1
        path.write_text(text.replace(needle, "(result (let ((*b-cpl-conditions* (or (eql (search \"CORE-CPL-\" (symbol-name name)) 0) (condition-system-entry-p name))) ("))
    if name == 'numeric-files.lisp':
        path.write_text(path.read_text() + '\n' + (HERE / 'class-typep.lisp').read_text())
    if name == 'execute.lisp':
        text = path.read_text()
        text = text.replace("(dolist (name '(core-generic-order", "(dolist (name '(core-cpl-select core-cpl-decline core-cpl-types core-cpl-restart core-cpl-construct core-cpl-resignal core-generic-order")
        path.write_text(text)
        graph = path.parent / 'graph.lisp'
        text = graph.read_text()
        needle = '(when (> (length root) 8)\n                 (loop for name'
        assert text.count(needle) == 1
        text = text.replace(needle, '(if (= (length root) 6)\n                 (loop for name in *cpl-class-names* collect (cons (symbol-name name) (reference (find-class name))))\n                 (when (> (length root) 8)\n                 (loop for name')
        text = text.replace('(reference (find-class name)))))))', '(reference (find-class name))))))))')
        graph.write_text(text)
    if name == 'arch-probes.lisp':
        copy(HERE / 'methods.lisp', path.parent / 'condition-methods.lisp')
        text=path.read_text().replace('(defun arch-probe-forms ()', '(load (merge-pathnames "condition-methods.lisp" *load-pathname*))\n\n(defun arch-probe-forms ()')
        text=text.replace('(append (generic-method-forms)', '(append (condition-system-support-forms) (condition-system-method-forms) (generic-method-forms)')
        path.write_text(text)
    if name == 'execute.lisp':
        path.write_text(path.read_text().replace("(let ((needed (append '(", "(let ((needed (append (loop for name being the hash-keys of by-name when (eql (search \"CORE-CONDITION-\" (symbol-name name)) 0) collect name) '(").replace("(dolist (name '(core-cpl-select", "(dolist (name '(core-condition-table-grow core-condition-table-remove core-condition-remove-read-only core-condition-own-table core-condition-table-clear core-condition-implicit-type core-condition-implicit-divide core-condition-implicit-unbound core-condition-implicit-undefined core-condition-implicit-float core-condition-table-store core-condition-new-cell core-condition-rebuild-cells core-condition-table-read-only core-condition-create core-condition-signal-string core-condition-signal-symbol core-condition-spread core-condition-initargs core-condition-slots core-condition-unlisted core-condition-cerror core-condition-indirect core-condition-class-cell core-cpl-select"))
        copy(HERE / 'graph.lisp', path.parent / 'graph.lisp')
        text = path.read_text()
        text = text.replace('(setq modules (nreverse modules))',
            """(dolist (module *condition-cpl-modules*)
      (dolist (entry (cons module (getf module :children)))
        (push entry modules)))
    (setq modules (remove-duplicates (nreverse modules) :key (lambda (m) (getf m :name)) :test #'equal))""")
        text = text.replace('(let ((needed (append (loop for name',
            """(let ((needed (append (loop for module in *condition-cpl-modules* append
                                  (remove-if-not (lambda (name) (gethash name by-name))
                                                 (getf module :dependencies)))
                                (loop for name""")
        text = text.replace('(write-string "],\\"function\\":" s)',
                            '(format s "],\\"cplMode\\":~a,\\"function\\":" (if (member m *condition-cpl-modules*) "true" "false"))')
        text = text.replace('(setq modules (remove-duplicates', """(with-open-file (s (concatenate 'string out "condition-callers.json")
                            :direction :output :if-exists :supersede)
      (write-char #\\[ s)
      (loop for name in *condition-system-callers* for i from 0 do
        (unless (zerop i) (write-char #\\, s))
        (format s "~s" (symbol-name name)))
      (write-char #\\] s))
    (setq modules (remove-duplicates""")
        path.write_text(text)
    return result
