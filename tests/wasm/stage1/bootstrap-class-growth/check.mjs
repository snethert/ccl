...
if(isMainThread){
  ...
  // Run the changed paths first; every case still receives its full reset.
  const orderedNative=[...native].sort((a,b)=>Number(conditionCallers.has(b.definition))-Number(conditionCallers.has(a.definition)));
  for(const expected of orderedNative.filter(x=>process.env.CCL_DISPATCH_METADATA ? x===native[0] : !process.env.CCL_LIBRARY_CASE||(process.env.CCL_LIBRARY_CASE.endsWith('*') ? x.definition.startsWith(process.env.CCL_LIBRARY_CASE.slice(0,-1)) : x.definition===process.env.CCL_LIBRARY_CASE))){ fs.writeSync(2,expected.definition+' '+(expected.args[0]?.graph ? expected.args[0].graph.nodes.length+' graph nodes' : JSON.stringify(expected.args))+'\n');
    ...
    for(const move of [false,true]){
      ...
      if(expected.definition==='CORE-CONDITION-TABLE-GROW'&&!move&&expected.values[1]===1500){
  ...
  parentPort.postMessage({bignums,keywordMetadata,installation,funcallable,signedZero,libmRows,shiftRows,refusals,layout:{...layout,values:layoutValues},retryCollections,integerCalls,floatCalls,fastChecks,base,comparisons:rows.length,collections,internalCollections,growthChecks,rows,...gen.summary()});
}
}
