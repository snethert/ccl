...
def verify(packet,evidence):
 for name in ('loader.mjs','binary.mjs','stub.wat'):
  expected=(HERE.parent/'b-catch-throw'/name).read_text()
  if name=='loader.mjs':expected=expected.replace('wasm32-shared-B-exnref-tail-catch-v1','wasm32-shared-B-exnref-tail-block-v1')
 ...
 temp=Path(tempfile.mkdtemp(prefix='ccl-block-verify-')).resolve()
 try:
 ...
 print('S1-B-BLOCK-EXITS-VERIFIED: native R6/R6a, generated lexical exits, cons allocation and unwind restoration, native comparisons, mutants, lazy composition and refusals')
