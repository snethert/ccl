...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-BLOCK-EXITS-R1','scope':'Generated BLOCK/RETURN-FROM, closed lexical exits, rooted CONS and restoration through cleanup/special extents; auxiliary execution, no LL05/LL19 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 sources += [HERE.parent/'b-catch-throw'/name for name in ('loader.mjs','binary.mjs','stub.wat')]
