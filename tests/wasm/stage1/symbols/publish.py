"""Publish LL09-a only after a full retained replay, with production role refusals."""
...
def role_controls(e,inventory,report,inventory_hash):
 ...
 for role in sorted(set(inventory['required_record_roles']+t['required_record_roles'])):
  ...
  status,reasons=assess(inventory,damaged,inventory_hash,e);diagnostic='missing artifact roles: S1-LL09-a [generated]'
...
def publish(e,p):
 ...
 t=next(t for t in inventory['tests'] if t['id']==TEST);need(t['status']=='EXECUTED' and t['runner']=='tests/wasm/stage1/symbols/run.py','registered runner')
