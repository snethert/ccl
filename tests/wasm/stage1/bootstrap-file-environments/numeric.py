...
def copy_driver(src,dest,*args,**kwargs):
    ...
    if Path(dest).name=='compile.lisp':copy(HERE/'numeric-files.lisp',Path(dest).parent/'numeric-files.lisp')
    return result
...
assert set(prior['names'])<=originals
assert originals-set(prior['names'])=={'MULTIPLY-FIXNUMS','UPGRADED-COMPLEX-PART-TYPE'}
assert {'MULTIPLY-FIXNUMS','UPGRADED-COMPLEX-PART-TYPE'}<={r['definition'] for r in rows}
parent.save(out/'numeric-execution.json',dict(status='PASS',original_definitions_executed=len(originals),non_nil_witness=len(non_nil),new_executions=sorted(originals-set(prior['names'])),whole_file_numeric_definitions=sorted({r['definition'] for r in rows}),whole_file_numeric_cases=len(rows),whole_file_numeric_comparisons=4*len(rows),target_comparisons=sum(r['comparisons'] for r in execution['rows']),scope='Every numeric row names its actual whole-file module. Other corpus definitions retain their prior compilation provenance; the independent recount does not claim all prior executions used file environments. Multiplication of common-domain fixnums includes boxed products; full bignum library arithmetic still has missing dependencies.'))
