...
for(const m of mods){const b=fs.readFileSync(dir+'/installed/'+m.name+'.wasm');assert.throws(()=>legacyInspect(b),/FUNCTION_IMPORT/,'legacy loader refuses new capability');const imports=WebAssembly.Module.imports(modules.get(m.name));assert.deepEqual(imports.filter(i=>i.kind==='function'),[{module:'owner',name:'ensure',kind:'function'}]);}
...
for(const scenario of [{cap:8},{cap:16},{cap:32},{cap:128},{cap:512},{cap:512,empty:true},{cap:32768,high:true},{cap:32,fail:true}])for(const c of cases){
 ...
 const symbol=()=>{const p=allocImage(32);store(p,1850);for(let j=1;j<8;j++)store(p+4*j,NIL);store(p+28,0);return p+6;};
 ...
 for(const [i,n] of ['dyn_a','dyn_b','dyn_u'].entries())if(symbols[n]){store(symbols[n]+2,i===2?51:4*(101+2*i));store(symbols[n]+22,4*(i+1));}
 ...
 set(80,196608);set(76,196608);set(84,229376);set(92,229376);set(88,229376);set(96,245760);set(104,266240);set(108,64);for(let i=0;i<64;i++)store(266240+4*i,243);set(188,NIL);set(120,OUT);set(124,OUT+4*c.capacity);
 const regions=[['tcr',1024,1280],['external',4096,16384],['image',77824,77864],['vstack',ROOT,163840],['temp',196608,229376],['control',229376,245760],['bindings',266240,270336],['c-stack',1048576,1114112],['root-list',1180000,1184096],['scratch',1200000,1800000]].map(([role,start,end],i)=>({name:role+i,role,start,end}));
 ...
 const owner=CollectorOwner.create(memory,bytes,digest,layout);let retries=0,moves=0,grown=0;
 ...
 const entries={};mods.forEach((m,i)=>{const instance=new WebAssembly.Instance(modules.get(m.name),{env:{memory,tcr,table,tail_table,code_registry:4096,call_error,type_error,nonlocal_exit},symbols,keywords,codes,owner:{ensure}});entries[m.name]=instance.exports.entry;table.set(i+1,instance.exports.entry);tail_table.set(i+1,instance.exports.tail_entry);});
 assert.equal(retries,0,'instantiation has no allocation authority');
 let result,refusal=null;try{result=entries[c.function](handles[c.function],c.args.length);}catch(e){if(e instanceof WebAssembly.Exception&&e.is(call_error)){refusal=e.getArg(call_error,0);if(!scenario.fail)throw new Error(c.id+': checked '+refusal);}else throw e;}
 ...
 const graph=[...objects];const token=v=>{v=v>>>0;if(v===NIL)return 'nil';if(v===T)return 't';if(v%4===0)return (v|0)/4;if(v%8===1){let i=graph.indexOf(v);if(i<0){i=graph.length;graph.push(v);}return 'n'+i;}throw new Error('unhandled result '+v);};
 ...
 collections+=moves;growths+=grown;rows.push({id:c.id,capacity:cap,empty:!!scenario.empty,high:!!scenario.high,retries,moves,grown});
}
