...
from run import HERE,ROOT,E,sha,save,CLANG,NODE,WABT
ID='STAGE1-POPULATION-ACCESS-R1'
SOURCES=['README.md','population.c','derive.py','native.lisp','check.mjs','run.py','packet.py']
def read(p):return json.loads(p.read_text())
def pins():
 paths=[HERE/n for n in SOURCES]+[ROOT/n for n in ['runtime/wasm32/bootstrap-populations.mjs','runtime/wasm32/collector.c','tests/wasm/stage1/hash-tables/adapter.wat','tests/wasm/stage1/hash-tables/generated.mjs','lib/misc.lisp','library/lispequ.lisp']]
...
def main():
 ...
 if a.mode=='retain':
  ...
  save(p/'source-pins.json',pins());save(p/'deterministic.json',deterministic(x));save(p/'tools.json',{str(t):sha(t) for t in [CLANG,NODE,WABT]})
  for f in files(x):
