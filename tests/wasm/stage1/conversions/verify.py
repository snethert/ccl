#!/usr/bin/env python3
"""Replay native qualification and recompile every conversions compiler mutant."""
...
spec=importlib.util.spec_from_file_location("conversions_run",HERE/"run.py");runner=importlib.util.module_from_spec(spec);spec.loader.exec_module(runner);execute=runner.execute
...
def verify(packet,evidence):
 inherited=(HERE/'wasm32-backend.lisp').read_text().split(';;; Typed internal primitives.')[0]
 inherited=inherited.replace('(defvar *primitive-signature* nil)\n','').replace('  (when *primitive-signature* (return-from wasm32-pass2 (primitive-pass2 afunc)))\n','').rstrip()
 require(inherited==(HERE.parent/'representation/wasm32-backend.lisp').read_text().rstrip(),'REVIEWED_LL04_UNCHANGED')
 ...
 with tempfile.TemporaryDirectory(prefix='ccl-conversions-verify-') as tmp:
  ...
  for name,backend in [('positive',None)]+list(proposed.items()):
   ...
   for p in original.iterdir():
    if p.suffix in ('.wat','.wasm') or p.name in ('native.json','cases.json','refusals.json','modules.json','compiler.dx64fsl'):
 ...
 print('S1-LL07-VERIFIED: native R6/R6a, generated typed conversions, static refusals and recompiled compiler mutants')
