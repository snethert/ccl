...
def run(evidence,native,qualification,out):
 ...
 for name,text in mutations((HERE/'wasm32-backend.lisp').read_text()).items():
  ...
  controls.append({'name':name,'status':'REJECTED','oracle':'unchanged native arithmetic and physical memory assertions','exit_code':code})
 save(out/'commands.json',commands);save(out/'controls.json',controls);target=read(out/'execution.json')
 summary={'status':'PASS','modules':target['modules'],'cases':len(target['cases']),'returns':sum(c['error']==0 for c in target['cases']),'checked_refusals':sum(c['error']!=0 for c in target['cases']),'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'memory_pages':32769,'scope':'Real pass-2 generated typed internal primitives; raw ABI separate from Lisp B. No allocator, module installation or general Lisp calls.'};save(out/'summary.json',summary);print('S1-LL07-PASS',summary)
