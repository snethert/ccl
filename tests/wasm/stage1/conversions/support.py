...
def compile_cases(evidence,out,backend=None):
 ...
 with tempfile.TemporaryDirectory(prefix='ccl-conversions-') as temp:
  ...
  with Unit(src,out/'proposal'):
   ...
   env={'PATH':'/usr/local/bin:/usr/bin:/bin','LANG':'C','LC_ALL':'C','CCL_DEFAULT_DIRECTORY':str(src),'CONVERSION_OUTPUT':str(out)+'/', 'CONVERSION_CASES':str(out/'cases.lisp'),'CONVERSION_REFERENCE':str(HERE/'native.lisp')}
   ...
   require(child.returncode==0 and 'S1-CONVERSIONS-COMPILE-PASS' in (out/'compile.log').read_text(),'NATIVE_COMPILE '+str(out/'compile.log'))
 ...
 native=read(out/'native.json');expected=[{k:c[k] for k in ('id','expected','after')} for c in cases()];require(native==expected,'NATIVE_ORACLE')
 (out/'installed').mkdir()
 for m in modules():
  name=m['name'];subprocess.run(['/usr/local/bin/wat2wasm','--enable-threads','--enable-exceptions',str(out/(name+'.wat')),'-o',str(out/(name+'.wasm'))],check=True,capture_output=True);shutil.copy(out/(name+'.wasm'),out/'installed'/(name+'.wasm'))
