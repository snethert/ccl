...
export async function install({dir,memory,tcr,get,put,service,collector,config,result}){
 ...
 const adapter=await WebAssembly.compile(fs.readFileSync(dir+'/adapter.wasm')),adapters=[];
 for(const [name,op] of [['pop_raw_get',0],['pop_raw_set',1],['pop_raw_type',2],['pop_gc',4]]){
  const id=op+1,instance=new WebAssembly.Instance(adapter,{env,hash:{run:service.pop_run,collect:collector.collect,config,operation:op,scratch:1900000,scratch_end:2031072,result}});
  register(id,instance);adapters.push(instance);functions.set(name,object(id,NIL));
 }
 const compiled=[];
 ...
 return {keywords,
  invoke(name,args){const entry=entries.get(name),incoming=132096,output=132352,owner=132512;
   ...
   assert.equal(pair[1]>>>0,get(tcr+116),name+' result count');assert.equal(pair[0]>>>0,get(output),name+' primary');invocations++;
   const values=Array.from({length:pair[1]>>>0},(_,i)=>get(output+i*4));put(tcr+116,0);return values;
  },summary(){return {invocations,modules:mods.length,delivery:adapters.map(a=>Object.fromEntries(['fixed_calls','dynamic_calls','direct_calls'].map(n=>[n,a.exports[n].value])))};}
 };
}
