...
def run(evidence,native,qualification,out):
 ...
 summary={'status':'PASS','modules':target['modules'],'native_cases':len(read(out/'positive/cases.json')),'comparisons':target['comparisons'],'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'tail_runs':len(target['tail_runs']),'tail_steps_per_run':100000,'tail_stack_bytes':2048,'tail_root_depth':2,'non_tail_checks':target['non_tail_checks'],'tail_checks':target['tail_checks'],'development_regressions':len(regression),'local_checks':target['local_checks'],'resource_refusals':target['resource_refusals'],'allocation_boundary_checks':target['allocation_boundary_checks'],'result_capacity_checks':target['result_capacity_checks'],'callable_checks':target['callable_checks'],'closure_checks':target['closure_checks'],'ownership_inspections':target['ownership_inspections'],'peak_root_frames':target['peak_root_frames'],'lazy_composition':composition['status'],'cleanup_entries':target['cleanup_entries'],'cleanup_resources':target['cleanup_resources'],'unwind_functions':sum(m['name'].startswith('uw_') for m in read(out/'positive/modules.json')),'public_dispatches':target['public_dispatches'],'internal_entries':target['internal_entries'],'scope':'Generated UNWIND-PROTECT over checked Wasm exceptions. Cleanup checkpoints, exact normal multiple values, exception replacement and pending-extent tail inhibition. No condition classes/handlers, catch/throw, published TCR handler chain, GC or complete LL05/LL19 credit.'};save(out/'summary.json',summary);print('S1-B-UNWIND-PROTECT-PASS',summary)
