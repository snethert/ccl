...
let cleanupEntries=0,cleanupResources=0;let publicDispatches=0,internalEntries=0;let checks=0,peakRoots=0,activeFrame=0,activeRoots=0,topContext=0;
...
for(const observed of [false,true]){
 ...
 for(const m of mods){
  ...
  tail_table.set(functions.size,observed?new WebAssembly.Instance(tailObserver,{env:{entry:tail,observe:(self,n,context)=>{internalEntries++;inspect('internal '+m.name);assert.equal(load(u32(self)-2),4*(mods.findIndex(x=>x.name===m.name)+1),'tail self');assert.equal(get(64),u32(context)+48,'tail argument area');assert.equal(get(128),u32(context)+32,'retired root frames');assert.equal(load(u32(context)+40),u32(self),'continuation SELF root');assert.equal(load(u32(context)+12),load(u32(context)+32),'saved and linked parent root');assert.equal(load(u32(context)+36),2+4*Math.ceil(n/4),'tail root count');if(m.name==='uw_cleanup'){
 const c=u32(context),budget=c-get(120),retention=load(c+12);
 assert.equal(retention,c-budget-16*Math.ceil((8+budget)/16),'cleanup resumes at original stack cursor');
 assert.equal(load(retention+4),budget/4,'cleanup retention root extent');
