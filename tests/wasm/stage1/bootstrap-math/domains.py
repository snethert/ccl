...
def check(out):
    ...
    assert all(operators[op] for op in list(operators)[:4])
    save('math-operator-coverage.json',dict(executed={k:v for k,v in operators.items() if v},
        ...
        scope='The four requested portable operators are source-emitted and executed. Additional MINUS1 dispatch reaches the already exercised subtraction helper but currently has no source-emitted execution witness.'))
