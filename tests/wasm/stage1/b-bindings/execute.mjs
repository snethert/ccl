...
const keywords=Object.fromEntries(['a','b','external','bad','allow-other-keys'].map((k,i)=>[k,262150+i*16]));
...
for(const observed of [false,true]){
 ...
 for(const m of mods){const fnImports={},selfImports={};for(const [name,fn]of functions){selfImports[name]=handles.get(name);fnImports[name]=observed?((self,n)=>{assert.equal(u32(self),handles.get(name),'direct self identity');inspect('direct '+name);const before=[get(64),get(120),get(124),get(128)];const result=fn(self,n);assert.deepEqual([get(64),get(120),get(124),get(128)],before,'callee ownership '+name);inspect('after '+name);return result;}):fn;}
  const instance=new WebAssembly.Instance(compiled.get(m.name),{env:{memory,tcr,table,resolve,call_error,type_error},functions:fnImports,handles:selfImports,keywords});functions.set(m.name,instance.exports.entry);table.set(functions.size,instance.exports.entry);}
 for(const start of [2048,2147483648]){
  const root=start-8,out=start+2048,owner=out+256,limit=start+32768;
  const encode=x=>typeof x==='number'?(x*4+4294967296)%4294967296:x==='nil'?NIL:x==='t'?T:x.startsWith(':')?keywords[x.slice(1)]:x.startsWith('n')?16385+8*Number(x.slice(1)):handles.get(x.slice(2));
  const decode=x=>{x=u32(x);if(x===NIL)return'nil';if(x===T)return't';for(const [k,v]of Object.entries(keywords))if(x===v)return ':'+k;if(x>=16385&&x<16449&&(x-16385)%8===0)return'n'+(x-16385)/8;if(x%4===0)return (x>=2147483648?x-4294967296:x)/4;throw Error('Unrepresented result '+x);};
  for(const c of cases){activeFrame=owner;activeRoots=64+mods.find(m=>m.name===c.function).bound_words;new Uint8Array(memory.buffer,0,18000).fill(0);if(start>18000)new Uint8Array(memory.buffer,root,32776).fill(0);
...
parentPort.postMessage({status:'PASS',modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:20});
}
