...
(defun b-binding-code (required opt keys)
...
(defun b-call (callee argument-list)
  (unless (and (null (second argument-list)) (<= (length (first argument-list)) 64)) (refuse :b-arguments))
...
(defun b-multiple (ir)
  (let ((op (ccl::acode-operator-name (ccl::acode-operator ir))) (args (ccl::acode-operands ir)))
    (case op
      (ccl::call (b-call (first args) (second args)))
      (ccl::values
...
(defun b-call-pass2 (afunc)
  (let* ((ir (ccl::afunc-acode afunc)) (args (ccl::acode-operands ir)))
    (unless (and (eq (ccl::acode-operator-name (ccl::acode-operator ir)) 'ccl::lambda-list)
                 (null (third args)) (equal (fifth args) '(nil nil))) (refuse :b-lambda))
    (let* ((*required-vars* (first args)) (*temporary-count* 0) (*b-imports* nil) (*b-keywords* nil)
           (opt (second args)) (keys (fourth args))
           (*b-bound-vars* (remove nil (append (first opt) (third opt) (second keys) (third keys))))
           (arity (length *required-vars*)) (maximum (+ arity (length (first opt))))
           (bindings (b-binding-code arity opt keys))
           ...
           (wat (with-output-to-string (s)
             ...
             (write-string "(func (export \"entry\") (type $b_entry) (param $self i32) (param $nargs i32) (result i32 i32) (local $incoming i32) (local $output i32) (local $owner i32) (local $root i32) (local $old_count i32) (local $frame i32) (local $results i32) (local $top i32) (local $count i32) (local $value i32) (local $exception exnref)" s)
             ...
             (write-string (b-condition (b-wat "(i32.or (i32.lt_u (local.get $nargs) (i32.const ~d)) (i32.gt_u (local.get $nargs) (i32.const ~d)))" arity (if keys 64 maximum)) 1) s)
             ...
             (write-string (b-condition "(i64.gt_u (i64.add (i64.extend_i32_u (local.get $incoming)) (i64.extend_i32_u (i32.and (i32.add (i32.mul (local.get $nargs) (i32.const 4)) (i32.const 15)) (i32.const -16)))) (i64.extend_i32_u (local.get $output)))" 2) s)
                             ...
                             ((consp x)
                              (let* ((xs (items x)) (head (car xs)) (n (length (cdr xs))))
                                (unless (or (and (member head '(car cdr)) (= n 1)) (and (member head '(rplaca rplacd)) (= n 2))
                                            ...
                                            (and (eq head 'funcall) (<= 1 n 65))
                                            (and (assoc head *b-call-links*) (<= n 64))) (refuse :b-source))
                                (dolist (part (cdr xs)) (walk part vars (1+ depth)))))
                             (t (refuse :b-source)))))
                ...
                (labels ((add-var (v)
                  ...
                  (dolist (parameter parameters)
                    (cond ((eq parameter '&optional)
                          ...
                          ((eq parameter '&key)
                           (unless (member mode '(:required :optional)) (refuse :b-source)) (setq mode :key))
                          ...
                          (t (refuse :b-source)))))
                (unless (<= (length vars) 64) (refuse :b-source))
                (walk (third parts) vars 0))))
          (catch *module-result-tag*
            (ccl::compile-named-function form :name name :target :wasm32 :policy ccl::*default-compiler-policy*)
            (refuse :b-no-output)))))))
