...
(defvar *validation-wire-prefix* "core")
...
(defun core-compile-file (path &optional native (source-name path))
        ...
        (progn
          (setf (fdefinition 'ccl::fcomp-named-function)
                (lambda (definition name env &optional source-note)
                  ...
                  (let* ((wire (format nil "~a_~d" *validation-wire-prefix* (length *core-records*)))
                         (result nil)
                    ...
                    (push (list source-name name wire outcome (and result (getf result :dependencies))) *core-records*)
                    (when result
                      (setf (getf result :source-name) name
                            (getf result :source-file) source-name)
          ...
          (setf (fdefinition 'ccl::fcomp-file)
                (lambda (&rest args)
                  (let ((records (apply file-compiler args)))
                    (push (list source-name records) *core-files*)
                    (throw done records))))
          (flet ((compile-source ()
                   (catch done
