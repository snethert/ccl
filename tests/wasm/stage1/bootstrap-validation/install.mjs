...
export async function install({dir,memory,tcr,get,put,service,collector,config,result,collect,calculateI,calculateF,ensure}){
 ...
 const hashModule=await WebAssembly.compile(fs.readFileSync(dir+'/hash-adapter.wasm'));
 const hashLeaf=new WebAssembly.Instance(hashModule,{env,hash:{run:eqTable.ht_run,collect:()=>{throw Error('unexpected hash collection');},config:0,operation:0,scratch:1800000,scratch_end:1940000,result:1169504}});
 register(1,hashLeaf);const hashFunction=object(1,NIL);
 const hashSetLeaf=new WebAssembly.Instance(hashModule,{env,hash:{run:eqTable.ht_run,collect:()=>{throw Error('unexpected hash collection');},config:0,operation:1,scratch:1800000,scratch_end:1940000,result:1169504}});
 register(2,hashSetLeaf);const hashSetFunction=object(2,NIL);
 const hashRemoveLeaf=new WebAssembly.Instance(hashModule,{env,hash:{run:eqTable.ht_run,collect:()=>{throw Error('unexpected hash collection');},config:0,operation:2,scratch:1800000,scratch_end:1940000,result:1169504}});
 register(3,hashRemoveLeaf);const hashRemoveFunction=object(3,NIL);
 const compiled=[];
 for(let i=0;i<mods.length;i++){
  ...
  codes[row.name]=id*4;functions.set(row.name,object(id,material.roots[row.poolRoot??i]));if((row.name==='core_gf_identity'||row.name.startsWith('core_key_'))){const raw=functions.get(row.name)-6;put(raw+16,get(material.roots[row.poolRoot??i]-2));put(raw+20,get(material.roots[row.poolRoot??i]+2));}compiled.push({row,wasm,id});
 ...
 function bindMode(cpl){
  for(const row of mods)if(row.function)put(ownerWords.get(row.function)+6,NIL);
  for(const row of mods)if(row.function&&!row.cplMode)put(ownerWords.get(row.function)+6,functions.get(row.name));
  if(cpl)for(const row of mods)if(row.function&&row.cplMode)put(ownerWords.get(row.function)+6,functions.get(row.name));
 }
 bindMode(false);
 const hashRemoveName=owners.find(row=>row.package==='CCL'&&row.name==='%WASM-EQ-TABLE-REMOVE');assert(hashRemoveName);
 const hashSetName=owners.find(row=>row.package==='CCL'&&row.name==='%WASM-EQ-TABLE-SET');assert(hashSetName);
 const hashName=owners.find(row=>row.package==='CCL'&&row.name==='%WASM-EQ-TABLE-GET');assert(hashName);
 ...
 const classCells=new Map();
 symbols.error_message=string('Checked Lisp runtime operation failed.');

 owners.forEach((row,i)=>put(7000000+32*i+4,string(row.name)));
 ...
 put(ownerWords.get(hashRemoveName.id)+6,hashRemoveFunction);
   ...
   mods.forEach((row,i)=>{const self=functions.get(row.name);put(self+18,mat.roots[row.poolRoot??i]);if((row.name==='core_gf_identity'||row.name.startsWith('core_key_'))){put(self+10,get(mat.roots[row.poolRoot??i]-2));put(self+14,get(mat.roots[row.poolRoot??i]+2));}});
   put(tcr+104,680000);put(tcr+108,4096);
   for(let i=0;i<4096;i++)put(680000+4*i,243);
   owners.forEach((row,i)=>{let value=NIL;
    ...
    if(row.package===null)value=51;if(row.package==='CCL'&&row.name==='%TYPE-ERROR-TYPESPECS%')value=mat.roots[material.globalRoots[0]];if(row.package==='CCL'&&row.name==='*PRINT-STRING-LENGTH*')value=44;
    if(row.package==='CCL'&&row.name==='*LOWER-TO-UPPER*')value=mat.roots[material.globalRoots[1]];
    if(row.package==='CCL'&&row.name==='$NHASH.VECTOR_OVERHEAD')value=56;
    if(row.package==='CCL'&&row.name==='*PATHNAME-ESCAPE-CHARACTER*')value=92*256+75;put(7000000+32*i+24,NIL);if(row.package==='KEYWORD')value=ownerWords.get(row.id);
    put(7000000+32*i+8,value);
   });
  },
