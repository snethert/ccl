...
from run import HERE,ROOT,WIN,JOIN,CONFIG,SOURCES,pins,sha,read,save,run,CLANG,NODE
ID='STAGE1-STARTUP-WINNERS-REVIEW-R1'
...
def keep(p):
 for f in files(p):
  ...
  if any(x in n.parts for x in ['faults','inherited']) and f.name not in ['hash.c','hash.wasm','escaped.json','escaped.log','rejected.log','build.log'] and not f.name.endswith('.command.json'):continue
  yield f
def deterministic(p):return {str(f.relative_to(p)):sha(f) for f in keep(p) if f.suffix in ['.mjs','.wasm','.json','.c'] and f.name!='command.json' and not f.name.endswith('.command.json')}
...
def main():
 ...
 if a.mode=='retain':
  ...
  save(p/'inputs.json',{name+'/'+n:sha(e/name/n) for name in [WIN,JOIN,CONFIG,'2026-09-20-stage1-hash-tables-r1'] for n in ['packet.json','source-pins.json','verification.json','toolchain.json']})
  save(p/'tools.json',{str(t):sha(t) for t in [CLANG,NODE]})
