...
for(const observed of [false,true]){
 ...
 for(const start of [65536,2147483648]){
  ...
  let objects=[],objectNames=new Map();
  ...
  for(const c of cases){activeFrame=owner;activeRoots=64+mods.find(m=>m.name===c.function).bound_words;new Uint8Array(memory.buffer,0,32768).fill(0);new Uint8Array(memory.buffer,root,32776).fill(0);new Uint8Array(memory.buffer,heap,32768).fill(0xcd);set(48,heap);set(52,heapLimit);set(56,heap);objects=c.nodes.map((_,i)=>16385+8*i);objectNames=new Map(objects.map((p,i)=>[p,i]));
...
parentPort.postMessage({status:'PASS',modules:mods.length,comparisons:observations.length,ownership_inspections:checks,peak_root_frames:peakRoots,cases:observations,resource_refusals:60,allocation_boundary_checks:8});
}
