...
def manifest(out):
 save(out/'packet.json',{'id':'STAGE1-B-REST-APPLY-R1','scope':'Generated rest lists and APPLY, without the 64-argument ceiling; auxiliary execution, no LL05 gate credit, not integrated.','files':[{'path':str(p.relative_to(out)),'sha256':sha(p),'bytes':p.stat().st_size} for p in sorted(out.rglob('*')) if p.is_file() and p.name!='packet.json']})
def retain(evidence,run,native,qualification,out,development):
 ...
 save(out/'development.json',{'failures':['Attempt 1: the Python corpus wrote the rest marker and its variable as one string; native CCL read the rendered source correctly and disagreed with the Python model. The model tokens and native graph traversal were corrected.','Attempt 2: one extra parenthesis in the emitted APPLY frame-size expression was refused by WABT. Original output and assembler log retained.'],'superseded':'Later positive runs and development mutant trials were superseded by the finalized producer and retained verifier.'})
 sources=[p for p in HERE.iterdir() if p.is_file()]+list(REG.glob('*.py'))+list(REG.glob('*.lisp'))+[REG/'payload/wasm32-backend.lisp',REG/'payload/xwasm32-fasload.lisp',HERE.parent/'architecture/generate.py',HERE.parent/'b-bindings/wasm32-backend.lisp',ROOT/'doc/WASM/tools/r6_registration.py',ROOT/'tests/wasm/native-census/observer.lisp',ROOT/'tests/wasm/native-baseline/tests.lisp',ROOT/'tests/wasm/stage0/abi-decision/decision.json',ROOT/'doc/WASM/contracts/wasm32-layout.v1.json',ROOT/'doc/WASM/contracts/tcr.v1.json']
 pins={}
 for p in sources:
 ...
 save(out/'toolchain.json',tools);save(out/'scope.json',{'accepted_slots_unchanged':True,'inventory_entries_executed':[],'maximum_arguments':'Runtime stack/heap extent; fixed source terms also subject to the existing bounded source-reader budget. Executed through 1024 arguments.','maximum_values':64,'entry_frame_bytes':'align16(264 + 4 * bound_variable_slots)', 'keyword_identities':'explicit fixture-supplied imports, no production namespace or symbol layout claim','root_record':'previous:u32, count:u32, tagged words','stores':{'arguments and retained values':'tagged root slots','result words':'TCR descriptor ownership','frame links and counts':'raw metadata','cons writes':'tagged heap fields; initialized before allocation pointer and bound rest root publication; no polling/collection path'},'remaining':['collection and allocator slow path','dynamic result capacity (still 64)','closure/function objects','full Lisp condition path','lazy stubs and role-authenticated installation','tail chains and cleanup/binding extent','live-local spill/reload under collection']});manifest(out)
