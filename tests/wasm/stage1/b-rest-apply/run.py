...
def run(evidence,native,qualification,out):
 ...
 summary={'status':'PASS','modules':target['modules'],'native_cases':len(read(out/'positive/cases.json')),'comparisons':target['comparisons'],'static_refusals':len(read(out/'positive/refusals.json')),'mutants':len(controls),'resource_refusals':target['resource_refusals'],'allocation_boundary_checks':target['allocation_boundary_checks'],'ownership_inspections':target['ownership_inspections'],'peak_root_frames':target['peak_root_frames'],'scope':'Generated rest-list construction and APPLY with runtime-sized arguments; no 64-argument ceiling. Owned bump-allocation area, no collection. No LL05 slot credit; closure/function objects, full conditions, dynamic result capacity, lazy stubs and tails remain.'};save(out/'summary.json',summary);print('S1-B-REST-APPLY-PASS',summary)
