...
(defun b-bound-address (var)
  (let ((n (position var *b-bound-vars* :test #'eq)))
    ...
    (b-wat "(i32.add (local.get $frame) (i32.const ~d))" (+ 264 (* 4 n)))))
...
(defun b-call (callee argument-list)
  ...
  (let* ((args (first argument-list)) (n (length args)) (arg-slots (* 4 (ceiling n 4)))
         ...
         (out-offset (+ 16 (* 4 arg-slots))) (bytes (+ out-offset 256))
         ...
         (self (if direct (b-wat "(global.get $self_~a)" (second direct)) (b-scalar callee)))
         (codes (mapcar #'b-scalar args)))
    (when (and name (not direct)) (refuse :b-unlinked-function))
    (when direct (pushnew (second direct) *b-imports* :test #'equal))
    (with-output-to-string (s)
      ...
      (write-string (b-reserve bytes) s)
      ...
      (if direct
        (format s "(call $call_~a (i32.load offset=8 (local.get ~a)) (i32.const ~d))" (second direct) base n)
        (format s "(call_indirect (type $b_entry) (i32.load offset=8 (local.get ~a)) (i32.const ~d) (call $resolve (i32.load offset=8 (local.get ~a))))" base n base))
      (write-string "(local.set $count) (local.set $value)" s)
      (write-string (b-condition "(i32.gt_u (local.get $count) (i32.const 64))" 3) s)
...
(defun b-scalar (ir)
  (let ((op (ccl::acode-operator-name (ccl::acode-operator ir))) (args (ccl::acode-operands ir)))
    (case op
      ...
      (ccl::immediate
       (if (keywordp (first args)) (b-keyword (first args)) (emit-expression ir)))
    ...
    (case op
      ...
      (ccl::values
       (let* ((forms (first args)) (n (length forms)))
         (when (> n 64) (refuse :b-value-capacity))
      ...
      ((ccl::multiple-value-prog1 ccl::prog1)
       (let ((forms (first args)) (saved (temporary)))
         (b-frame 64 (lambda (base)
           (with-output-to-string (s)
             (if (eq op 'ccl::prog1)
               (format s "(i32.store ~a ~a) (local.set ~a (i32.const 1))" (b-at base 8) (b-scalar (first forms)) saved)
               (format s "~a (local.set ~a (local.get $count)) (memory.copy ~a (local.get $results) (i32.mul (local.get $count) (i32.const 4)))" (b-multiple (first forms)) saved (b-at base 8)))
...
(defun b-call-pass2 (afunc)
  (let* ((ir (ccl::afunc-acode afunc)) (args (ccl::acode-operands ir)))
    ...
    (let* ((*required-vars* (first args)) (*temporary-count* 0) (*b-imports* nil) (*b-keywords* nil)
           ...
           (wat (with-output-to-string (s)
             (write-string "(module (type $b_entry (func (param i32 i32) (result i32 i32))) (import \"env\" \"memory\" (memory 1 32769 shared)) (import \"env\" \"tcr\" (global $tcr i32)) (import \"env\" \"table\" (table 0 funcref)) (import \"env\" \"resolve\" (func $resolve (param i32) (result i32))) (import \"env\" \"call_error\" (tag $call_error (param i32))) (import \"env\" \"type_error\" (tag $type_error (param i32 i32)))" s)
             (dolist (name (sort *b-keywords* #'string<))
               (format s "(import \"keywords\" ~s (global $key_~a i32))" name name))
             (dolist (name (sort *b-imports* #'string<))
               (format s "(import \"functions\" ~s (func $call_~a (type $b_entry))) (import \"handles\" ~s (global $self_~a i32))" name name name name))
             (write-string "(func (export \"entry\") (type $b_entry) (param $self i32) (param $nargs i32) (result i32 i32) (local $incoming i32) (local $output i32) (local $owner i32) (local $root i32) (local $old_count i32) (local $frame i32) (local $results i32) (local $top i32) (local $count i32) (local $value i32) (local $exception exnref) (local $wide i64)" s)
             ...
             (write-string (b-reserve (* 16 (ceiling (+ 264 (* 4 (length *b-bound-vars*))) 16))) s)
             (write-string (b-initialize-roots "(local.get $frame)" (+ 64 (length *b-bound-vars*))) s)
                             ...
                             ((consp x)
                              (let* ((xs (items x)) (head (car xs)) (n (length (cdr xs))))
                                (unless (or (and (member head '(car cdr)) (= n 1)) (and (member head '(rplaca rplacd)) (= n 2))
                                            ...
                                            (and (member head '(values progn)) (<= n 64))
                                            (and (member head '(prog1 multiple-value-prog1)) (<= 1 n 64))
...
(defun b-apply (callee argument-list)
  ...
  (let* ((prefix (first argument-list)) (n (length prefix))
    ...
    (with-output-to-string (s)
      ...
      (write-string (b-condition (b-wat "(i64.gt_u (i64.add (i64.extend_i32_u (local.get $top)) (i64.add (local.get $wide) (i64.const 256))) (i64.extend_i32_u ~a))" (b-load wasm32::tcr.vsp_limit)) 2) s)
      (format s "(local.set ~a (i32.add (local.get $top) (i32.wrap_i64 (local.get $wide)))) (local.set ~a (i32.div_u (i32.sub (i32.wrap_i64 (local.get $wide)) (i32.const 8)) (i32.const 4))) (local.set $top (i32.add (local.get ~a) (i32.const 256)))"
        output roots output)
      ...
      (write-string (b-store wasm32::tcr.mv_count "(i32.const 0)") s)
      (if direct
        (format s "(call $call_~a (i32.load offset=8 (local.get ~a)) (local.get ~a))" (second direct) base total)
        (format s "(call_indirect (type $b_entry) (i32.load offset=8 (local.get ~a)) (local.get ~a) (call $resolve (i32.load offset=8 (local.get ~a))))" base total base))
      (write-string "(local.set $count) (local.set $value)" s)
      (write-string (b-condition "(i32.gt_u (local.get $count) (i32.const 64))" 3) s)
