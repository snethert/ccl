...
HERE=Path(__file__).resolve().parent;CLOS=HERE.parent/'bootstrap-clos-accessors';PRIOR=HERE.parent/'bootstrap-limb-division'
...
def driver_copy(src,dest,*args,**kwargs):
 ...
 if name=='compile.lisp':
  for name in ('numeric-files.lisp','arch-probes.lisp','new-inputs.lisp','clos-inputs.lisp','extra-controls.lisp'):
