...
(defun recipe-observer (name)
  (when (eq name 'ccl::%%before-and-after-combined-method-dcode)
    (return-from recipe-observer
      `(lambda (magic)
         (declare (notinline ,name))
         (multiple-value-bind (first second) (,name magic)
           (core-collect)
           (values first second (car (caddr magic)))))))

  (let ((arity (cond ((member name '(ccl::add-bignums ccl::subtract-bignum
                                    ccl::add-bignum-and-fixnum
                                    ccl::bignum-logical-and ccl::bignum-logandc2
                                    ccl::bignum-logical-ior ccl::bignum-logical-xor
                                    ccl::bignum-ashift-left ccl::bignum-ashift-right)) 2)
                     ((member name '(ccl::negate-bignum ccl::copy-bignum
                                     ccl::maybe-copy-bignum ccl::bignum-logical-not)) 1))))
    (when arity
      (let ((args (if (= arity 2) '(a b) '(a))))
        (return-from recipe-observer
          `(lambda ,args
             (declare (notinline ,name))
  ...
  (case name
    (invoke-restart
     '(lambda (value)
        (declare (notinline invoke-restart))
        (restart-case (invoke-restart 'answer value)
          (answer (new-value) new-value))))
    (find-restart
     '(lambda ()
        (declare (notinline find-restart))
        (let ((ccl::%restarts% nil))
          (restart-case
              (let ((restart (find-restart 'answer)))
                (core-collect)
                (values (restart-name restart) (not (null restart))))
            (answer () nil)))))
    (restart-name
     '(lambda (name)
        (declare (notinline restart-name))
        (let ((restart (ccl::%cons-restart name nil nil nil nil)))
          (core-collect)
          (restart-name restart))))
    (ccl::applicable-restart-p
     '(lambda (match)
        (declare (notinline ccl::applicable-restart-p))
        (let* ((condition (cons 1 nil))
               (other (cons 2 nil))
               (restart (ccl::%cons-restart 'answer nil nil nil
                           (lambda (condition) (not (null condition)))))
               (ccl::*condition-restarts* (list (cons restart condition))))
          (core-collect)
          (ccl::applicable-restart-p restart (if match condition other)))))
    (compute-restarts
     '(lambda ()
        (declare (notinline compute-restarts))
        (let ((ccl::%restarts% nil))
          (restart-case
              (let ((names nil))
                (dolist (restart (compute-restarts) (nreverse names))
                  (core-collect)
                  (push (restart-name restart) names)))
            (first () nil)
            (second () nil)))))
    (ccl::%active-restart
     '(lambda ()
        (declare (notinline ccl::%active-restart))
        (let* ((restart (ccl::%cons-restart 'answer 0 nil nil nil))
               (cluster (list restart))
               (ccl::%restarts% (list cluster)))
          (multiple-value-bind (active tag) (ccl::%active-restart 'answer)
            (core-collect)
            (values (eq active restart) (eq tag cluster))))))
