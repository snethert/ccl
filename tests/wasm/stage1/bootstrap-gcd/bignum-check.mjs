...
export function checkBignums({gen,memory,tcr,root,collect,encode,get,put}) {
 ...
 for (const name of ['core_float_sign_double','core_float_sign_single']) {
  for (const value of [NIL,0]) {
   const used=get(tcr+48);
   assert.throws(()=>gen.invoke(name,[value]),/checked 4$/);
   assert.equal(get(tcr+48),used);
   records.push(name+' refuses '+value);
  }
 }
 // The generic vector arm must use the same checked digit span as typed access.
 save(0,call('new',[12])[0]);
 call('store',[read(0),0,0xffff*4,0xffff*4]);
 const boxed=call('uvread',[read(0),0])[0];
 assert.equal(get(boxed-6),519);assert.equal(get(boxed-2),0xffffffff);assert.equal(get(boxed+2),0);
 call('uvstore',[read(0),4,17*4]);assert.deepEqual(call('read',[read(0),4]),[0,17*4]);
 collect();assert.equal(call('uvread',[read(0),4])[0],17*4);
 records.push('generic bignum digit reads box unsigned words; writes and movement preserve digits');
 for(const bad of [75,0xfffffffc,12])refuse('generic index '+bad,'uvstore',[read(0),bad,0]);
 for(const bad of [75,0xfffffffc,NIL])refuse('generic value '+bad,'uvstore',[read(0),0,bad],5);
 refuse('generic header backing','uvread',[(end+6)>>>0,0]);
 put(end-8,3*256+7);refuse('generic digit backing','uvread',[end-2,0]);put(end-8,old);
 const owners=JSON.parse(fs.readFileSync(new URL('./compiled/symbols.json',import.meta.url)));
 const symbol=owners.find(row=>row.package==='CCL'&&row.name==='%WASM-DIVIDE-DIGIT');
 assert.ok(symbol,'digit division symbol');
 const module=gen.mods.find(row=>row.function===symbol.id);
 assert.ok(module,'digit division installed');
 for(const [label,args] of [['zero digit divisor',[0,0,0,0,0,0]],
                            ['quotient overflow',[0,1,0,0,0,1]]]) {
  assert.throws(()=>gen.invoke(module.name,args.map(x=>x*4)),/checked [0-9]+$/,label);
  records.push(label);
 }
 return records;
}
