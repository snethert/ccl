...
(defun gcd-dense-inputs ()
  (let ((state 20260922))
    (labels ((word ()
               (setq state (logand #xffffffff (+ (* state 1664525) 1013904223))))
             (number (words)
               (let ((value 1))
                 (dotimes (i words value)
                   (setq value (+ (ash value 32) (word)))))))
      (loop for i below 16 collect
        (let ((factor (1+ (mod (word) 65535))))
          (list (* factor (number (+ 2 (mod i 14))))
                (* factor (number (+ 2 (mod (* i 3) 14))))))))))
...
(defun new-inputs (name)
  ...
  (let ((s (symbol-name name)))
    (cond ((equal s "CORE-CONDITION-PROGRAM") (values '(()) t))
          ((equal s "INVOKE-RESTART")
           (values (mapcar #'list (list -1 0 1 "value" (list 1 2) (ash 1 90))) t))
          ((member s '("FIND-RESTART" "COMPUTE-RESTARTS" "%ACTIVE-RESTART") :test #'equal)
           (values '(()) t))
          ((equal s "RESTART-NAME") (values '((answer) (nil) (:continue)) t))
          ((equal s "APPLICABLE-RESTART-P") (values '((nil) (t)) t))
          ((equal s "CORE-CONDITION-CONSTRUCTION")
           (values (list (list (list 1 2) 'real) (list "datum" 'integer)
                         (list (ash 1 90) 'number)) t))
          ((equal s "CORE-RESTART-ASSOCIATION")
           (values (mapcar #'list (list nil #\a "not real")) t))
          ((equal s "CORE-RESTART-EXTRA-ARGUMENT") (values '(()) t))
          ((equal s "CORE-NUMERIC-RESTART-CAUGHT")
           (values (mapcar #'list (list nil #\a "not real" #c(1 2))) t))
          ((member s '("CORE-NUMERIC-RESTART-USE" "CORE-NUMERIC-RESTART-TWICE"
                       "CORE-NUMERIC-RESTART-HOOK") :test #'equal)
           (values (loop for x in (list nil #\a "not real") append
                     (loop for y in (list -1 0 1 (- (ash 1 90)) 1.5d0)
                       collect (list x y))) t))
          ((member s '("%BIGNUM-BIGNUM-GCD" "%POSITIVE-BIGNUM-BIGNUM-GCD") :test #'equal)
           (values (append (gcd-dense-inputs)
                           (when (equal s "%BIGNUM-BIGNUM-GCD")
                             (loop for (a b) in (subseq (gcd-dense-inputs) 0 4) append
                               (list (list (- a) b) (list a (- b)) (list (- a) (- b)))))
                           (loop for a in (list (1+ (ash 1 70)) (1- (ash 1 96))
                                        (* 45 (1+ (ash 1 127))) (ash 1 192)) append
                     (loop for b in (list (1- (ash 1 65)) (1+ (ash 1 96))
                                          (* 75 (1+ (ash 1 127))) (ash 1 128))
                       collect (list a b)))) t))
          ((equal s "BIGNUM-FIXNUM-GCD")
           (values (loop for a in (list (1+ (ash 1 70)) (1- (ash 1 96))
                                        (* 45 (1+ (ash 1 127)))) append
                     (loop for b in '(1 3 17 45 65535 536870911)
                       collect (list a b))) t))
          ((equal s "GCD-2")
           (values (loop for a in (list 0 1 -1 45 -75 536870911 -536870912
                                        (1- (ash 1 96)) (- (1- (ash 1 96)))) append
                     (loop for b in (list 0 1 -1 45 -75 536870911 -536870912
                                          (1- (ash 1 70)) (- (1- (ash 1 70))))
                       collect (list a b))) t))
          ((equal s "%%BEFORE-AND-AFTER-COMBINED-METHOD-DCODE")
          ...
          ((equal s "CORE-WORD-DIVISION")
           (values (loop for x in (list (ash 1 70) (1- (ash 1 128))
                                        (1+ (ash 1 257)) (1- (ash 1 1024))) append
                     (loop for y in '(1 2 3 65535 65536 536870911 536870912 2147483647 4294967295)
                       collect (list x y))) t))
          ((equal s "CORE-POSITIVE-FIXNUM-PRODUCT")
           (values (loop for a in (list (ash 1 70) (1- (ash 1 96))
                                        (1+ (ash 1 520)) (1- (ash 1 32800))) append
                     (loop for b in '(0 1 2 65535 65536 536870911 536870912)
                       collect (list a b))) t))
          ((equal s "CORE-FLOAT-SIGN-DOUBLE")
           (values '((0.0d0) (-0.0d0) (1.5d0) (-1.5d0)) t))
          ((equal s "CORE-FLOAT-SIGN-SINGLE")
           (values '((0.0s0) (-0.0s0) (1.5s0) (-1.5s0)) t))
          ((equal s "MINUSP")
           (values (mapcar #'list (list 0 1 -1 536870911 -536870912
                                       (ash 1 70) (- (ash 1 70))
                                       0.0s0 -0.0s0 1.5s0 -1.5s0
                                       0.0d0 -0.0d0 1.5d0 -1.5d0)) t))
          ((equal s "%DOUBLE-FLOAT-MINUSP")
           (values '((0.0d0) (-0.0d0) (1.5d0) (-1.5d0)) t))
          ((equal s "%SHORT-FLOAT-MINUSP")
           (values '((0.0s0) (-0.0s0) (1.5s0) (-1.5s0)) t))
          ((member s '("BIGNUM-TRUNCATE" "BIGNUM-TRUNCATE-NO-REM" "BIGNUM-REM") :test #'equal)
           (values
            (loop for a in (list (ash 1 80) (1- (ash 1 128))
                                  (1+ (ash 1 257)) (1- (ash 1 1024))) append
              (loop for b in (list (1+ (ash 1 65)) (1- (ash 1 96))
                                    (1+ (ash 1 130))) append
                (loop for sa in '(1 -1) append
                  (loop for sb in '(1 -1) collect (list (* sa a) (* sb b)))))) t))
          ((equal s "MULTIPLY-BIGNUMS")
           (let ((numbers (list (ash 1 70) (1- (ash 1 96))
                                (- (ash 1 70)) (- (1+ (ash 1 96)))
                                (1- (ash 1 520)) (- (1- (ash 1 544))))))
             (values (append (loop for a in numbers append
                               (loop for b in numbers collect (list a b)))
                             (list (list (1- (ash 1 32800)) (1+ (ash 1 70)))
                                   (list (- (1- (ash 1 32800))) (1+ (ash 1 70))))) t)))
          ((equal s "MULTIPLY-BIGNUM-AND-FIXNUM")
           (values (loop for a in (list (ash 1 70) (1- (ash 1 96))
                                        (- (ash 1 70)) (- (1+ (ash 1 96)))) append
                     (loop for b in '(0 1 -1 2 -3 536870911 -536870912)
                       collect (list a b))) t))
