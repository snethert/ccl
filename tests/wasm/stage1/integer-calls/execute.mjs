...
const bytes=fs.readFileSync(collectorPath),digest=createHash('sha256').update(bytes).digest('hex'),integerBytes=fs.readFileSync(integerPath),integerDigest=createHash('sha256').update(integerBytes).digest('hex');
...
for(const scenario of [{high:false,collect:false},{high:false,collect:true},{high:true,collect:false},{high:true,collect:true},{high:false,collect:true,tiny:true},{high:true,collect:true,tiny:true}]){
 ...
 set(80,196608);set(76,196608);set(84,229376);set(92,229376);set(88,229376);set(96,245760);set(104,266240);set(108,2);for(let i=0;i<2;i++)store(266240+4*i,243);set(188,NIL);set(120,OUT);set(124,OUT+64);
 ...
 const encode=s=>{if(typeof s==='object'){const car=encode(s.cons),p=t(48);assert(p+8<=t(52));store(p,NIL);store(p+4,car);set(48,p+8);return p+1;}let n=BigInt(s);if(n>=-536870912n&&n<=536870911n)return Number(BigInt.asUintN(32,n<<2n));let width=1;while(n<-(1n<<BigInt(width*32-1))||n>=(1n<<BigInt(width*32-1)))width++;const size=8*Math.ceil((4+4*width)/8),p=t(48);assert(p+size<=t(52));store(p,width*256+7);n=BigInt.asUintN(width*32,n);for(let i=0;i<width;i++){store(p+4+4*i,Number(n&0xffffffffn));n>>=32n;}set(48,p+size);return p+6;};
 const decode=v=>{if(v%4===0)return String((v>=2**31?v-2**32:v)/4);assert.equal(v%8,6);const p=v-6,h=get(p),n=Math.floor(h/256);assert.equal(h%256,7);let x=0n;for(let i=n-1;i>=0;i--)x=(x<<32n)|BigInt(get(p+4+4*i));return String(BigInt.asIntN(n*32,x));};
 for(const c of native){
  ...
  if(c.function==='n_add'&&c.args[0]==='0')assert.equal(slow,before,'small fixnum stays in Wasm');
 }
 if(!tiny){
  const refusals=[];
  for(const [name,fn,args,code] of [['zero-divisor','n_truncate',[4,0],34],['noninteger','n_add',[NIL,4],32],['input-budget','n_ash',[4,131072],33]]){
  ...
  rows.push({high,collect,cases:native.length,slow,moves,growths,modules:mods.length,refusals,nested:6});
