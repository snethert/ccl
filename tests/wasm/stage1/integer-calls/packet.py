...
HERE=Path(__file__).resolve().parent;BASE='2026-09-19-stage1-generic-dispatch-r1'
...
def pins(e):
 result=read(e/BASE/'source-pins.json');integration=read(ROOT/'doc/WASM/stage1/integration-ll11b.json');changes={r['file']:r for r in integration['files']}
 ...
 for n in ['runtime/wasm32/integer.c','runtime/wasm32/collector.c','runtime/wasm32/collector-owner.mjs','doc/WASM/stage1/integration-integer-core.json','doc/WASM/stage1/integration-ll11b.json']:result[n]=sha(ROOT/n)
 return result
def selected(d):
 ...
 return {str(f.relative_to(d)):sha(f) for f in files(d) if (f.suffix in ['.wat','.wasm','.dx64fsl'] or f.name in names or f.name in ['native-results.txt','source-refusals.txt'] or f.name.startswith('materialized-')) and not any(n in f.relative_to(d).parts for n in ['proposal','executed-sources'])}
def manifest(p):save(p/'packet.json',dict(id='STAGE1-INTEGER-CALLS-R1',kind='AUXILIARY_GENERATED_INTEGER_CALLS',review_disposition='NOT_REVIEWED',files=[dict(path=str(f.relative_to(p)),sha256=sha(f),bytes=f.stat().st_size) for f in files(p) if f.name!='packet.json']))
...
def retain(e,x,native,p):
 ...
 # Original unsuccessful qualification: positive passed; first bound mutant
 # failed at n_mul rather than the guessed n_add diagnostic. No escaped fault.
 archive(p/'development.tar.gz',[(f,'ccl-numeric-r4/'+str(f.relative_to(Path('/tmp/ccl-numeric-r4')))) for f in files(Path('/tmp/ccl-numeric-r4'))])
 save(p/'development.json',dict(attempt='ccl-numeric-r4',failure='Correct positive execution; fixnum-bound mutant failed first at n_mul/25, not the expected n_add label. The expected label was corrected and nonzero fixnum remainders were added before final qualification. No implementation correction or escaped mutant is claimed.'))
