...
from run import HERE,ROOT,sha,read,save
ID='STAGE1-TERMINATION-EXCLUSION-R1'
SOURCES=['README.md','entries.lisp','native.lisp','admission.mjs','admission-check.mjs','bindings.json','derive.py','generated-check-tail.mjs','run.py','packet.py']
def pins():
 prior=read(ROOT.parent/'ccl-evidence/2026-09-20-stage1-startup-runtime-review-r1/source-pins.json')
 for n,h in prior.items():assert sha(ROOT/n)==h,n
 paths=[HERE/n for n in SOURCES]+[ROOT/n for n in ['runtime/wasm32/collector-owner.mjs','runtime/wasm32/collector.c','tests/wasm/stage1/startup-runtime-review/conditions.mjs','level-1/l1-lisp-threads.lisp','level-1/l1-dcode.lisp','lib/backtrace.lisp']]
 return dict(prior,**{str(p.relative_to(ROOT)):sha(p) for p in paths})
def kept(p):
 for f in sorted(p.rglob('*')):
  ...
  yield f
def deterministic(p):
 return {str(f.relative_to(p)):sha(f) for f in kept(p) if f.suffix in ['.json','.mjs','.wat','.wasm','.lisp'] and not f.name.endswith('.command.json') and f.name!='command.json'}
def inputs(e):
 return {n:sha(e/n) for n in ['2026-09-20-stage1-startup-runtime-review-r1/packet.json','2026-09-20-stage1-startup-config-r2/native-reuse.json','2026-09-20-stage1-bootstrap-heap-census-r1/packet.json','2026-09-12-native-census-r7/baseline/build/dx86cl64','2026-09-16-stage1-1a-r2/native/baseline.image']}
def main():
 ...
 if a.mode=='retain':
  ...
  record=dict(status='PASS',files=len(read(p/'deterministic.json')),pins=len(pins()),summary=read(out/'execution/summary.json'));save(out/'verification.json',record);print(record)
