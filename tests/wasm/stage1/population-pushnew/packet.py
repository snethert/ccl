...
import run as r
ID='STAGE1-POPULATION-PUSHNEW-R1'
SOURCES=['run.py','packet.py','consumers.lisp','lower.lisp','eql-export.c','raw.mjs','admission.lisp','README.md','development.json']
P=r.T
...
def pins():
 ...
 for n in ['tests/wasm/stage1/startup-joined/compile.py','tests/wasm/stage1/equality-tables/keys.c']:
...
def retained(p):
 for f in files(p):
  ...
  if rel.parts[0]=='faults' and f.name in ['eql.wasm','eql.c'] and rel.parts[1] not in ['duplicate','operand-order']:
   yield f;continue
  if rel.parts[0]=='faults' and f.name in ['check.mjs','install.mjs','builder.mjs','adapter.wasm','collector.wasm','population.wasm','native.json','eql-adapter.wasm','eql.wasm','raw.mjs','base-lower.lisp']:continue
  yield f
