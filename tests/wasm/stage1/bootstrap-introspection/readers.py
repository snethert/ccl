...
def replay(evidence,proposal,out):
 ...
 paths=['level-1/l1-aprims.lisp','level-1/l1-clos-boot.lisp'];proof=[]
 edits={
 ...
 for name in paths:
  old=(ROOT/name).read_text();new=(proposal/name).read_text();a,b=edits[name]
  assert old.count(a)==1 and new.count(b)==1 and new.replace(b,a)==old
  # The unchanged native body is a placeholder in this local conditional
  # proof; byte inversion above binds every byte of that body and the rest.
  ...
  proof.append(dict(file=name,original_sha256=hashlib.sha256(old.encode()).hexdigest(),proposal_sha256=hashlib.sha256(new.encode()).hexdigest(),sites=1,inverse_bytes_equal=True))
