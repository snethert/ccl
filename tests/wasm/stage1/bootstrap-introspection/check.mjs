...
if(isMainThread){
  ...
  const memory=new WebAssembly.Memory({initial:Math.max(80,Math.ceil((base+size)/65536)),maximum:32769,shared:true});
  ...
  function services(){
   const regions=[['tcr',1024,1280],['vstack',131064,196608],['temp',700000,780000],['control',900000,1000000],['c-stack',1048576,1114112],['scratch',1200000,1800000],['root-list',4600000,4862144],['external',1170000,1174096],['bindings',650000,654096]].map(([role,start,end])=>({name:role,role,start,end}));
  ...
  function encode(x){
    ...
    if(x&&x.instance){
      ...
      [882,0,NIL,slots].forEach((word,i)=>put(p+4*i,word));put(tcr+48,p+16);return p+6;
    }
    if(x&&x.funcallable){
  ...
  function decode(x){
    ...
    if((x&7)===6){const h=get(x-6),n=h>>>8,tag=h&255;
     ...
     if(tag===114&&n===3)return {instance:{slots:decode(get(x+6))}};
  ...
  let collections=0;const originalRootCount=gen.extraRoots.length;const initialSymbolFields=[...gen.ownerWords.values()].flatMap(p=>[10,14,18].map(d=>[p+d,get(p+d)]));
  for(const expected of native.filter(x=>!process.env.CCL_LIBRARY_CASE||x.definition===process.env.CCL_LIBRARY_CASE)){ fs.writeSync(2,expected.definition+' '+JSON.stringify(expected.args)+'\n');
    ...
    initialSymbolFields.forEach(([p,v])=>put(p,v));gen.extraRoots.length=originalRootCount;gen.reset(movingPools[base]);nodeNext=4194304;nodeDescriptions.clear();rawDescriptions.clear();flaggedSymbols.clear();services();
    ...
    for(const move of [false,true]){
      if(move){for(const row of JSON.parse(fs.readFileSync(dir+'/compiled/symbols.json')))if(row.package===null)put(gen.ownerWords.get(row.id)+2,51);
        // Start with fresh original arguments; the first call may mutate them.
        globals();const fresh=expected.args.map(encode);fresh.forEach((x,i)=>put(root+8+4*i,x));
      ...
      const after=args.map((_,i)=>decode(get(root+8+4*i)));
  ...
  parentPort.postMessage({keywordMetadata,installation,funcallable,signedZero,libmRows,shiftRows,refusals,layout:{...layout,values:layoutValues},retryCollections,integerCalls,floatCalls,fastChecks,base,comparisons:rows.length,collections,internalCollections,rows,...gen.summary()});
}
