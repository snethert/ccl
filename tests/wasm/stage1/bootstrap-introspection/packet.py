...
ID='STAGE1-BOOTSTRAP-INTROSPECTION-R1'

def pins():
    result=parent.pins()
    for directory in ('bootstrap-introspection','bootstrap-funcallable','bootstrap-recipes','bootstrap-core-acceptance'):
        result.update({str(p.relative_to(ROOT)):sha(p) for p in files(HERE.parent/directory)})
    result.update({str(p.relative_to(ROOT)):sha(p) for p in files(ROOT/'runtime/wasm32')})
    return result

def dependencies():
    result=parent.dependencies()
    for name in ('packet.json','artifacts.tar.gz','native/proposal/unit.json'):
        path='2026-09-22-stage1-bootstrap-funcallable-r1/'+name
        result[path]=sha(EVIDENCE/path)
    return result

def source_check(numeric,native):
    for name in ('compile.lisp','cases.lisp','numeric-files.lisp','arch-probes.lisp','controls.lisp','probes.lisp','inputs.lisp','execute.lisp','new-inputs.lisp'):
    ...
    assert report['r6']['native_state_equal'] and report['r6']['identical']==141
    ...
    for path in (ROOT/'runtime/wasm32').glob('*.mjs'):
        assert (numeric/'runtime'/path.name).read_bytes()==path.read_bytes(),path.name
...
def deterministic(environment,numeric):
    ...
    result['numeric/requested-frontier.json']=sha(numeric/'requested-frontier.json')
...
def main():
    ...
    if args.mode=='retain':
        ...
        import frontier
        frontier.report(environment,numeric)
