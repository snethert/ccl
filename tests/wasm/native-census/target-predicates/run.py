#!/usr/bin/env python3
"""Qualify bounded TYPEP constant and optimizer paths in disposable U1."""
import argparse
...
def sources():
    ...
    for p in [HERE/n for n in ('driver.lisp', 'analysis.py', 'run.py')] + [
        HERE.parent/'source-expanders/selection.lisp', HERE.parent/'target-helpers/driver.lisp', HERE.parent/'target-helpers/analysis.py', HERE.parent/'target-types/driver.lisp', HERE.parent/'target-types/analysis.py', ROOT/'compiler/X86/X8632/x8632-arch.lisp',
    ...
    try:
        ...
        for mode in ('normal', 'repeat', *CONTROLS):
            env = dict(PATH='/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin', LANG='C', LC_ALL='C',
                       ...
                       CCL_TRAVERSE_OUTPUT=str(work/(mode+'-traversal.json')), CCL_PREDICATE_OUTPUT=str(work/(mode+'-predicates.json')),
                       CCL_PREDICATE_MODE='normal' if mode == 'repeat' else mode, CCL_TRAVERSE_MODE='normal', CCL_DESCRIPTION_MODE='normal')
            ...
            for p in (HERE.parent/'source-traversal/driver.lisp', HERE.parent/'target-descriptions/descriptions.lisp',
                      ...
                      HERE.parent/'source-expanders/driver.lisp', HERE.parent/'source-expanders/selection.lisp', HERE.parent/'target-helpers/driver.lisp', HERE.parent/'target-types/driver.lisp', HERE/'driver.lisp'):
                argv += ['--load', str(p)]
            argv += ['--eval', '(progn (ccl-target-predicates::run) (ccl:quit))']
            ...
            with (out/(mode+'.log')).open('wb') as log:
                ...
                try: command['exit_code'] = child.wait(timeout=60)
                    ...
                    child.wait(); raise
            captured = {}
            for kind in ('predicates', 'traversal'):
            ...
            c, t = (json.loads(captured[k]) for k in ('predicates', 'traversal'))
            if mode in ('normal', 'repeat'):
                ...
                # These mutations leave the file's existing type tests unchanged.
                # The constant and guard probes expose them.
        ...
        save(out/'summary.json', dict(version=1, status='BOUNDED_TYPEP_PATHS_QUALIFIED', review_disposition='NOT_REVIEWED',
        ...
        for kind in ('predicates', 'traversal'): (out/('repeat-'+kind+'.json.gz')).unlink()
        ...
        print('PASS: TYPEP helper paths; 5 native and 22 checker controls reject; file traversal unchanged, no gate credit.')
...
def verify(packet):
    ...
    c, t = (read(packet/('normal-'+kind+'.json.gz')) for kind in ('predicates', 'traversal'))
    ...
    for mode, reason in CONTROLS.items():
        changed, changed_t = (read(packet/(mode+'-'+k+'.json.gz')) for k in ('predicates', 'traversal'))
    ...
    print('PASS: retained TYPEP paths, 5 native controls, 22 checker controls, direct sources and unchanged traversal.')
