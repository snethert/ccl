...
def sources():
    ...
    for p in [HERE/n for n in ('driver.lisp', 'analysis.py', 'run.py')] + [
        HERE.parent/'source-expanders/selection.lisp', ROOT/'compiler/X86/X8632/x8632-arch.lisp',
    ...
    try:
        ...
        for mode in ('normal', 'repeat', *CONTROLS):
            env = dict(PATH='/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin', LANG='C', LC_ALL='C',
                       ...
                       CCL_TRAVERSE_OUTPUT=str(work/(mode+'-traversal.json')), CCL_HELPER_OUTPUT=str(work/(mode+'-helpers.json')),
                       CCL_HELPER_MODE='normal' if mode == 'repeat' else mode, CCL_TRAVERSE_MODE='normal', CCL_DESCRIPTION_MODE='normal')
            ...
            for p in (HERE.parent/'source-traversal/driver.lisp', HERE.parent/'target-descriptions/descriptions.lisp',
                      ...
                      HERE.parent/'source-expanders/driver.lisp', HERE.parent/'source-expanders/selection.lisp', HERE/'driver.lisp'):
                argv += ['--load', str(p)]
            argv += ['--eval', '(progn (ccl-target-helpers::run) (ccl:quit))']
            ...
            c, t = (json.loads(captured[k]) for k in ('helpers', 'traversal'))
            if mode in ('normal', 'repeat'):
                ...
                # Even the inherited numeric-helper defect leaves this file's
                # existing traversal unchanged. The added numeric probes expose it.
        ...
        save(out/'summary.json', dict(version=1, status='BOUNDED_HELPER_PATHS_QUALIFIED', review_disposition='NOT_REVIEWED',
        ...
        for kind in ('helpers', 'traversal'): (out/('repeat-'+kind+'.json.gz')).unlink()
        ...
        print('PASS: numeric/layout helper paths; 4 native and 15 checker controls reject; file traversal unchanged, no gate credit.')
...
def verify(packet):
    ...
    c, t = (read(packet/('normal-'+kind+'.json.gz')) for kind in ('helpers', 'traversal'))
    ...
    for mode, reason in CONTROLS.items():
        changed, changed_t = (read(packet/(mode+'-'+k+'.json.gz')) for k in ('helpers', 'traversal'))
    ...
    print('PASS: retained helper paths, 4 native controls, 15 checker controls, direct sources and unchanged traversal.')
