#!/usr/bin/env python3
"""Rebuild a bounded macro slice from source in a fresh disposable U1 copy."""
import argparse
...
import sys
import tarfile
from check import HERE, ROOT, NATIVE_CONTROLS, check, controls, read
...
def sources():
    native = HERE.parent
    files = [HERE/p for p in ('driver.lisp', 'check.py', 'run.py', 'verify.py', 'source-inputs.json')]
    ...
    try:
        ...
        for mode in ('normal', 'repeat', *NATIVE_CONTROLS):
            env = dict(PATH='/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin', LANG='C', LC_ALL='C',
                       ...
                       CCL_MACRO_OUTPUT=str(work/(mode+'-macros.json')), CCL_TRAVERSE_MODE='normal',
                       CCL_DESCRIPTION_MODE='normal', CCL_MACRO_MODE='normal' if mode == 'repeat' else mode)
            ...
            for p in (HERE.parent/'source-traversal/driver.lisp', HERE.parent/'target-descriptions/descriptions.lisp',
                      HERE.parent/'target-descriptions/driver.lisp', HERE/'driver.lisp'):
                argv += ['--load', str(p)]
            argv += ['--eval', '(progn (ccl-census-macros::run) (ccl:quit))']
            ...
            try:
                ...
                c, t = (json.loads(captured[k]) for k in ('macros', 'traversal'))
        ...
        save(output/'controls.json', controls(normal, traversal))
        for p, expected in read(HERE/'source-inputs.json').items():
        ...
        save(output/'summary.json', dict(version=1, status='SOURCE_MACRO_SLICE_REPRODUCED',
             review_disposition='NOT_REVIEWED', **counts, native_controls_rejected=len(NATIVE_CONTROLS),
             checker_controls_rejected=20, native_reproduction_equal=True, sessions=sessions,
        ...
        print('PASS: 14 rebuilt macros; 7 used in traversal; 4 native and 20 checker controls rejected; '
              '7 definitions captured, 5 stops remain.')
