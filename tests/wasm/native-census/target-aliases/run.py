#!/usr/bin/env python3
"""Witness DEFTYPE expansion before target translation in disposable U1."""
import argparse
...
import sys
import tarfile
from analysis import HERE, ROOT, CONTROLS, check, controls, read, require, traversal_equivalence
...
def sources():
    ...
    for p in [HERE/n for n in ('driver.lisp', 'aliases.lisp', 'analysis.py', 'run.py')] + [
        HERE.parent/'target-predicates/driver.lisp', HERE.parent/'target-predicates/analysis.py', HERE.parent/'source-expanders/selection.lisp', HERE.parent/'target-helpers/driver.lisp', HERE.parent/'target-helpers/analysis.py', HERE.parent/'target-types/driver.lisp', HERE.parent/'target-types/analysis.py', ROOT/'compiler/X86/X8632/x8632-arch.lisp',
        ROOT/'level-1/l1-typesys.lisp', ROOT/'xdump/xfasload.lisp', HERE.parent/'stub-backend/payload/census-arch.lisp']:
...
def run(store, work, out):
    ...
    try:
        ...
        for path in ('compiler/X86/X8632/x8632-arch.lisp', 'xdump/xfasload.lisp', 'level-1/l1-typesys.lisp'):
        ...
        for mode in ('normal', 'repeat', *CONTROLS):
            env = dict(PATH='/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin', LANG='C', LC_ALL='C',
                       ...
                       CCL_TRAVERSE_OUTPUT=str(work/(mode+'-traversal.json')), CCL_ALIAS_SOURCE=str(HERE/'aliases.lisp'), CCL_ALIAS_OUTPUT=str(work/(mode+'-aliases.json')),
                       CCL_ALIAS_MODE='normal' if mode == 'repeat' else mode, CCL_TRAVERSE_MODE='normal', CCL_DESCRIPTION_MODE='normal')
            ...
            for p in (HERE.parent/'source-traversal/driver.lisp', HERE.parent/'target-descriptions/descriptions.lisp',
                      ...
                      HERE.parent/'source-expanders/driver.lisp', HERE.parent/'source-expanders/selection.lisp', HERE.parent/'target-helpers/driver.lisp', HERE.parent/'target-types/driver.lisp', HERE.parent/'target-predicates/driver.lisp', HERE/'driver.lisp'):
                argv += ['--load', str(p)]
            argv += ['--eval', '(progn (ccl-target-aliases::run) (ccl:quit))']
            ...
            c, t = (json.loads(captured[k]) for k in ('aliases', 'traversal'))
            if mode in ('normal', 'repeat'):
                ...
                # The prior builder shifts observer IDs; require a bijective identity
                # renaming with all other traversal content exactly preserved.
                traversal_equivalence(t,traversal)
                try: check(c, t)
        ...
        save(out/'summary.json', dict(version=1, status='BOUNDED_ALIAS_ORDER_QUALIFIED', review_disposition='NOT_REVIEWED',
             **counts, native_controls_rejected=len(CONTROLS), checker_controls_rejected=len(checks),
             source_reproduction_byte_identical=True, inherited_file_traversal_equivalent_under_identity_renaming=True,
             source_unchanged=True, fasls_unchanged=True, gate_credit=False, sessions=outcomes))
        for kind in ('aliases', 'traversal'): (out/('repeat-'+kind+'.json.gz')).unlink()
        ...
        print('PASS: 16 alias constant probes, 5 optimizer probes, 2 bounded refusals; 5 native and 20 checker controls reject. No gate credit.')
...
def verify(packet):
    ...
    c, t = (read(packet/('normal-'+kind+'.json.gz')) for kind in ('aliases', 'traversal'))
    ...
    for mode, reason in CONTROLS.items():
        changed, changed_t = (read(packet/(mode+'-'+k+'.json.gz')) for k in ('aliases', 'traversal'))
        traversal_equivalence(changed_t,t)
        try: check(changed, changed_t)
        except ValueError as e: require(str(e) == reason, 'NATIVE_REASON '+mode)
        else: raise ValueError('NATIVE_CONTROL_ESCAPED '+mode)
    print('PASS: retained alias order, 5 native and 20 checker controls, direct sources and traversal identity equivalence.')
