#!/usr/bin/env python3
"""Witness the U1 architecture macro lookup chain in disposable U1."""
import argparse
...
NATIVE_SOURCES = ('compiler/arch.lisp', 'compiler/X86/X8632/x8632-arch.lisp',
                  'compiler/X86/X8664/x8664-arch.lisp', 'level-1/l1-typesys.lisp')
LOADS = ('source-traversal/driver.lisp', 'target-descriptions/descriptions.lisp',
    ...
    'target-predicates/driver.lisp', 'target-aliases/driver.lisp', 'target-dispatch/driver.lisp')
    ...
    try:
        ...
        for mode in ('normal', 'repeat', *CONTROLS):
            env = dict(PATH='/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/bin', LANG='C', LC_ALL='C',
                       ...
                       CCL_TRAVERSE_OUTPUT=str(work/(mode+'-traversal.json')), CCL_DISPATCH_OUTPUT=str(work/(mode+'-dispatch.json')),
                       CCL_DISPATCH_MODE='normal' if mode == 'repeat' else mode, CCL_TRAVERSE_MODE='normal', CCL_DESCRIPTION_MODE='normal')
            ...
            argv += ['--eval', '(progn (ccl-target-dispatch::run) (ccl:quit))']
            ...
            with (out/(mode+'.log')).open('wb') as log:
                ...
                try: command['exit_code'] = child.wait(timeout=60)
                    ...
                    child.wait(); raise
            captured = {}
            for kind in ('dispatch', 'traversal'):
            ...
            c, t = (json.loads(captured[k]) for k in ('dispatch', 'traversal'))
        ...
        save(out/'summary.json', dict(version=1, status='BOUNDED_ARCHITECTURE_DISPATCH_QUALIFIED', review_disposition='NOT_REVIEWED',
        ...
        for kind in ('dispatch', 'traversal'): (out/('repeat-'+kind+'.json.gz')).unlink()
        ...
        print('PASS: 4 source functions, 2 real dispatch events, 9 registry probes/native comparisons; 4 native and 25 checker controls reject. No gate credit.')
...
def verify(packet):
    ...
    c, t = (read(packet/('normal-'+kind+'.json.gz')) for kind in ('dispatch', 'traversal'))
    ...
    for mode, reason in CONTROLS.items():
        changed, changed_t = (read(packet/(mode+'-'+k+'.json.gz')) for k in ('dispatch', 'traversal'))
    ...
    print('PASS: retained source/lexical/registry joins, 9 native reference comparisons, 4 native and 25 checker controls, direct pins.')
