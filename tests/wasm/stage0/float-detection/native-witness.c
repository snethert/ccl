/* Native x86-64 SSE witness for the floating-point oracle on the macOS reference
   ...
   instruction (or a comisd comparison, whose result is 1.0 or 0.0) through volatile operands, and prints "op a b result flags" with
...
int main(void) {
  ...
  while (scanf("%7s %llx %llx", op, &a, &b) == 3) {
    ...
    else if (!strcmp(op, "cmp")) r = (double) _mm_comilt_sd(_mm_set_sd(x), _mm_set_sd(y));   /* comisd: invalid for any NaN operand */
    else return 2;
