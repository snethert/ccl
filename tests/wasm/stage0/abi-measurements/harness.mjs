...
class Actor {
  ...
  async configure() {
    const done=this.next(m=>m.kind==='configured');this.worker.postMessage({kind:'configure',memory:this.owner.memory,plan:this.owner.map.workers[this.id],control:new SharedArrayBuffer(4)});this.configuration=await done;
  }
...
export class ABIHarness {
  ...
  async reset() {
    ...
    const publication=allocate('prebuilt-code-publication',64);
    for(const w of map.workers) {
      w.publication=publication;w.schedule=allocate(`measurement-inputs-${w.id}`,8192);
  ...
  async run(anchor,count,{id=0,path=0,nested=false,nonlocal=false,collect=true,published=false}={}) {
    const a=this.actors[id],w=this.map.workers[id];this.setField(id,'mode',nonlocal?2:nested?1:0);
    a.command(published?'published-start':'start',[anchor,count,path]);let outer=null;
    for(;;) {
