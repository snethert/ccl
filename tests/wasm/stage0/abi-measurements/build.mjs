...
import {driverWat, bundleWat, registryWat, scheduleWat} from './emit.mjs';
export const here=path.dirname(fileURLToPath(import.meta.url));
export const repo=path.resolve(here,'../../../..');
export const sha=b=>createHash('sha256').update(b).digest('hex');
export const save=(p,x)=>fs.writeFileSync(p,JSON.stringify(x,null,2)+'\n');
export function build(out,prerequisite,{mutant={},candidate,packaging="cross-instance"}={}) {
  ...
  save(path.join(out,'build-configuration.json'),{candidate:candidate||'all',packaging,mutant,prerequisites:Object.fromEntries(['kernel.o','boundary.o','emitted.wasm'].map(p=>[p,sha(fs.readFileSync(path.join(prerequisite,p)))]))});
  const log=[];
  ...
  fs.cpSync(path.join(here,'../dynamic-call'),path.join(out,'source/dynamic-call'),{recursive:true});
  fs.cpSync(here,path.join(out,'source/abi-measurements'),{recursive:true});
  ...
  command(clang,['--target=wasm32-unknown-unknown','-O1','-ffreestanding','-fno-builtin','-fno-stack-protector','-matomics','-mbulk-memory',
    '-c',path.join(out,'source/abi-measurements/measure.c'),'-o',path.join(out,'measure.o')]);
  command(ld,['--no-entry','--import-memory','--import-table','--shared-memory','--initial-memory=524288','--max-memory=1048576',
    ...
    '--Map='+path.join(out,'kernel.map'),path.join(prerequisite,'boundary.o'),path.join(prerequisite,'kernel.o'),path.join(out,'abi.o'),path.join(out,'measure.o'),'-o',path.join(out,'kernel.wasm')]);
  ...
  for(const [name,k] of Object.entries(schema.candidates)) {
    ...
    const registry=emit(name+'/registry',registryWat(slots));
    const schedule=emit(name+'/schedule',scheduleWat());
    const driver=emit(name+'/driver',packaging==='cross-instance'
      ? driverWat(runtime,schema,k,slots,packaging,mutant)
      : bundleWat(runtime,schema,k,slots,packaging,mutant));
    if(packaging!=='cross-instance') {
      const source=fs.readFileSync(path.join(out,name,'driver.wat'),'utf8');
      source.split('\n').forEach((line,i)=>{
        const m=line.match(/@site ([0-9,]+) (.+)$/);if(!m)return;
        for(const id of m[1].split(','))debug.descriptors[Math.floor(Number(id)/100)].source_sites[Number(id)]={path:name+'/driver.wat',sha256:sha(Buffer.from(source)),line:i+2,description:m[2]};
      });
      save(path.join(out,name,'debug-metadata.json'),debug);
    }
    candidates[name]={k,support,stub,modules,wrong,debug,registry,schedule,driver,driverSha256:sha(driver),packaging};
  }
