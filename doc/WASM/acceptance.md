...
Required regression. Bind every required initializer to its prerequisite state and completion result; seed the loader's own dependencies before loading later bundles. By the user's 15 September 2026 decision, Stage 0 LL15-b/c qualify the retained native census instrument, reviewed seed recipe, transitive startup worklist with named unknowns, identity-bound queries and scenario-specific call-site witnesses under contracts/census.md v0.2. Independent omission, identity, scope-promotion and witness mutants must fail. Exhaustively bounding every native computed call is no longer a Stage 0 prerequisite; observed targets remain witnesses rather than bounds. The complete-closure checker remains strict and the working graph remains blocked when required edges are unresolved. Stage 1 must implement and test the selected bootstrap dependencies, compile/load-time effects and primitive-to-Lisp cycles, with correct initializer order and completion. A no-load build path does not prove loading; required libraries are not deferred because their breadth stage is later.
