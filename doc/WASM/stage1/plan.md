...
The [native reset effects](../../../tests/wasm/stage1/startup-resets/README.md) now execute thirteen selected literal-reset callbacks from the retained startup inventory through generated global-cell writes, with actual pristine-U1 callbacks as oracle. All 35 snapshot rows remain explicit; 22 stay open, and this is not the whole emitted-initializer population. The accompanying private-catalog adapter binds the actual installed digest to the schedule. Auxiliary, not reviewed or integrated, no LL15 credit. Next remains the selected host-service/nonliteral callback and definition effects, ordinary-condition activation, and their concrete startup dependency/implementation joins before LL15 closure.
