...
D3 uses [B for implementation](stage0/abi-choice.md): all arguments on the explicit stack, chosen for simplicity and clarity. The user directed the project to stop ranking roughly equal exploratory timings; comparative timing and proposed sensitivity/instrumentation work are deferred. Claude reviewed the measurement derivative without finding a defect, and the user accepted S0-LL21-a for identity/generation publication with module bytes already held by Workers. The gate has 26 accepted, 23 missing and zero unreviewed records. The next work is a qualified census and a small generated B pass-2 slice; shared compiler work still needs an authorized author.