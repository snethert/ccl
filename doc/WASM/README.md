...
- [IDE design goals](ui-overview.md): the CLIM-based IDE and its reference screens, stated as checkable goals against the port's constraints. Draft; not a Stage 1 obligation.
...
D3 uses [B for implementation](stage0/abi-choice.md): all arguments on the explicit stack, chosen for simplicity and clarity. The user directed the project to stop ranking roughly equal exploratory timings; comparative timing and proposed comparison instrumentation are deferred. Claude reviewed the measurement derivative without finding a defect, and S0-LL21-a is accepted by the user's own statement in the reviewer's session, superseding a withdrawn misattributed decision. The gate has 27 accepted, 22 missing and zero unreviewed records. [Reversible native observation](stage0/native-census.md) has now executed under the separate startup-instrumentation authorization. The [dependency extension](stage0/native-dependencies.md) resolves 1,918 unnamed calls and records L0/L1/L2 source and sampled binding histories. The external Terminal trace is captured, with four loader pathname contexts still unresolved. Complete census closure and generated B code remain next; functional shared compiler work still needs an authorized author. The implementation starts from pristine U1.
