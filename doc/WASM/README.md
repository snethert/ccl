...
D3 now has an [executed exploratory measurement fixture](stage0/abi-measurements.md) and an explicitly identified S0-LL21-a publication case. The derivative passes its correctness and rejection corpus across all three ABIs and packaging choices; Claude's fifth audit reviewed and reproduced it with no defect found, and S0-LL21-a awaits the project's acceptance decision. Raw baseline-tier timings are retained, but representative workloads, production startup/scale, resource budgets and ABI selection remain open. The gate has 23 missing records and one unreviewed record; the 25 earlier scoped acceptances remain unchanged.
