...
Status: initial implementation contract, exercised by the [isolated S0-LL23-b fixture](../../../tests/wasm/stage0/debug-frames/README.md); the r3 hand-built mechanism has received Claude’s independent review with no defect found; not accepted or ABI-frozen. This specifies the information the Wasm compiler/runtime must preserve before D3 freezes. S0-LL23-b proves a hand-built slice; Stage 1 repeats with generated frames under LL23, and Stage 3 exercises the real debugger under R2/LL19/LL20.
...
## Executed hand-built scope

The fixture exercises the header and restoration protocol using the reviewed cons-only moving runtime. It covers admitted self-inspection and serialized host replies, with a separate literal-offset byte oracle. Source sites resolve to hashed emitted WAT; lexical maps distinguish shadowed bindings and explicit unavailable reasons. The shared cell is also retained by the Worker’s permanent root frame, so its survival demonstrates relocation and inspection rather than capture or escape semantics. The old/new entries remain hand-built surrogates for redefinition. Generated closure capture and escape remain required later. Tail-frame elimination, arbitrary target inspection, generated frames, saved-image reconstruction and ABI publication costs remain unproved. See the [execution record](../stage0/debug-frames.md) for counts, limits and review disposition.
