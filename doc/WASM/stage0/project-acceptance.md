...
## Fifth-audit disposition corrected — S0-LL21-a, 12 September 2026

S0-LL21-a is externally reviewed and awaiting project acceptance. Codex previously interpreted the pasted sentence “I accept S0-LL21-a” as a user decision. The user explicitly clarified, “no, that was all claude,” and has not subsequently accepted it. The unsupported acceptance is withdrawn. The original mistaken decision, envelope and aggregates remain retained as historical evidence; they must not be used as current authorization.

The correction changes only `review_disposition` and `review_record` on LL21-a in the latest aggregate. All execution fields, timestamps, contract bindings and artifact references remain identical, and all 26 other result objects—including the subsequently accepted S0-LL08-b record—are unchanged. The [index](../evidence/index.json) identifies the correction decision, preservation check and corrected aggregate. The gate has 26 accepted records, 22 missing and one it labels “unreviewed”; for LL21-a that means project acceptance is pending, not that Claude's fifth audit is missing.

The reviewed scope remains identity/digest/generation publication with bytes already held in each Worker and host-actor acquisition and installation, at the retained hand-built bounds. This correction does not withdraw Claude's no-defect audit or change the user's separate [B engineering decision](abi-choice.md). No benchmark-selection or performance claim is added.
