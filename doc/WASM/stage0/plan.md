...
The logical-frame fixture and its v2 evidence tooling have been independently reviewed by Claude and accepted within the [seven-record decision](project-acceptance.md). The [C/C4/B corpus](dynamic-call.md) now supplies bounded hand-built correctness executions and controls. Next: independent review of that new implementation, then freeze and execute measurements with explicit module-granularity, startup and Worker assumptions. Native census instrumentation and a real pass-2 spike require an author authorized to modify shared compiler source under the standing rules.
