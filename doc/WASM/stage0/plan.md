...
1. Complete the startup census under S0-LL15-b/c. The [joined projection](joined-census.md) now preserves the retained input identities, effects and evaluated metadata in the exchange format, with specific omission controls and the reviewed dyld classification. The [richer collector](rich-census.md) now supplies actual expander, installation, materialization and initializer identities, native instruction operands and inspection of the exact traced image. Join those records into the exchange graph, account for the boot-image subprocess separately, classify lowering/import/store operands, and review the seed set and conservative call bounds. Do not infer missing observations from printed previews or treat image construction as initializer execution.
