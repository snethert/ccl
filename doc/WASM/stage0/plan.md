...
| 0E — ABI selection | 0C/0D correctness prerequisites | C/C4/B correctness, then their measurements. Contract includes arguments, result-region lifetime, frames, cleanup and debugger policy. Census distributions refine workload weights before selection measurements are frozen. |
...
5. Implement all generic ABI candidates through one parameterized test corpus. Select no ABI from the initial probes. Freeze workload weights and the benchmark policy digest before selection measurements, then compare C/C4/B with the declared module-granularity assumptions.
...
The logical-frame fixture and its v2 evidence tooling have been independently reviewed by Claude and accepted within the [seven-record decision](project-acceptance.md). The [C/C4/B corpus](dynamic-call.md) now supplies bounded hand-built correctness executions and controls. The eighteen ABI records are now accepted after Claude's fourth audit. The isolated [measurement fixture](abi-measurements.md) now executes all three packaging choices and supplies a fresh S0-LL21-a record. Next: obtain independent review, extend missing resource/phase-latency instrumentation and product-risk experiments, and use the qualified census and product budgets to freeze a representative matrix before selection measurements. Native census instrumentation and a real pass-2 spike require an author authorized to modify shared compiler source under the standing rules.
