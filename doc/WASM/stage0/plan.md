...
The logical-frame fixture and its v2 evidence tooling have been independently reviewed by Claude and accepted within the [seven-record decision](project-acceptance.md). The [C/C4/B corpus](dynamic-call.md) now supplies bounded hand-built correctness executions and controls. The eighteen ABI records are now accepted after Claude's fourth audit. Next: build the isolated measurement harness under the [handoff](abi-measurements.md), supply the separately required S0-LL21-a record, and freeze the representative matrix before selection measurements. Native census instrumentation and a real pass-2 spike require an author authorized to modify shared compiler source under the standing rules.
