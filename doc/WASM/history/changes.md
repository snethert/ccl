...
## 2026-09-12 — reversible native observation from pristine U1

The separate standing-rule exception in 37f2b8da authorizes reversible startup/compiler observation before functional Wasm backend edits. The user reiterates reversibility and a clean CCL implementation start. Codex adds an isolated native harness that applies one hashed three-file observation patch only to disposable source archives, then restores original sources and clean outputs. The main compiler/kernel tree is unchanged. Observed images never become implementation inputs.

Native r5 runs the pinned 21,843 eligible tests in both baseline and observed builds, compares evaluated operator IDs/flags/reserved slots, observes compilation and all 35 registered startup callbacks, removes the patch and reproduces all 164 clean FASLs. Its S0-LL08-b envelope is NOT_REVIEWED. The prior 26 accepted records keep their original fields and identities; the new aggregate changes only relative artifact/snapshot/review paths to share the evidence repository root instead of copying old packs again. Stage 0 remains BLOCKED: 22 missing and one unreviewed.

Original development failures remain immutable. An early patch reload required the upstream rebuild redefinition bindings. Formal r1 used mismatched compiler/gensym contexts and failed native-byte comparison. R2 declared common seeds but injected three DEFVAR initializers, consuming gensyms and shifting later generated symbol names. R3 removed those initializers and passed both native suites and the 161/164 unchanged-FASL comparison, then failed cold startup because a private observer stream opened in the Initial process could not be used by the listener. A shared stream plus complete-record lock fixed that defect, reproduced directly and in successful r4. Final inspection found r4 recovery omitted the intermediate boot image; r5 retains and restores it with the other 166 outputs and adds actual cleanup controls. No failed run is relabeled passing. R4 is successful execution with superseded recovery coverage; only r5 supports the final recovery claim.

Source reversal, output recovery, native omission/corruption and evidence-producer controls pass. Final inspection strengthened the partial-recovery control to start an actual separate Python process; the earlier same-process reconstruction and its imprecise label remain preserved, with a separate passing control record. Native disassemblies identify every patched function for independent review; unchanged inputs use explicit common compiler seeds with no normalization. Actual external tracing remains UNAVAILABLE because fs_usage requires administrator access and sudo -n cannot authenticate. Conservative closure, full lowering/initializer joins, trace reconciliation and independent review remain open; no LL15-b/c or generated-Wasm claim is made. ABI comparison timing remains deferred under the B project decision.

## 2026-09-12 — accept S0-LL21-a and choose B for implementation

The user accepts S0-LL21-a following Claude's fifth audit committed as 5ff0dc54. A separate envelope retains the original r2 execution and exact review, scoped to identity/digest/generation publication with bytes already held by each Worker and host-actor acquisition/rejection. The prior 25 accepted result objects are preserved exactly: 26 accepted, 23 missing, zero unreviewed. No fixture, compiler or kernel source changes or runtime re-execution are claimed.

The user then directs the project to stop spending effort ranking near-equal timings and choose on speed or simplicity. B is selected for its simpler all-stack argument rule and reviewed correctness, without claiming a measured speed advantage. Benchmark policy v3 replaces the mandatory comparison with this engineering choice; all numerical comparison parameters and dedicated-host deadlines remain unchanged. The 24 comparison rows become an optional deferred worklist, with none promoted to passing. Only the unexecuted S0-ABI-selection contract changes; all 37 other test contracts remain compatible. Original policy/inventory hashes and the final change rationale are retained in the evidence repository.

The initially proposed sensitivity/instrumentation assignment is superseded and deferred. It is needed only if a concrete generated-code problem justifies reopening a performance comparison. The next work is the qualified census and a small generated B pass-2 slice. Shared compiler work still needs an authorized author; none is assigned by this decision. Stage 0 correctness, generated confirmation and product risks remain open.
...
## 2026-09-12 — isolated D3 measurements and S0-LL21-a

Implemented an isolated derivative of the reviewed ABI fixture. Fresh r2 executes all C/C4/B and three packaging configurations: 486 positive correctness cases, 279 rejected controls, 72 positive batches and 27 rejected batch controls, plus fresh unchanged prerequisites. An explicit prebuilt publication case calls new code from existing and late Workers while retaining the old function object. Physical scanner assertions, a real private table-role registry and an accurate circular-APPLY capacity label address the fourth-audit observations.

Retained three paired exploratory trials per packaging with a pinned Liftoff-only tier, full input cycles, complete graph observations, raw first/warm/sample timings, compilation/resources and paired bootstrap reports. The unchanged policy is not satisfied by these exploratory counts/durations and missing product evidence; NO_SELECTION is explicit. Original development build/launch failures and the full r1 prototype are retained. R2 fixes partial-cycle tail sampling and cumulative workload counters without changing emitted bytes.

The new LL21-a envelope remains unreviewed. Twenty-five earlier scoped acceptances are preserved, and the gate remains BLOCKED for 23 missing and one unreviewed record. Reviewed fixtures, shared compiler and upstream kernel sources remain unchanged. Independent review, missing instrumentation, representative census/startup/scale experiments, engine/contracts and product budgets precede selection. H(G) remains optional.

## 2026-09-12 — record the fifth audit of the measurement derivative

Claude's fifth audit, in its own session at HEAD 023181c0, reviewed the complete `abi-measurements` derivative and its source delta against the reviewed fixture, re-executed it with the documented engine flags (486/279/72/27, 27 trial rows, exit 0), matched all 4,518 Wasm modules and objects and the 864 reviewed prerequisite binaries byte-for-byte against retained r2, verified the r1/r2 build-byte claim, and recomputed every paired ratio and bootstrap interval independently from the raw trials. The fdebe7b3 acceptance operation was checked separately. No defect was found. Five nonblocking observations are recorded: the publication carries code identity rather than bytes; the exploratory timer's fixed per-call cost places all candidates inside the 5 % band, so the selection harness needs a positive sensitivity control before the frozen rule is applied; the direct packaging dispatches through a comparison chain; bundled packagings validate bytes that are not the executed bundle; and the two publication controls are host-side assertions. The index records the review on ABI-MEASUREMENTS-r2 without changing its original envelope. Stage 0 remains BLOCKED for 23 missing records and one unreviewed record; acceptance of S0-LL21-a is the project's decision.

## 2026-09-12 — decisions: D3 selects B; reversible startup instrumentation permitted

The user selected candidate B for D3 on speed and simplicity, cancelling the remaining selection trials. This is a project decision with a dated rationale, not an outcome of the frozen `benchmarks.json` rule, whose trial and duration minima were never met; the exploratory r2 timings remain retained as exploratory only. The user also directed that startup information be gathered before any Wasm backend work alters CCL, and amended the standing rules to permit reversible startup instrumentation of shared compiler source as an exception, recorded in `CLAUDE.md`. Decisions v1.8 and the D3 section still describe D3 as open; updating that projection is Codex's next document step.
