...
14 September 2026 clarification following the native-internals review: retain D1, B, D4 and D5. The hand-built copying collector establishes its recorded mechanisms; it does not select the production collector algorithm or qualify all native object semantics. The initial non-generational collector still owes moved-key/hash-cache correctness and separate dispositions for weak objects and GCTWA. The non-generational choice justifies omitting classified generational bookkeeping, not other store semantics. B does not eliminate roots outside the argument stack, and cooperative suspension requires complete publication at every collecting transition. The [work plan](stage0/plan.md) schedules these existing census and generated-bootstrap obligations; no earlier evidence scope or acceptance disposition changes.
