...
- Contradiction source docs are now reconciled to replacement-track posture across `C-01`..`C-14`; closure synchronization is complete.
...
- Startup-gate executor wiring and structured diagnostics enforcement are now implemented for loader/smoke entrypoints `LHI-01`/`LHI-02`/`LHI-03`, including strict pass/fail parsing over `STARTUP_GATE_CHECK` + `STARTUP_GATE_SUMMARY` records and deterministic fail-injection assertions (`SRG-01`/`SRG-07`/`SRG-11` + malformed-summary `RPL01-E011` semantics).
- `LHI-04` delegated all-smoke failure-lane behavior and `LHI-05` wasm-ui-persist startup-gate wiring remain stable, and `LHI-06` browser-lane execution evidence is now captured with deterministic pass output (`npm --prefix web-ui run test:browser`).
- Contradiction source-doc rewrites for `C-01`/`C-03`/`C-04`/`C-08` are now landed and tracked as completed queue tasks (`RPL01-CF-01`..`RPL01-CF-04`).
- ADR-0006 contradiction rewrite (`C-02`) is now landed and tracked as completed task `RPL01-CF-05`.
- Project-overview contradiction rewrite (`C-05`) is now landed and tracked as completed task `RPL01-CF-06`.
- Thread-model contradiction rewrite (`C-06`) is now landed and tracked as completed task `RPL01-CF-07`.
- Yield/resume contradiction rewrite (`C-07`) is now landed and tracked as completed task `RPL01-CF-08`.
- UI bridge transport contradiction rewrite (`C-09`) is now landed and tracked as completed task `RPL01-CF-09`.
- Persistence-service contradiction rewrite (`C-10`) is now landed and tracked as completed task `RPL01-CF-10`.
- Unattended-plan override contradiction rewrite (`C-11`) is now landed and tracked as completed task `RPL01-CF-11`.
- Porting-status lane-split contradiction rewrite (`C-12`) is now landed and tracked as completed task `RPL01-CF-12`.
- Roadmap sequencing contradiction rewrite (`C-13`) is now landed and tracked as completed task `RPL01-CF-13`.
- Front-end phase-plan supersession contradiction rewrite (`C-14`) is now landed and tracked as completed task `RPL01-CF-14`.

## Immediate Next Step

- Action: keep `RPL-01` startup-gate outputs frozen as a closed baseline while `RPL-06` Step 1 begins replacement-lane sync/merge contract drafting.
- Why now: contradiction follow-through (`RPL01-CF-01`..`RPL01-CF-15`) and integration follow-through (`RPL01-IG-01`/`RPL01-IG-02`) are complete.
- Success evidence: runtime master and ticket board keep `RPL-01` at `done`, and downstream work references frozen `SRG-*`, `RPL01-E*`, and `LHI-*` IDs without mutation.

## Contradiction Follow-Through Task Queue

| Task ID | Contradiction ID | Scope | Status | Closure evidence |
| --- | --- | --- | --- | --- |
| RPL01-CF-01 | C-01 | Update ADR-0001 in `doc/wasm/decisions.md` to make shared-memory transport the normative hot-path baseline; keep copy/message lanes bootstrap/control/diagnostics-only. | done | `doc/wasm/decisions.md` now establishes shared-memory hot-path baseline and limits copy/message lanes to compatibility scopes. |
| RPL01-CF-02 | C-03 | Replace copy-first ABI wording in `doc/wasm/kernel-request-abi.md` with ABI-v2 shared-channel primary transport + no-fallback mismatch behavior. | done | `doc/wasm/kernel-request-abi.md` now defines shared-channel-first hot-path transport and repositions copy responses as compatibility-only lanes. |
| RPL01-CF-03 | C-04 | Rewrite `doc/wasm/js-microkernel-spec.md` execution-mode/startup-failure text to secure-only required-capability posture with explicit hard-fail behavior. | done | `doc/wasm/js-microkernel-spec.md` now defines secure-only startup contract and explicit abort-on-missing-capability behavior with no silent fallback. |
| RPL01-CF-04 | C-08 | Align `doc/wasm/interrupts.md` interrupt and signaling assumptions to secure-only shared-memory runtime posture. | done | `doc/wasm/interrupts.md` now defines secure runtime worker interrupt posture and required shared-memory capability alignment. |
| RPL01-CF-05 | C-02 | Rewrite ADR-0006 in `doc/wasm/decisions.md` to secure-only startup requirements with explicit runtime-vs-CL thread semantics boundary. | done | ADR-0006 now states required runtime worker/thread capability with deferred CL-thread semantics boundary aligned to startup gates. |
| RPL01-CF-06 | C-05 | Rewrite `doc/wasm/project-overview.md` to secure-environment performance-first MVP posture and move portability ladder wording to explicitly non-MVP legacy context. | done | `project-overview.md` now defines secure-only replacement MVP posture and labels portability ladder content as non-MVP legacy context. |
| RPL01-CF-07 | C-06 | Rewrite `doc/wasm/threads.md` to required startup threading/worker-topology assumptions for replacement MVP runtime architecture. | done | `threads.md` now defines secure-only replacement startup threading/worker topology requirements aligned to `SRG-03`..`SRG-05` and `SRG-12`, with no single-runner replacement fallback lane. |
| RPL01-CF-08 | C-07 | Re-scope `doc/wasm/yield-resume.md` to secure capability prerequisites and non-secure unsupported replacement-lane posture. | done | `yield-resume.md` now assumes secure worker/shared-memory prerequisites and marks non-secure portability mode as legacy/non-normative for replacement startup. |
| RPL01-CF-09 | C-09 | Rewrite `doc/wasm/ui-bridge-protocol.md` to shared-memory-first transport mapping for required UI bridge hot-path classes. | done | `ui-bridge-protocol.md` now defines shared-channel-first hot-path routing in replacement lanes with copy/message transport restricted to control/legacy scopes. |
| RPL01-CF-10 | C-10 | Mark `doc/wasm/persistence-service-spec.md` legacy for replacement lane and anchor replacement persistence to Storage V2 local-core contracts. | done | `persistence-service-spec.md` now labels memory-snapshot service as legacy lane and points replacement requirements to `RPL-05` + `SRG-06/07/10/11`. |
| RPL01-CF-11 | C-11 | Add explicit replacement-track override in `doc/wasm/mvp-unattended-execution-plan.md` and migration path references to secure-gating/storage contracts. | done | Unattended plan now labels memory-snapshot flow as legacy-lane execution and references `RPL-01`/`RPL-05` as replacement authority. |
| RPL01-CF-12 | C-12 | Split `doc/wasm/porting-status.md` into legacy-lane vs replacement-lane posture to prevent memory-snapshot default from appearing as future architecture target. | done | `porting-status.md` now includes normative lane split and tags memory-snapshot defaults as legacy-only status context. |
| RPL01-CF-13 | C-13 | Update `doc/wasm/roadmap.md` sequencing to secure runtime gating/shared-memory prerequisites instead of deferred single-thread baseline framing. | done | `roadmap.md` now defines replacement prerequisites as secure startup + shared-memory required posture with explicit no-fallback trajectory. |
| RPL01-CF-14 | C-14 | Add replacement-track supersession/dependency notes in `web-ide/phase-8/implementation-plan.md` before release execution gates. | done | Phase 8 plan now declares runtime replacement milestone dependencies and labels memory-snapshot posture as legacy-lane context. |
| RPL01-CF-15 | C-ALL | Publish contradiction-closure synchronization across runtime/backend governance docs and program board references. | done | Master/governance/matrix/board docs now reflect `C-01`..`C-14` closure and completed queue state without reopening frozen cutover artifacts. |
...
| --- | --- | --- | --- | --- | --- |
| C-01 | `doc/wasm/decisions.md:7` | ADR-0001 makes copy-based `kernel_request` responses the required baseline. | Supersede ADR language so shared-memory IPC is normative for hot paths; keep copy/message path only for bootstrap/control/diagnostics. | done | ADR-0001 now states shared-memory hot-path baseline and compatibility-only copy/message lanes aligned to `SRG-08` plus `IPCP-*`/`IPCV-*` transport contracts. |
| C-02 | `doc/wasm/decisions.md:52` | ADR-0006 defines single-runner portable baseline and deferred shared-heap threading. | Replace with secure-only MVP runtime stance: required worker/thread posture for startup, while CL thread semantics may still be deferred. | done | ADR-0006 now records secure-only runtime worker baseline and explicit runtime-vs-CL thread semantics boundary aligned to `SRG-01`..`SRG-05` and `SRG-12`. |
| C-03 | `doc/wasm/kernel-request-abi.md:3` | ABI spec is explicitly copy-based MVP with zero-copy as optional future extension. | Define ABI v2 transport contract where shared-memory channels are primary data-plane/hot-path mechanism. | done | ABI contract text now defines shared-memory hot-path primacy and compatibility-only copy-response behavior with startup-gate violation semantics for mismatches. |
| C-04 | `doc/wasm/js-microkernel-spec.md:14` | Microkernel goals and requirements retain single-thread compatibility/degrade-to-baseline behavior. | Rewrite execution modes to secure-only MVP with required capabilities and explicit unsupported-startup failure contract. | done | Microkernel spec now states secure-only startup contract, required capabilities, and deterministic hard-fail behavior with no fallback runtime lane. |
| C-05 | `doc/wasm/project-overview.md:69` | Overview frames a portability ladder with message-passing/sandbox baseline and optional shared memory. | Reframe as secure-environment performance-first MVP; move portability/embeddable discussion to future non-MVP notes only. | done | `project-overview.md` now expresses secure-only replacement MVP posture with portability notes explicitly marked as legacy/non-MVP context. |
| C-06 | `doc/wasm/threads.md:1` | Threading doc marks shared-heap mode optional and single-runner as baseline target. | Rewrite to required startup threading/worker topology assumptions for MVP runtime architecture. | done | `threads.md` now states secure-only replacement startup contract with required worker topology/thread capability and explicit runtime-vs-CL semantics boundary (`SRG-03`..`SRG-05`, `SRG-12`). |
| C-07 | `doc/wasm/yield-resume.md:1` | Yield/resume guidance preserves portable baseline without SAB/Atomics and makes Stage 3 optional. | Re-scope document so secure capability prerequisites are assumed; treat non-secure mode as unsupported in MVP. | done | `yield-resume.md` now assumes secure worker/shared-memory prerequisites, keeps stepping semantics for control-plane integration, and relegates portable non-secure framing to legacy notes only. |
| C-08 | `doc/wasm/interrupts.md:9` | Interrupt model centers portable single-runner baseline and optional shared-memory upgrade. | Align interrupt requirements with secure-only startup and required shared-memory runtime posture. | done | Interrupt spec now defines secure-runtime cooperative model with required shared-memory capability alignment and no-fallback startup posture. |
| C-09 | `doc/wasm/ui-bridge-protocol.md:1` | UI bridge protocol assumes transport over copy-based `kernel_request` ABI. | Define shared-memory transport path for UI ingress/egress hot paths; retain message/copy channel only for non-hot/control paths. | done | UI bridge spec now defines shared-memory-first hot-path routing and control-lane-only compatibility transport aligned to `SRG-08`/`SRG-09` and `R4M-*` class contracts. |
| C-10 | `doc/wasm/persistence-service-spec.md:1` | Persistence spec is memory-first snapshot + optional host backends, not Storage V2 object/ref model. | Mark this spec legacy for replacement track and author Storage V2 normative local-core contract. | done | Persistence service spec now marks memory-snapshot posture as legacy and points replacement-lane normative persistence to `RPL-05` and startup gates (`SRG-06/07/10/11`). |
| C-11 | `doc/wasm/mvp-unattended-execution-plan.md:1` | MVP unattended plan sets `memory-snapshot` as default persistence lane. | Add explicit replacement-track override note and migration path to Storage V2/secure-runtime gating docs. | done | Unattended plan now labels memory-snapshot as legacy-lane checklist and adds explicit replacement authority references (`RPL-01`, `RPL-05`). |
| C-12 | `doc/wasm/porting-status.md:1` | Current status report treats memory-snapshot persistence as default unattended posture. | Split status into current-lane vs replacement-lane and prevent old default from appearing as future architecture target. | done | Porting status now has normative lane-split language and memory-snapshot-default references are explicitly legacy-lane context. |
| C-13 | `doc/wasm/roadmap.md:1` | Roadmap defers concurrency and states single-threaded baseline first. | Update roadmap to replacement sequence where secure runtime gating and shared-memory IPC are prerequisite, not optional late-phase work. | done | Roadmap now frames secure startup/shared-memory prerequisites as replacement-lane requirements and removes deferred-single-runner-target posture. |
| C-14 | `web-ide/phase-8/implementation-plan.md:1` | Phase 8 plan locks memory-snapshot decoupling/defaults and declares runtime architecture rewrite out-of-scope. | Add replacement-track supersession notes and explicit dependency on runtime replacement milestones before front-end release execution. | done | Phase 8 plan now declares runtime replacement milestone dependencies for release gating and labels memory-snapshot work as legacy-lane stabilization context. |
...
- 2026-02-10: Broke contradiction follow-through into ordered tasks (`RPL01-CF-01`..`RPL01-CF-04`) and advanced immediate execution to `RPL01-CF-01` (`C-01` ADR baseline rewrite).
- 2026-02-10: Started `RPL01-CF-01` by rewriting ADR-0001 in `doc/wasm/decisions.md` to shared-memory hot-path baseline language; `C-01` remains `in_progress` pending closure evidence sync.
- 2026-02-10: Completed `RPL01-CF-01`..`RPL01-CF-04` by landing contradiction source-doc rewrites in `decisions.md`, `kernel-request-abi.md`, `js-microkernel-spec.md`, and `interrupts.md`; advanced immediate execution to `RPL01-CF-05` (`C-02`).
- 2026-02-10: Completed `RPL01-CF-05` by rewriting ADR-0006 in `doc/wasm/decisions.md` to secure-only startup wording and advanced immediate execution to `RPL01-CF-06` (`C-05`).
- 2026-02-10: Completed `RPL01-CF-06` by rewriting `project-overview.md` to secure-only replacement MVP posture and advanced immediate execution to `RPL01-CF-07` (`C-06`).
- 2026-02-10: Completed `RPL01-CF-07` by rewriting `threads.md` to required secure-only startup threading/worker-topology posture with explicit runtime-vs-CL semantics boundary language.
- 2026-02-10: Completed `RPL01-CF-08` by rewriting `yield-resume.md` to secure-prerequisite replacement posture and advanced immediate execution to `RPL01-CF-09` (`C-09`).
- 2026-02-10: Completed `RPL01-CF-09` by rewriting `ui-bridge-protocol.md` to shared-memory-first UI hot-path transport posture with control-lane-only compatibility semantics.
- 2026-02-10: Completed `RPL01-CF-10` by marking `persistence-service-spec.md` as legacy-lane scope and anchoring replacement persistence requirements to `RPL-05` + `SRG-06/07/10/11`.
- 2026-02-10: Completed `RPL01-CF-11` by adding explicit replacement-track override language to `mvp-unattended-execution-plan.md`.
- 2026-02-10: Completed `RPL01-CF-12` by adding lane-split posture language in `porting-status.md`.
- 2026-02-10: Completed `RPL01-CF-13` by updating `roadmap.md` to secure runtime/shared-memory prerequisite sequencing.
- 2026-02-10: Completed `RPL01-CF-14` by adding replacement-track dependency/supersession notes to `web-ide/phase-8/implementation-plan.md` and advanced immediate execution to `RPL01-CF-15` closure synchronization.
- 2026-02-10: Started `RPL01-CF-15` by syncing runtime/backend master/governance/matrix/board immediate-next-step wording to contradiction-closure state while preserving immutable `X-08` artifacts.
- 2026-02-10: Completed `RPL01-CF-15` by publishing contradiction-closure synchronization across runtime/backend governance surfaces (`C-01`..`C-14` all `done`) and advanced immediate execution to `RPL01-IG-01` startup-gate integration (`LHI-01`/`LHI-02`/`LHI-03`).
- 2026-02-10: Completed `RPL01-IG-01` by landing startup-gate execution in loader `start-lisp`/`run-toplevel` lanes plus deterministic `STARTUP_GATE_CHECK`/`STARTUP_GATE_SUMMARY` parsing/assertions in noninteractive smoke (including fail-injection and malformed-summary handling), and advanced immediate execution to `RPL01-IG-02` (`LHI-04`/`LHI-05`/`LHI-06`).
- 2026-02-10: Started `RPL01-IG-02` by landing `LHI-05` startup-gate enforcement in `wasm-ui-persist-smoke.mjs`, wiring `LHI-06` browser harness bootstrap gating, and validating delegated/all-smoke + persistence fail lanes; browser execution evidence remains pending where Playwright browsers can launch.
- 2026-02-10: Completed `RPL01-IG-02` by fixing browser harness runtime bootstrap to consume packed v2 UI module bundles (`wasm-ui-modules.json` + `.idx` + `.bin`) and validating deterministic browser lane success via `npm --prefix web-ui run test:browser` (`pass=1`, `fail=0`).
