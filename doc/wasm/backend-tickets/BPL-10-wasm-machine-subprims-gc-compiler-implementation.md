...
- Action: advance from helper-boundary split into hot-math primitive swapout.
- `1.` start `B10C-01A-10` by replacing binary fixnum compat fallback default emission with direct WASM lowering in `compiler/WASM/wasm2.lisp`, keeping explicit overflow/type compatibility fallback edges only.
- `2.` continue strict hot-math order through `B10C-01A-11`..`B10C-01A-17` with per-slice rebuild + strict audit + smoke evidence.
...
- Why now: `B10C-01A-04`/`A-05`, `B10C-01A-08`, and `B10C-01A-09` are now closed with required validation evidence, so hot-lane migration can proceed from explicit direct-vs-compat boundaries.
