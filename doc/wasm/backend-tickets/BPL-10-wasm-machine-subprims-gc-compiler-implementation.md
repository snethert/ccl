...
- Action: advance from compat-lane narrowing into subprim-boundary tightening.
- `1.` start `B10C-01A-15` by constraining `.SPbuiltin-div`, `.SPbuiltin-negate`, and `.SPbuiltin-ash` callsites to explicit compatibility boundaries and removing any remaining default-lane dependence.
- `2.` continue strict hot-math order through `B10C-01A-16`..`B10C-01A-17` with per-slice rebuild + strict audit + smoke evidence.
...
- Why now: `B10C-01A-04`/`A-05`, `B10C-01A-08`, `B10C-01A-09`, `B10C-01A-10`, `B10C-01A-11`, `B10C-01A-12`, `B10C-01A-13`, and `B10C-01A-14` are now closed with required validation evidence, so hot-lane migration can continue into subprim-boundary tightening.
- Success evidence: hot math lanes compile to direct lowering by default, spill envelopes remain constrained to GC-capable fallback edges, compatibility callsites are explicit and bounded, and required validation remains green at each slice.
