...
- Action: continue `B10G-04` with phase 3 by extending non-local-exit coherence checks to `_SPnthrowvalues` and unwind-cleanup (`_SPmkunwind` cleanup-entry) paths, asserting the same `last_lisp_frame`/`save_vsp` boundary invariants with deterministic signatures.
- Why now: phase 2 now covers single-value `nthrow` and funcall boundary exits, so the remaining `B10G-04` correctness risk is multi-value and cleanup-entry unwind boundaries.
- Success evidence: extended `B10G-04` selftest signatures cover mv+cleanup throw/unwind boundaries, `scripts/wasm/arm-retirement-audit.sh --strict` remains `total_hits=0`, and `node doc/wasm/js/all-smoke.mjs` remains green.
