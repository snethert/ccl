...
- Action: hold promotion at `B10C-01A-17` follow-up until measured add-lane regression is reduced.
- `1.` continue `B10C-01A-17` by tightening direct fixnum add/sub hot-lane prologue/epilogue overhead and rerunning the checkpoint artifact (`doc/wasm/tickets/evidence/bpl-10/b10c-01a-17-fixnum-add-checkpoint-2026-02-10.json`) until budgeted latency bounds are met while preserving dynamic helper-call elimination.
- `2.` keep `B10C-01A-16` as a mandatory per-slice gate for every remaining `B10C-01` increment (rebuild via `env.sh` + strict audit + all-smoke, manifest refresh only on hash-mismatch-only smoke failures).
- `3.` defer `B10C-01A-18`+ until the `B10C-01A-17` checkpoint budget is met; only after add-lane checkpoint closure proceed to high-impact object lanes (`B10C-01A-18`..`B10C-01A-22`), medium-impact call/control lanes (`B10C-01A-23`..`B10C-01A-25`), and promotion gates (`B10C-01A-26`..`B10C-01A-27`).
- Why now: latest `B10C-01A-17` evidence still shows above-budget direct-lane latency (`+1.110 ns/op`, `+2.478%`) even after scratch-local init trimming, so redline promotion rule `#3` continues to block phase advance.
- Success evidence: updated checkpoint artifacts show no semantic regression, helper usage remains bounded (`wasm_return_fixnum_add` hot-lane dynamic calls/op stays `0`), and add-lane latency/instruction-path deltas are inside promotion budget before moving to `A-18`.
