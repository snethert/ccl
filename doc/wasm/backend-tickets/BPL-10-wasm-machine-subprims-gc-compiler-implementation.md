...
- High-level subprim overhead scan across wasm2 compiler callsites (`compiler/WASM/wasm2.lisp`) now confirms the largest remaining compatibility/subprim concentration is still in deferred `A-18+` object/call lanes: `.SPmisc-set` (`10`), `.SPmisc-alloc` (`8`), `.SPmisc-ref` (`7`), MV helpers (`.SPsave-values/.SPrecover-values/.SPadd-values` = `9` combined), and `.SPfuncall` (`3`), reinforcing the defer-until-budget rule for `B10C-01A-18+`.
...
- Action: keep promotion held at `B10C-01A-17` follow-up until measured add-lane regression is within budget.
- `1.` continue `B10C-01A-17` with follow-up optimization/measurement on the new typed-entry/prologue slice and rerun checkpoint evidence (`doc/wasm/tickets/evidence/bpl-10/b10c-01a-17-fixnum-add-checkpoint-2026-02-10.json`) to confirm budget bounds stay met while preserving helper-call elimination.
- `2.` keep `B10C-01A-16` mandatory on every remaining `B10C-01` slice (rebuild via `env.sh` + strict audit + manifest/image refresh + all-smoke).
- `3.` defer `B10C-01A-18`+ until the `B10C-01A-17` checkpoint budget is met consistently; only then proceed to high-impact object lanes (`B10C-01A-18`..`B10C-01A-22`), medium-impact call/control lanes (`B10C-01A-23`..`B10C-01A-25`), and promotion gates (`B10C-01A-26`..`B10C-01A-27`).
- Why now: latest checkpoint evidence is inside budget on this host (`-3.667 ns/op`, `-6.782%`), so this slice shifts to repeatability confirmation rather than first-pass budget recovery.
- Success evidence: refreshed checkpoint artifacts continue to show no semantic regression, helper usage remains bounded (`wasm_return_fixnum_add` hot-lane dynamic calls/op stays `0`), and add-lane latency/instruction-path deltas remain inside budget before moving to `A-18`.
