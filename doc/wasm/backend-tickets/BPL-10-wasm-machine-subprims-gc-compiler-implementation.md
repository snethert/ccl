...
- Action: start `B10G-03` safepoint integration contract pass by auditing wasm2 GC-capable call boundaries and pinning compiler publication semantics at those boundaries to the descriptor/mode contract.
- Why now: `B10G-02` forwarding-neutrality now has dedicated stress coverage and passes in baseline smoke, so next remaining GC correctness risk is compiler/runtime safepoint publication alignment.
- Success evidence: documented boundary map is implemented in compiler paths, audit remains `total_hits=0`, and smoke/regression lanes remain green.
