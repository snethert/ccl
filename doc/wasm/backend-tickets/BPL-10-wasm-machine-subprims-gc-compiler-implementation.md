...
- Action: start `B10G-04` phase 4 by extending non-local-exit coherence checks to funcall-driven `_SPnthrowvalues` unwind-cleanup boundaries and asserting `save_tsp`/`save_vsp` post-funcall coherence alongside existing frame invariants.
- Why now: phase 3 closed direct-path mv+cleanup coverage; the remaining `B10G-04` correctness risk is the same unwind-cleanup behavior when entered through `wasm_funcall_common`.
- Success evidence: phase-4 signatures added to `wasm_subprim_nonlocal_exit_coherence_selftest`, `scripts/wasm/arm-retirement-audit.sh --strict` remains `total_hits=0`, and `node doc/wasm/js/all-smoke.mjs` remains green.
