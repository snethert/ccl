...
- `1.` continue `B10C-01A-17` with follow-up optimization/measurement on the new typed-entry/prologue slice and rerun checkpoint evidence (`doc/wasm/tickets/evidence/bpl-10/b10c-01a-17-fixnum-add-checkpoint-2026-02-10.json`) until budget bounds are met while preserving helper-call elimination.
...
- `3.` defer `B10C-01A-18`+ until the `B10C-01A-17` checkpoint budget is met; only then proceed to high-impact object lanes (`B10C-01A-18`..`B10C-01A-22`), medium-impact call/control lanes (`B10C-01A-23`..`B10C-01A-25`), and promotion gates (`B10C-01A-26`..`B10C-01A-27`).
- Why now: latest checkpoint evidence remains above budget (`+1.110 ns/op`, `+2.478%`), so redline promotion rule `#3` still blocks phase advance.
- Success evidence: refreshed checkpoint artifacts show no semantic regression, helper usage remains bounded (`wasm_return_fixnum_add` hot-lane dynamic calls/op stays `0`), and add-lane latency/instruction-path deltas are inside budget before moving to `A-18`.
