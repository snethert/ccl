...
- `B10C-01A-17` follow-up optimization slice is now landed in `compiler/WASM/wasm2.lisp`: direct fixnum binary prologue now uses a combined pair-tag lowtag check, and add/sub overflow checks now consume one result-local copy in hot-path math to reduce local traffic. Smoke modules were regenerated (`doc/wasm/wasm-smoke-modules.bin`, `doc/wasm/wasm-smoke-modules.idx`) and evidence artifact `doc/wasm/tickets/evidence/bpl-10/b10c-01a-17-fixnum-add-checkpoint-2026-02-10.json` was refreshed; helper-call elimination remains intact (`wasm_return_fixnum_add`: dynamic calls/op `1 -> 0`) and instruction-path delta tightened (`+92` vs prior `+96`), but measured add-lane latency regression remains above budget on this host (`+13.579 ns/op`, `+32.468%` direct vs compat).
...
- Why now: latest `B10C-01A-17` evidence still shows above-budget direct-lane latency (`+13.579 ns/op`, `+32.468%`) even after instruction-path tightening, so redline promotion rule `#3` continues to block phase advance.
