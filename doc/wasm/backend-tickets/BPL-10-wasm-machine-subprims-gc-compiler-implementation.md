...
- `B10C-01A-17` performance checkpoint capture is now active in `doc/wasm/js/fixnum-add-smoke.mjs` with an explicit `--perf-checkpoint` lane that records compat-vs-direct latency plus static/dynamic instruction-path deltas; direct fixnum add/sub module emission validity is now fixed in `compiler/WASM/wasm2.lisp` (`local.tee` -> `local.set` in overflow-check paths), smoke modules are regenerated (`doc/wasm/wasm-smoke-modules.bin`, `doc/wasm/wasm-smoke-modules.idx`), and evidence is recorded at `doc/wasm/tickets/evidence/bpl-10/b10c-01a-17-fixnum-add-checkpoint-2026-02-10.json` (dynamic helper calls/op `1 -> 0`, measured add-lane latency delta `+12.288 ns/op` / `+29.851%` direct vs compat on this host).
...
- `3.` only after add-lane checkpoint closure, proceed to high-impact object lanes (`B10C-01A-18`..`B10C-01A-22`), medium-impact call/control lanes (`B10C-01A-23`..`B10C-01A-25`), and promotion gates (`B10C-01A-26`..`B10C-01A-27`).
- Why now: `B10C-01A-17` evidence is now captured and proves helper-call elimination on the direct lane, but current host numbers still show a measurable latency regression versus the compat baseline, so redline promotion rule `#3` blocks phase advance.
