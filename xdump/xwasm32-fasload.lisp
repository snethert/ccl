      ...
      (push (list :id id :name (format nil "module_~d" id)
                  :arity (destructuring-bind (schema required optional restp keysp allow-other-keys keys)
                           ...
                           (unless (and (= (wasm32-scratch-fixnum schema) 1)
                                        (= (logand (xload-%svref keys -1) 255) 250))
                             (error "Unsupported wasm32 callable metadata"))
                           ;; V2 admitted only an empty key vector. V3 keeps
                           ;; indices into SYMBOLS and publishes module-local
                           ;; wires, never scratch or target addresses.
                           (when (and (= (wasm32-scratch-fixnum version) 2)
                                      (/= (xload-%svref keys -1) 250))
                             (error "Unsupported wasm32 v2 keyword metadata"))
                           (list (wasm32-scratch-fixnum required)
                                 ...
                                 (loop for i below (ash (xload-%svref keys -1) -8)
                                       for index = (wasm32-scratch-fixnum (xload-%svref keys i))
                                       for wire = (find index (wasm32-scratch-list wires)
                                                        :key (lambda (pair)
                                                               (wasm32-scratch-fixnum (xload-cdr pair))))
                                       collect (if wire
                                                 (wasm32-scratch-string (xload-car wire))
                                                 (error "Missing wasm32 keyword symbol wire")))))
