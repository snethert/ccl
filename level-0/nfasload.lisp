...
(defun fasl-read-gvector (s subtype)
  ...
  (let* ((n (%fasl-read-count s))
    ...
    #+wasm32-target (when (= subtype target::subtag-function)
                      (%fix-fn-entrypoint vector))
...
(defun %simple-fasl-set-file-pos (s new)
  (let* ((fd (faslstate.faslfd s))
    ...
    (progn
      ...
      (fd-lseek fd new
                #+(or wasm32-target wasm-target) 0
                #-(or wasm32-target wasm-target) #$SEEK_SET))))
...
(defun %simple-fasl-open (string s)
  (let* ((ok nil)
	 (fd (fd-open string
                      #+(or wasm32-target wasm-target) 0
                      #-(or wasm32-target wasm-target) #$O_RDONLY))
	 (err 0))
    (declare (fixnum fd))
    (if (>= fd 0)
      (if (< (fd-lseek fd 0
                       #+(or wasm32-target wasm-target) 2
                       #-(or wasm32-target wasm-target) #$SEEK_END) 4)
        (setq err $xnotfasl)
        (progn
          (setq err 0)
          (setf (faslstate.bufcount s) 0
