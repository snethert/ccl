...
test("runtime command client dispatches command.invoke and resolves command.result", async () => {
  ...
  assert.equal(sent[0].kind, RUNTIME_MESSAGE_KINDS.commandInvoke);
  assert.equal(sent[0].payload.invocation.commandId, "runtime.eval.form");
  assert.equal(sent[0].payload.invocation.args.form, "(+ 1 2)");
  ...
  assert.equal(settled.requestId, sent[0].requestId);
...
test("runtime command client rejects on command.error and state marks invocation failed", async () => {
  ...
  await assert.rejects(dispatch.promise, (err) => {
    ...
    assert.equal(err.requestId, sent[0].requestId);
    return true;
  });
  assert.equal(client.pendingCount(), 0);
});
