...
1. Normal text (below 24 px regular / 19 px bold) MUST meet >= 4.5:1.
2. Large text (>= 24 px regular or >= 19 px bold) MUST meet >= 3:1.

Non-text contrast:

1. Visual boundaries for controls, icons, focus indicators, and selected rows MUST meet >= 3:1 against adjacent colors.

Focus visibility:

1. Focus indicator MUST be present for keyboard focusable controls.
2. Focus indicator width MUST be >= 2 px.
3. Focus indicator contrast against adjacent colors MUST be >= 3:1.
4. Focus indicator MUST remain visible in dark, light, high-contrast, and forced-colors modes.

Color dependency:

1. Color MUST NOT be the sole channel for critical state (`error`, `warning`, `selected`, `recommended`).
2. At least one non-color cue MUST exist (iconography, border pattern, label text, or structural marker).

## 4. Motion Accessibility Mapping

1. Reduced-motion setting MUST disable transform/position animations.
2. Reduced-motion mode MUST preserve semantic state transitions using non-motion cues.
3. Motion suppression MUST NOT remove focus, selection, or error feedback.
...
1. A semantic accessibility proxy MUST exist for keyboard focus and screen-reader mapping.
2. Proxy focus location MUST match rendered target bounds within +/- 1 px.
3. Proxy state (`selected`, `error`, `disabled`) MUST mirror rendered state exactly.
4. Loss of proxy synchronization MUST be treated as a conformance failure.

## 7. Failure Semantics

If any criterion fails:

1. The UI MUST degrade to a safer accessible fallback presentation.
2. The conformance run MUST fail with criterion ID and component/state context.
3. A structured diagnostic event MUST be emitted for telemetry correlation.
...
The following IDs MUST be used in accessibility reports:
