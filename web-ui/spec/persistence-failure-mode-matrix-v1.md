...
This matrix defines exactly which failure points MUST be tested, what state outcomes are required, and which fixture IDs provide evidence.

The matrix MUST be used as:
...
1. All `P0` rows MUST have automated fixture coverage with deterministic replay.
...
| `CRASH-OBJ-001` | P1 | crash during split immutable write before chunk manifest/closure completion | ref not advanced, incomplete closure unreadable | `ERR_OBJECT_TXN_FAILED` | `pending.crash.object_split_before_manifest.v1` | scaffolded |
| `CRASH-RTX-001` | P1 | crash after `reftxn` prepared marker before metadata CAS apply | no ref moved or deterministic repair on startup | `ERR_METADATA_TXN_FAILED` | `pending.crash.reftxn_prepared.v1` | scaffolded |
| `CRASH-REC-001` | P1 | crash during startup recovery scanner | scanner restart is idempotent; no guessed ref rewrites | `ERR_RECOVERY_MANUAL_ACTION_REQUIRED` | `pending.crash.recovery_scanner_restart.v1` | scaffolded |
...
| `LEASE-RACE-003` | P1 | lease heartbeat lost immediately before ref advance | in-flight write rejected by epoch/token check | `ERR_LEASE_LOST` | `pending.lease.heartbeat_loss_before_write.v1` | scaffolded |
| `LEASE-RACE-004` | P1 | simultaneous takeover attempts after expiry | metadata commit order tie-break is deterministic | `ERR_LEASE_HELD` | `pending.lease.double_takeover_tiebreak.v1` | scaffolded |
...
| `SYNC-NET-005` | P1 | remote ref CAS mismatch during push | no force-update; divergence path recorded | `ERR_SYNC_REF_CAS_MISMATCH` | `pending.sync.push.cas_mismatch_divergence.v1` | scaffolded |
| `SYNC-NET-006` | P1 | remote object hash mismatch on pull | object rejected; ref unchanged | `ERR_SYNC_OBJECT_HASH_MISMATCH` | `pending.sync.pull.hash_mismatch_reject.v1` | scaffolded |
| `SYNC-NET-007` | P1 | duplicate push retry after client timeout | idempotent object uploads, no duplicate side effects | `ERR_SYNC_NETWORK` | `pending.sync.push.retry_idempotent_timeout.v1` | scaffolded |

## 6. Required Invariant Checks per Fixture

Every fixture in this matrix MUST assert at minimum:
...
A fixture run MUST be replayable byte-for-byte in serialized result output for identical seed and fixture version.

## 8. Evidence and Traceability

Each matrix row MUST map to:
...
1. All listed `P0` rows in sections 3-5 are covered.
2. `P1` rows are now scaffolded in fixtures and remain pending promotion to covered.
3. Coverage is strong for first-pass crash/lease/sync behavior, but not yet exhaustive for recovery scanner and remote corruption branches.

## 10. Conformance

Persistence fault coverage is conformant to this matrix only if:

1. every `P0` row is covered by automated fixtures,
2. fixtures pass deterministically on repeated runs,
3. fixture assertions include required invariant checks,
4. uncovered rows are explicitly marked pending with target fixture IDs.
