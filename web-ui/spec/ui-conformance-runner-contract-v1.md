...
The runner MUST execute using the `executionProfile` declared in `ui-conformance-fixtures-v1.json`:
...
1. If a fixture declares `modeLanes`, the runner MUST execute only those modes; otherwise all profile modes apply.
2. If a fixture declares `viewportLanes`, the runner MUST execute only those viewports; otherwise profile default viewport applies.
3. If a fixture declares `parityBackendPairs` and uses `requiredHarness=parity`, the runner MUST execute only those backend pairs and MUST NOT synthesize additional backend pairs.
4. Every backend named in `parityBackendPairs` MUST be present in the fixture's effective backend lane set; missing or unknown backend members are fixture failures (`runner-lane-missing`).
5. If a fixture declares backend scope indirectly via test binding, the runner MUST still annotate report rows with effective backend.
6. Any missing declared lane is a fixture failure (`runner-lane-missing`).

### 3.3 Clock and Scheduling

1. Timing-based harnesses MUST use monotonic time.
2. Animation scheduling assumptions MUST match `executionProfile.timing.animationFrameHz`.
3. Measured drift constraints MUST use fixture assertion thresholds, not hardcoded defaults.
...
4. Snapshot harness MUST fail on structural mismatch, missing required node/state cues, or non-deterministic serialization.
...
3. For coarse-pointer lanes, effective interactive target size MUST include hit-slop expansion where applicable.
...
3. Mode-specific contrast checks MUST run in all declared mode lanes.
...
3. Keyboard harness MUST fail on dead focus, missing indicator, or nondeterministic traversal.

## 5. Assertion Evaluation Rules

1. `==` requires exact equality after harness normalization.
2. `<=` and `>=` are inclusive.
3. If `tolerance` exists, it applies only to numeric metrics and MUST be explicit in report output.
...
1. Each fixture result MUST list emitted artifact IDs/paths from its fixture definition.
2. Missing required artifact is a fixture failure (`runner-artifact-missing`).
3. Runner MUST emit structured diagnostics for lane/setup failures, measurement failures, and unsupported effects.

## 7. Report Emission Contract

1. Report root fields and fixture rows MUST validate against `ui-conformance-report-schema-v1.json`.
2. `env.mode` and `env.backend` MAY be lane-specific values or `mixed` when aggregating multi-lane runs.
3. Assertion rows MUST include `id`, `metric`, `comparator`, `expected`, and `actual`.
4. Failed assertions MUST include enough detail to deterministically reproduce failure (lane tuple + binding).

## 8. Failure Semantics

If a required fixture cannot run or fails:

1. Fixture status MUST be `failed`.
2. Run summary status MUST be `failed`.
3. Runner MUST continue executing remaining required fixtures unless aborted by explicit policy.
4. Final report MUST include all encountered fixture failures.
