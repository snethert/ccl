...
(progn
  ...
  (defvar *runtime-inspector-current-target-id* nil)
  (defvar *runtime-watch-store* nil)
  (defvar *runtime-edit-group-store* nil)
  (defvar *runtime-edit-group-counter* 0)
  (defvar *runtime-job-store* nil)
  (defvar *runtime-job-counter* 0)
  ...
  (defun runtime-command--coerce-list (value)
    (cond
      ((vectorp value) (coerce value 'list))
      ((listp value) value)
      (t nil)))

  (defun runtime-command--truthy-p (value)
    (and value (not (eq value :false)) (not (eq value :null))))

  (defun runtime-command--watch-id (watch)
    (runtime-command--alist-value watch "id" nil))
  ...
  (defun runtime-command--upsert-watch (watch)
    (let* ((watch-id (runtime-command--watch-id watch))
           (next nil)
           (replaced nil))
      (dolist (entry *runtime-watch-store*)
        (if (and watch-id (string= (runtime-command--watch-id entry) watch-id))
          (progn
            (push watch next)
            (setf replaced t))
          (push entry next)))
      (unless replaced
        (push watch next))
      (setf *runtime-watch-store* (nreverse next))
      watch))

  (defun runtime-command--remove-watch (watch-id)
    (let* ((removed nil))
      (setf *runtime-watch-store*
            (remove-if
             (lambda (watch)
               (let ((match (and watch-id (string= (runtime-command--watch-id watch) watch-id))))
                 (when match (setf removed t))
                 match))
             *runtime-watch-store*))
      removed))

  (defun runtime-command--normalize-edit (edit)
    (let* ((place-id (runtime-command--alist-value edit "placeId" nil))
           (label (runtime-command--alist-value edit "label" nil))
           (before (runtime-command--alist-value edit "before" nil))
           (after (runtime-command--alist-value edit "after" nil)))
      (list (cons "placeId" place-id)
            (cons "label" (or label place-id "Edit"))
            (cons "before" before)
            (cons "after" after))))

  (defun runtime-command--normalize-edits (args)
    (let* ((raw-edits (runtime-command--alist-value args "edits" nil))
           (edits-list (runtime-command--coerce-list raw-edits))
           (normalized nil))
      (if edits-list
        (dolist (entry edits-list (nreverse normalized))
          (when (and (listp entry) (every #'consp entry))
            (push (runtime-command--normalize-edit entry) normalized)))
        (let* ((place-id (runtime-command--alist-value args "placeId" nil))
               (label (runtime-command--alist-value args "label" nil))
               (before (runtime-command--alist-value args "before" nil))
               (after (runtime-command--alist-value args "after" nil)))
          (if (or place-id before after)
            (list (list (cons "placeId" place-id)
                        (cons "label" (or label place-id "Edit"))
                        (cons "before" before)
                        (cons "after" after)))
            nil)))))

  (defun runtime-command--edit-group-id (group)
    (runtime-command--alist-value group "id" nil))

  (defun runtime-command--upsert-edit-group (group)
    (let* ((group-id (runtime-command--edit-group-id group))
           (next nil)
           (replaced nil))
      (dolist (entry *runtime-edit-group-store*)
        (if (and group-id (string= (runtime-command--edit-group-id entry) group-id))
          (progn
            (push group next)
            (setf replaced t))
          (push entry next)))
      (unless replaced
        (push group next))
      (setf *runtime-edit-group-store* (nreverse next))
      group))

  (defun runtime-command--find-edit-group (edit-group-id)
    (find edit-group-id
          *runtime-edit-group-store*
          :test #'string=
          :key #'runtime-command--edit-group-id))

  (defun runtime-command--emit-edit-group-update (edit-group status &key request-id error)
    (let* ((label (runtime-command--alist-value edit-group "label" nil))
           (edit-group-id (runtime-command--alist-value edit-group "id" nil))
           (audit-text (format nil "~a edit group ~a"
                               (string-capitalize status)
                               (or edit-group-id "unknown"))))
      (runtime-command--emit-inspector-update
       (list (cons "type" "edit-group")
  ...
  (defun runtime-command--watch-pin (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (entry-id (runtime-command--alist-value args "entryId" nil))
           (presentation-id (runtime-command--alist-value args "presentationId" nil))
           (recording-id (runtime-command--alist-value args "recordingId" nil))
           (requested-id (runtime-command--alist-value args "watchId" nil))
           (label (runtime-command--alist-value args "label" nil))
           (value-summary (runtime-command--alist-value args "valueSummary" nil))
           (target (or presentation-id entry-id recording-id)))
      (unless target
        (error "runtime.watch.pin requires entryId, presentationId, or recordingId"))
      (let* ((watch-id (or requested-id (format nil "watch-~d" (1+ (length *runtime-watch-store*)))))
             (watch (list (cons "id" watch-id)
                          (cons "kind" "presentation")
                          (cons "label" (or label (format nil "Watch ~a" target)))
                          (cons "entryId" entry-id)
                          (cons "presentationId" presentation-id)
                          (cons "recordingId" recording-id)
                          (cons "valueSummary"
                                (or value-summary
                                    (if presentation-id
                          ...
                          (cons "pinned" t)
                          (cons "updatedAt" now))))
        (runtime-command--upsert-watch watch)
        (runtime-command--emit-inspector-update
  ...
  (defun runtime-command--watch-unpin (args context)
    (declare (ignore context))
    (let* ((watch-id (runtime-command--alist-value args "watchId" nil)))
      (unless (and (stringp watch-id) (> (length watch-id) 0))
        (error "runtime.watch.unpin requires args.watchId"))
      (unless (runtime-command--remove-watch watch-id)
        (error "Unknown watch id: ~a" watch-id))
      (runtime-command--emit-inspector-update
       (list (cons "type" "watch.sync")
             (cons "watches" (runtime-command--watch-vector)))
       nil)
  ...
  (defun runtime-command--inspect-presentation (args)
    (let* ((presentation-id (runtime-command--alist-value args "presentationId" nil))
           (target-id (or presentation-id *runtime-inspector-current-target-id* "runtime-last-target"))
           (summary (if presentation-id
                      (format nil "inspect ~a" presentation-id)
                      (format nil "inspect ~a" target-id)))
           (place-id (format nil "pl-~a-value" target-id))
           (row (list (cons "id" (format nil "row-~a" target-id))
                      (cons "label" "Value")
                      (cons "valueSummary" summary)
                      (cons "presentationId" target-id)
                      (cons "place" (list (cons "placeId" place-id)
                                          (cons "description" "Value place")
                                          (cons "editable" t)))))
           (section (list (cons "id" "value")
                          (cons "title" "Value")
                          (cons "rows" (vector row))))
           (view (list (cons "type" "value")
                       (cons "summary" summary)
                       (cons "sections" (vector section))))
           (payload (list (cons "type" "snapshot")
                          (cons "targetId" target-id)
                          (cons "targetType" "value")
                          (cons "view" view)
                          (cons "watches" (runtime-command--watch-vector))
                          (cons "stale" nil))))
  ...
  (defun runtime-command--place-stage (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (edit-group-id (or (runtime-command--alist-value args "editGroupId" nil)
                              (format nil "edit-group-~d" (incf *runtime-edit-group-counter*))))
           (label (runtime-command--alist-value args "label" nil))
           (edits (runtime-command--normalize-edits args)))
      (unless edits
        (error "runtime.place.stage requires edits or placeId/after payload"))
      (let* ((group (list (cons "id" edit-group-id)
                          (cons "label" (or label edit-group-id))
                          (cons "status" "staged")
                          (cons "edits" (coerce edits 'vector))
                          (cons "createdAt" now)
                          (cons "updatedAt" now))))
        (runtime-command--upsert-edit-group group)
        (runtime-command--emit-edit-group-update group "staged")
        (list (cons "editGroupId" edit-group-id)
              (cons "status" "staged")))))

  (defun runtime-command--place-apply (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (edit-group-id (runtime-command--alist-value args "editGroupId" nil))
           (existing (and (stringp edit-group-id) (runtime-command--find-edit-group edit-group-id))))
      (unless (and (stringp edit-group-id) (> (length edit-group-id) 0))
        (error "runtime.place.apply requires args.editGroupId"))
      (unless existing
        (let* ((failed (list (cons "id" edit-group-id)
                             (cons "label" edit-group-id)
                             (cons "status" "failed")
                             (cons "edits" #())
                             (cons "failedAt" now)
                             (cons "updatedAt" now))))
          (runtime-command--emit-edit-group-update failed "failed" :error "Unknown edit group")
          (error "Unknown edit group: ~a" edit-group-id)))
      (let* ((updated (append
                       ...
                       (list (cons "status" "applied")
                             (cons "appliedAt" now)
                             (cons "updatedAt" now)))))
        (runtime-command--upsert-edit-group updated)
        (runtime-command--emit-edit-group-update updated "applied")
        (list (cons "editGroupId" edit-group-id)
              (cons "status" "applied")))))

  (defun runtime-command--place-undo (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (edit-group-id (runtime-command--alist-value args "editGroupId" nil))
           (existing (and (stringp edit-group-id) (runtime-command--find-edit-group edit-group-id))))
      (unless (and (stringp edit-group-id) (> (length edit-group-id) 0))
        (error "runtime.place.undo requires args.editGroupId"))
      (unless existing
        (let* ((failed (list (cons "id" edit-group-id)
                             (cons "label" edit-group-id)
                             (cons "status" "failed")
      ...
      (let* ((updated (append
                       (remove-if (lambda (pair)
                                    (member (car pair)
                                            '("status" "undoneAt" "updatedAt")
                                            :test #'string=))
                                  existing)
                       (list (cons "status" "undone")
                             (cons "undoneAt" now)
                             (cons "updatedAt" now)))))
        (runtime-command--upsert-edit-group updated)
        (runtime-command--emit-edit-group-update updated "undone")
        (list (cons "editGroupId" edit-group-id)
              (cons "status" "undone")))))

  (defun runtime-command--job-upsert (job)
    (let* ((job-id (runtime-command--alist-value job "id" nil))
           (next nil)
           (replaced nil))
      (dolist (entry *runtime-job-store*)
        (if (and job-id (string= (runtime-command--alist-value entry "id" nil) job-id))
          (progn
            (push job next)
            (setf replaced t))
          (push entry next)))
      (unless replaced
        (push job next))
      (setf *runtime-job-store* (nreverse next))
      job))

  (defun runtime-command--job-start (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (job-id (or (runtime-command--alist-value args "jobId" nil)
                       (format nil "job-~d" (incf *runtime-job-counter*))))
           (kind (or (runtime-command--alist-value args "kind" nil) "compile"))
           (label (or (runtime-command--alist-value args "label" nil) "Background Job"))
           (task-id (runtime-command--alist-value args "taskId" nil))
           (total (let ((value (runtime-command--alist-value args "total" 3)))
                    (if (integerp value) (max 1 value) 3)))
           (job-base (list (cons "id" job-id)
                           (cons "kind" kind)
                           (cons "label" label)
                           (cons "taskId" task-id))))
  ...
  (defun runtime-command--job-cancel (args context)
    (declare (ignore context))
    (let* ((now (runtime-bridge--now-ms))
           (job-id (runtime-command--alist-value args "jobId" nil))
           (job (and (stringp job-id)
                     (find job-id
                           *runtime-job-store*
                           :test #'string=
                           :key (lambda (entry) (runtime-command--alist-value entry "id" nil)))))
           (label (if job
                    (runtime-command--alist-value job "label" nil)
                    "Background Job"))
           (kind (if job
                   (runtime-command--alist-value job "kind" nil)
                   "compile"))
           (task-id (if job
                      (runtime-command--alist-value job "taskId" nil)
                      nil)))
      (unless (and (stringp job-id) (> (length job-id) 0))
        (error "runtime.job.cancel requires args.jobId"))
      (let* ((failed (list (cons "id" job-id)
                           (cons "kind" kind)
                           (cons "label" label)
                           (cons "taskId" task-id)
                           (cons "status" "failed")
      ...
      (handler-case
          (let* ((result
                  (cond
                    ...
                    ((string= command-id "runtime.watch.pin")
                     (runtime-command--watch-pin args context))
                    ((string= command-id "runtime.watch.unpin")
                     (runtime-command--watch-unpin args context))
                    ((string= command-id "runtime.place.stage")
                     (runtime-command--place-stage args context))
                    ((string= command-id "runtime.place.apply")
                     (runtime-command--place-apply args context))
                    ((string= command-id "runtime.place.undo")
                     (runtime-command--place-undo args context))
                    ((string= command-id "runtime.job.start")
                     (runtime-command--job-start args context))
                    ((string= command-id "runtime.job.cancel")
                     (runtime-command--job-cancel args context))
                    (t
  ...
  (let ((poll-buffer (make-array +runtime-command-max-bytes+ :element-type '(unsigned-byte 8)))
